...
## Stage78-1 CTP/SimNow 虚拟盘 SOP

1. 当用户要求运行或解释第78-1的每日影子盘、SimNow/券商测试虚拟盘、CTP/SimNow通路、Phase B委托草案/提交前闸门、1手报撤smoke order、AI池月更、`review`风险层级、每日对账或“今天/今晚/下个交易时段是否需要交易”时，先读取并遵循 `skills/stage78-simnow-shadow-sop/SKILL.md`。
2. 该 skill 只约束执行纪律与SOP，不用于优化策略alpha；若同时涉及新策略版本或A/B实验，仍需遵循 `skills/version-ab-experiment/SKILL.md`。
3. Stage78-1虚拟盘和实盘前流程默认只认50万口径；旧30万入口只能作为历史对照，不得作为当前执行路径。
