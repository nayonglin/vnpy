...
## 2026-04-27 11:17 CST - 第199阶段：股票震荡数据可用性审计，确认有研究脚手架但不能直接严肃回测
...
- 策略路线：股票震荡独立研究线
- 是否进入第78 A/B/C：否
- 是否重要突破版本：否。本阶段是数据审计，不是策略版本。
- 是否读取`skills/version-ab-experiment/SKILL.md`：否
  - 原因：本阶段没有新交易规则落地，也不讨论接入正式版本。
...
- 只读取现有股票 alpha 缓存：
  - `examples/alpha_research/native_results/cache/stock_panel.parquet`
  - `examples/alpha_research/native_results/cache/benchmark.parquet`
- 未修改期货第78趋势策略、期货震荡策略、配置或回测入口。
- 未新增、修改或删除交易参数。
- 未跑策略回测。
...
- 审计固定口径：
  - `HOLDING_DAYS=5`
  - 股票缓存范围：`2025-01-02`到`2026-04-17`
  - 股票数量：`300`
...
- 当前仓库没有发现已经落地的“股票震荡/股票均值回归”研究历史。
- 已有股票相关资产主要是：
  - `vnpy/alpha`股票 alpha 研究框架
  - `examples/alpha_research`成交量因子研究脚本
  - CSI1000/Tushare/Baostock 数据准备脚手架
  - 现有 parquet 缓存与信号示例
- 数据覆盖：
  - 原始行：`93,285`
  - 股票数：`300`
  - 基准交易日：`311`
  - 预期 symbol-date 行：`93,300`
  - 缺口：`15`
  - 覆盖率：`99.98%`
- 数据质量：
  - 重复 symbol-date 行：`0`
  ...
  - ST行：`70`
- 交易/标签过滤：
  - 5日标签与交易过滤后保留：`91,509`行
  - 保留比例：`98.10%`
  - 有效标签日期：`306`
  - 有效日期每日可交易横截面最小/中位/最大：`295 / 299 / 300`

### 核心判断

- 可以做股票震荡方向的第一轮信号层研究，但不能直接做严肃正式回测。
- 主要风险：
  - 幸存者偏差高：现有脚本使用最新中证1000成分，且缓存只有`300`只，不是历史成分全量面板。
  - 复权风险高：生成脚本使用 baostock `adjustflag="3"`，从代码看是不复权口径，缺复权因子。
  - 历史长度短：只有约一年多，不能证明穿越牛熊/风格周期。
- Polanyi式经验判断：
  - 这份数据适合闻股票横截面反弹有没有“气味”，但不是能上正式战场的地图。
...
- 覆盖率：
  - `examples/alpha_research/native_results/stock_range_reversion_data_audit_v1_symbol_coverage.csv`
  - `examples/alpha_research/native_results/stock_range_reversion_data_audit_v1_date_coverage.csv`
- 数据质量：
  - `examples/alpha_research/native_results/stock_range_reversion_data_audit_v1_quality_summary.csv`
- 过滤损耗：
  - `examples/alpha_research/native_results/stock_range_reversion_data_audit_v1_filter_summary.csv`
  - `examples/alpha_research/native_results/stock_range_reversion_data_audit_v1_daily_tradable_width.csv`
- 汇总：
  - `examples/alpha_research/native_results/stock_range_reversion_data_audit_v1_summary.json`
...
- 判断：否。
- 原因：本阶段只做数据边界和交易约束审计，不评价收益阈值，不选择策略参数。

### 运行前继续价值反思

- 判断：是。
- 原因：股票震荡策略若不先确认数据覆盖、复权、停牌/ST/涨跌停处理，后续很容易得到漂亮但虚假的结果。

### 运行后过拟合反思

- 判断：否。
- 原因：输出是数据层事实和风险清单，不产生交易规则。

### 运行后继续价值反思

- 判断：是，但只适合继续做信号层 smoke test。
- 原因：横截面宽度和基础交易过滤足够做第一轮归因，但幸存者偏差、复权和历史长度仍不足以支持正式回测。

### 决策

- `stock_range_reversion_data_audit_smoke_only`
- 股票震荡路线与期货趋势/期货震荡路线隔离。
- 不接入第78。
- 不进入第78 A/B/C。
- 不直接写股票组合回测。
- 下一步做固定口径横截面超跌反弹信号层归因。

### 后续规划和TODO

- 优先做股票长侧超跌反弹信号层归因。
- 信号必须先用横截面rank，不用固定RSI阈值。
- 同步规划数据修复：
  - 复权口径确认
  - 历史成分扩展
  - 更长年份覆盖
...
## 2026-04-27 11:18 CST - 第200阶段：股票横截面超跌反弹信号层归因，初步有边际但只能作为smoke test
...
- 信号日收盘后，按次日收盘入场。
- 持有`1/3/5/10`个交易日。
- 收益口径为股票未来收益减中证1000同期收益。
- 每日横截面分为五组：
  - group 5为分数最高，即更超跌/更低于均线/更强放量下跌压力。
- 固定朴素信号：
...
- 5日持有口径：
  - `score_below_ma20`：top-bottom平均超额收益`0.2656%`，Rank IC均值`0.0652`，t值`1.88`，正向日占比`56.29%`。
  - `score_oversold_ret_20`：top-bottom平均超额收益`0.2464%`，Rank IC均值`0.0671`，t值`1.59`，正向日占比`54.74%`。
  - `score_oversold_ret_10`：top-bottom平均超额收益`0.1864%`，Rank IC均值`0.0556`。
  - `score_oversold_ret_5`：top-bottom平均超额收益`0.0454%`，Rank IC均值`0.0329`。
- 全部 horizon 中：
  - top-bottom平均最高的是`score_below_ma20`/`10日`：平均`0.5633%`，t值`2.84`。
  - `score_oversold_ret_20`/`10日`：平均`0.4663%`，t值`2.15`。
- 市场状态粗分：
  - 5日`score_below_ma20`在`market_down_20d`中 top-bottom均值`0.5637%`，正向日占比`64.21%`。
  - 5日`score_below_ma20`在`market_up_20d`中 top-bottom均值`0.1334%`，正向日占比`52.63%`。
  - 5日`score_oversold_ret_20`在`market_down_20d`中 top-bottom均值`0.6109%`。

### 核心判断

- 股票横截面超跌反弹有初步边际，尤其是低于20日均线和20日超跌方向。
- 但这不是正式策略证据：
  - 样本只有`300`只当前成分股。
  - 历史只有`2025-01-02`到`2026-04-17`。
  - 数据可能是不复权口径。
  - 存在幸存者偏差。
- 不应把`score_below_ma20`直接交易化，也不应围绕当前样本调阈值。
- Polanyi式经验判断：
  - 这一步看的是“水温”，不是下水游泳；有温度，但水域还没探清。
...
- 汇总：
  - `examples/alpha_research/native_results/stock_range_reversion_signal_attribution_v1_summary.csv`
- 分组收益：
  - `examples/alpha_research/native_results/stock_range_reversion_signal_attribution_v1_group_returns.csv`
- 市场状态分层：
  - `examples/alpha_research/native_results/stock_range_reversion_signal_attribution_v1_market_state_summary.csv`
- 元信息：
  - `examples/alpha_research/native_results/stock_range_reversion_signal_attribution_v1_meta.json`
...
- 判断：轻微但受控。
- 原因：固定测试多个朴素信号会天然带来“看最好结果”的诱惑；但本阶段不改参数、不调阈值、不写策略，只作为方向 smoke test。

### 运行前继续价值反思

- 判断：是。
- 原因：如果固定朴素超跌信号都没有方向，股票震荡路线应优先停止策略设计、回到数据建设；如果有初步方向，则值得在更长更干净的数据上复验。

### 运行后过拟合反思

- 判断：轻微但未越界。
- 原因：`score_below_ma20`表现最好，但不能直接交易化；当前样本短、可能未复权、有幸存者偏差。

### 运行后继续价值反思

- 判断：是，但下一步不是策略化，而是复验和数据修复。
- 原因：初步信号边际为正，说明股票震荡路线有研究价值；但数据质量不足以支持资金曲线或正式版本。

### 决策

- `stock_range_reversion_signal_attribution_smoke_positive_not_tradable`
...
- 优先补股票数据：
  ...
  - 扩展历史中证1000成分和更长年份。
  - 增加退市、新股、上市天数、流动性过滤。
- 在数据修复前，若继续研究，只做固定口径归因：
  - 分行业/市值/流动性层归因。
  - 熊市/震荡/上涨市场状态分层。
  - 不做交易规则和资金曲线。

## 2026-04-27 11:41 CST - 第201阶段：股票震荡研究面板v1，补齐前复权价、原始交易约束、上市天数和流动性字段
...
- 策略路线：股票震荡独立研究线
- 是否进入第78 A/B/C：否
- 是否重要突破版本：否。本阶段是数据底座升级，不是交易策略版本。
- 是否读取`skills/version-ab-experiment/SKILL.md`：否
  - 原因：本阶段没有新交易规则落地，也不讨论接入正式版本。
...
- 修改股票数据审计脚本，使其可通过环境变量读取新面板：
  - `examples/alpha_research/analyze_stock_range_reversion_data_audit.py`
- 修改股票信号归因脚本，使其可通过环境变量读取新面板：
  - `examples/alpha_research/analyze_stock_range_reversion_signal_attribution.py`
- 构建新研究面板：
  - 输出目录：`examples/alpha_research/native_results/stock_range_reversion_cache/`
- 未修改期货第78趋势策略、期货震荡策略、配置或回测入口。
- 未新增、修改或删除交易参数。
- 未跑策略回测。
...
- 股票池：
  - `UNIVERSE_SOURCE=existing_cache`
  - 股票数：`300`
  - 由于当前环境缺少`TUSHARE_TOKEN`，历史成分数据不可用。
  - 当前面板仍为静态股票池，不能完全消除幸存者偏差。
- 价格口径：
  - `raw_adjustflag=3`：原始价，用于停牌、涨跌停、成交量、成交额、交易约束。
  - `qfq_adjustflag=2`：前复权价，用于信号和收益研究。
- 新增字段：
  - `raw_open/high/low/close/preclose`
  - `qfq_open/high/low/close/preclose`
  - `raw_up_limit/raw_down_limit`
  - `is_oneword_limit_up/is_oneword_limit_down`
  - `listing_days`
  - `adv20_turnover/adv20_volume`
  - `eligible_research_row`
  - `has_historical_component_source`
...
- 新面板数据审计：
  ...
  - ST行：`70`
- 新面板固定信号归因：
  - 5日`score_oversold_ret_20`：top-bottom平均超额收益`0.2220%`，Rank IC`0.0664`，t值`1.41`，正向日占比`54.04%`。
  - 5日`score_oversold_ret_10`：top-bottom平均超额收益`0.2158%`，Rank IC`0.0582`，t值`1.65`，正向日占比`53.85%`。
  - 5日`score_below_ma20`：top-bottom平均超额收益`0.2019%`，Rank IC`0.0637`，t值`1.42`，正向日占比`53.50%`。
  - 10日`score_below_ma20`：top-bottom平均超额收益`0.5052%`，Rank IC`0.0763`，t值`2.53`。

### 核心判断

- 复权和交易约束补齐后，股票横截面超跌反弹信号仍有初步边际，但强度比旧缓存口径略降。
- 这说明旧口径可能存在一定乐观成分，新口径更接近真实可研究状态。
- 当前最强结论不是“可以交易”，而是：
  - 20日超跌/低于20日均线方向值得在更长、更干净的数据上复验。
  - 放量下跌压力信号在新口径下表现弱，不应优先推进。
- 最大剩余缺口仍是历史成分：
  - 没有`TUSHARE_TOKEN`和`index_weight`权限时，无法完全消除中证1000历史成分幸存者偏差。
- Polanyi式经验判断：
  - 这不是一条强到可以马上下场的 alpha，更像真实市场里微弱但可重复闻到的回摆气味；继续价值在数据补齐和分层归因，不在马上写策略。
...
- 成分记录：
  - `examples/alpha_research/native_results/stock_range_reversion_cache/stock_range_reversion_components.parquet`
- 构建报告：
  - `examples/alpha_research/native_results/stock_range_reversion_cache/stock_range_reversion_research_manifest.md`
  - `examples/alpha_research/native_results/stock_range_reversion_cache/stock_range_reversion_research_manifest.json`
- 新面板审计：
  - `examples/alpha_research/native_results/stock_range_reversion_cache/audit/stock_range_reversion_data_audit_v1_report.md`
- 新面板信号归因：
  - `examples/alpha_research/native_results/stock_range_reversion_cache/signal/stock_range_reversion_signal_attribution_v1_report.md`
  - `examples/alpha_research/native_results/stock_range_reversion_cache/signal/stock_range_reversion_signal_attribution_v1_summary.csv`
...
- 判断：否。
- 原因：本阶段是数据口径补齐和固定口径复验，不调收益阈值、不选择交易参数、不产生资金曲线。

### 运行前继续价值反思

- 判断：是。
- 原因：第200阶段已显示股票超跌信号有初步边际，但原数据存在复权、上市天数、流动性和交易约束缺口；补齐这些口径比继续做策略更有价值。

### 运行后过拟合反思

- 判断：否，但不能交易化。
- 原因：新口径下只是复验固定信号；信号边际保留但变弱，不能据此调阈值或写交易版本。

### 运行后继续价值反思

- 判断：是。
- 原因：数据补齐后信号仍未消失，说明股票震荡路线值得继续；但继续方向应是历史成分和更长样本，而不是组合回测。

### 决策

- `stock_range_reversion_research_panel_v1_data_foundation`
- 后续股票震荡归因优先使用新面板。
- 不把当前信号直接交易化。
- 不做股票组合回测。
- 不接入第78。
...
- 第一优先级：
  - 配置`TUSHARE_TOKEN`并验证`index_weight`权限，补历史中证1000成分，消除核心幸存者偏差。
- 第二优先级：
  - 将同一固定信号口径扩展到更长历史。
  - 做市场状态、行业、市值、流动性分层归因。
- 暂不做：
  - 不写股票策略资金曲线。
  - 不扫阈值。
  - 不把`score_oversold_ret_20`或`score_below_ma20`直接作为交易规则。

## 2026-04-27 13:33 - 第202阶段：Tushare历史成分权限验证与IP限制归因
...
- 与期货震荡策略关系：
  ...
  - 股票震荡继续作为独立数据研究线。
- 本阶段目标：
  - 使用用户提供的 Tushare token 验证 `index_weight` 权限。
  - 补强历史成分失败保护，避免数据构建失败时静默退回静态股票池。
  - 验证历史成分 snapshot 映射是否可用于后续消除幸存者偏差。
- 本阶段未跑策略回测。
...
- 修改股票信号归因脚本：
  - `examples/alpha_research/analyze_stock_range_reversion_signal_attribution.py`
- 关键补强：
  - 新增 `ALLOW_UNIVERSE_FALLBACK`，默认值为`0`，Tushare历史成分失败时不再静默退回静态股票池。
  - 新增 `TUSHARE_RETRIES`，默认值为`3`。
  - 新增 `TUSHARE_RETRY_SLEEP`，默认值为`2.0`秒。
  - 新增历史成分 snapshot 映射，研究日只使用当日之前最近一次已知指数成分。
  - 新增 `is_index_component`、`component_weight`、`component_snapshot_date`、`eligible_component_row`。
  - 审计和归因脚本在存在 `eligible_component_row` 时优先使用成分内可研究样本。
...
- 删除交易参数：无
- 新增数据构建参数：
  - `ALLOW_UNIVERSE_FALLBACK=0`
  - `TUSHARE_RETRIES=3`
  - `TUSHARE_RETRY_SLEEP=2.0`
...
- Tushare `index_weight` 单月轻量查询成功：
  - 说明 token 可用。
  - 说明当前积分层级可访问 `index_weight`。
- 官方权限口径：
  - `index_weight` 需要至少`2000`积分。
  - 用户当前已充值到`5000`积分，积分不是当前阻断点。
- 小样本历史成分 smoke 成功：
  - 输出目录：`examples/alpha_research/native_results/stock_range_reversion_cache_tushare_smoke/`
  - `UNIVERSE_SOURCE=tushare_csi1000`
  - `MAX_SYMBOLS=5`
  - 日期范围：`2025-01-01`到`2025-02-28`
  ...
  - snapshot 日期可映射到研究日。
- 全量历史成分构建失败原因：
  - Tushare 返回：`您的IP数量超限，最大数量为10个！`
  - 当前出口 IP 观测值：`203.208.189.11`
  - 判断：账号/IP侧限制，不是积分不足，也不是脚本逻辑失败。
...
- 判断：否。
- 原因：本阶段只是验证数据权限、历史成分口径和失败保护，不触碰策略参数与收益优化。

### 运行前继续价值反思

- 判断：是。
- 原因：第201阶段最大残缺是历史成分；如果不补这一层，后续股票震荡信号很容易被幸存者偏差污染。

### 运行后过拟合反思

- 判断：否。
- 原因：本阶段没有选择收益更好的参数，也没有根据结果调信号；反而增加了失败中断保护，防止静态股票池假装历史成分。

### 运行后继续价值反思

- 判断：是，但下一步要先解决 Tushare IP 数量限制。
- 原因：小样本 smoke 已证明接入逻辑可行；全量运行被外部账号/IP限制挡住，解除后应继续跑历史成分全量面板。
...
- 不接入第78。
- 不进入第78 A/B/C。
- 待 Tushare IP 限制解除后，优先运行全量历史成分面板构建。

### 后续规划和TODO

- 用户侧处理 Tushare IP 数量限制：
  - 在 Tushare 账户后台查看 token/IP 相关状态。
  - 如无法自助清理，联系 Tushare 管理员/客服处理“IP数量超限，最大数量为10个”。
  - 建议使用固定网络出口运行全量数据构建。
- 安全建议：
  - 由于 token 已在聊天里明文出现，后续建议在 Tushare `接口TOKEN` 页面刷新 token，并只通过本机环境变量提供。
- 解除限制后执行：
  - `UNIVERSE_SOURCE=tushare_csi1000`
  - `START_DATE=20250101`
  - `END_DATE=20260417`
...
## 2026-04-27 16:54 - 第204阶段：股票震荡长历史扩展第一段2018-2020

### 本次版本变更

- 当前模式：`day`
- 是否重要突破版本：否。不是交易突破版本；是股票震荡长历史研究的数据工程关键分段。
- 本阶段为股票震荡独立研究线，与期货第78趋势策略、期货震荡策略隔离。
- 第78影响：无。未修改第78正式趋势策略、配置、回测入口或18品种趋势池。
- 本阶段未跑策略回测：
  - 只构建2018-2020历史成分股票研究面板。
  - 复跑数据审计和固定信号层归因。
  - 不写交易规则、不调阈值、不做资金曲线、不接入第78、不进入第78 A/B/C。
- 运行前过拟合反思：
  - 判断：否。
  - 原因：扩长历史是在增加检验难度，目标是降低一年多短样本带来的误判，不涉及收益参数优化。
- 运行前继续价值反思：
  - 判断：是。
  - 原因：2025-2026样本太短，只有扩到多年份后，股票横截面震荡路线才可能被严肃判断。

### 代码改动

- 新增股票震荡长历史 Tushare 构建器：
  - `examples/alpha_research/build_stock_range_reversion_tushare_daily_panel.py`
- 修改股票数据审计脚本：
  - `examples/alpha_research/analyze_stock_range_reversion_data_audit.py`
- 修改股票信号归因脚本：
  - `examples/alpha_research/analyze_stock_range_reversion_signal_attribution.py`
- 修改旧 Baostock 构建器：
  - `examples/alpha_research/build_stock_range_reversion_research_panel.py`
- 关键补强：
  - 新构建器按交易日拉取 Tushare `daily` 和 `adj_factor`，按交易日落 parquet 缓存，支持中断后续跑。
  - 历史成分仍使用 Tushare `index_weight`，并沿用“只用当日之前最近一次成分快照”的无未来函数映射。
  - 默认 `FETCH_STK_LIMIT=0`，缺失涨跌停价按板块制度和ST状态重建，降低 `stk_limit` 对 IP 频控的压力。
  - ST 暂用 Tushare `namechange` 最佳努力重建，2018-2020只识别到`24`行，明显偏低，后续必须补更可靠 ST 来源。
  - 修正 Tushare 面板 `vt_symbol` 生成逻辑，统一从六位 `symbol` 生成；已重写2018-2020面板，异常 `vt_symbol` 从`1,204,110`行降为`0`。
  - 审计/归因报告动态展示样本年限，避免沿用短样本文案。
  - Baostock旧构建器保留单股票缓存，但禁用多进程下载分支；多进程 smoke 出现登录态不稳定，不适合作为默认能力。
...
- 删除交易参数：无
- 新增数据构建参数：
  - `TUSHARE_DAILY_SLEEP_SECONDS`
  - `TUSHARE_DAILY_RETRIES`
  - `TUSHARE_DAILY_RETRY_SLEEP`
  - `DAILY_CACHE_REFRESH`
  - `COMPONENT_CACHE_REFRESH`
  - `BASIC_CACHE_REFRESH`
  - `FETCH_STK_LIMIT`
  - `BAR_CACHE_REFRESH`
- 本次数据构建参数：
...
- 固定信号归因：
  - 5日`score_oversold_ret_20`：top-bottom平均超额收益`0.5823%`，Rank IC`0.0603`，t值`7.26`，正向日占比`62.22%`。
  ...
  - 10日`score_oversold_ret_20`：top-bottom平均超额收益`1.0325%`，Rank IC`0.0682`，t值`9.30`，正向日占比`64.81%`。
  - 10日`score_below_ma20`：top-bottom平均超额收益`0.8744%`，Rank IC`0.0580`，t值`7.94`。
  - 10日`score_oversold_ret_10`：top-bottom平均超额收益`0.7462%`，Rank IC`0.0492`，t值`7.26`。
...
- 年度拆分：
  ...
  - 结论：2018/2019强，2020明显变弱但未明显反向；不能宣称已穿越周期。

### 输出文件

- 历史成分研究面板：
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2018_2020/stock_range_reversion_research_panel.parquet`
- 基准：
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2018_2020/stock_range_reversion_benchmark.parquet`
- 历史成分：
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2018_2020/stock_range_reversion_components.parquet`
- 构建报告：
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2018_2020/stock_range_reversion_research_manifest.md`
- 数据审计：
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2018_2020/stock_range_reversion_data_audit_v1_report.md`
- 信号归因：
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2018_2020/stock_range_reversion_signal_attribution_v1_report.md`
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2018_2020/stock_range_reversion_signal_attribution_v1_summary.csv`
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2018_2020/stock_range_reversion_signal_attribution_v1_yearly_top_bottom.csv`
...
- 判断：否。
- 原因：本阶段扩展历史、固定信号复验、没有扫阈值或选择交易参数；年度拆分暴露2020弱化，反而降低了盲目乐观。
...
- 不接入第78。
- 不进入第78 A/B/C。
- 长历史扩展采用 Tushare 按交易日缓存方案，不再用 Baostock 串行按股票硬跑全历史。

### 后续规划和TODO

- 第一优先级：
  - 继续扩展 `2021-2026` 分段，并与 `2018-2020` 合并做年度/市场状态归因。
  - 补更可靠的 ST/退市/新股状态来源，避免交易约束低估。
- 第二优先级：
  - 增加行业、市值、成交额分层，判断超跌反弹是否只是风格或流动性暴露。
  - 抽查除权除息附近前复权收益，确认没有复权跳变污染。
- 暂不做：
  - 不写资金曲线。
  - 不调阈值。
  - 不把`score_oversold_ret_20`直接变成交易策略。

## 2026-04-27 18:57 - 第205阶段：股票震荡2021年度分段面板与固定信号归因

### 本次版本变更

- 当前模式：`day`
- 是否重要突破版本：否。不是交易突破版本；是股票震荡长历史年度扩展与反证检验。
- 本阶段为股票震荡独立研究线，与期货第78趋势策略、期货震荡策略隔离。
- 第78影响：无。未修改第78正式趋势策略、配置、回测入口或18品种趋势池。
- 本阶段未跑策略回测：
  - 只构建2021年度历史成分股票研究面板。
  ...
  - 不写交易规则、不调阈值、不做资金曲线、不接入第78、不进入第78 A/B/C。
- 执行过程：
  - 先尝试直接跑`2021-2026`长分段，但 Tushare `IP数量超限`过于密集。
  - 判断一口气跑完整段不够稳，改为年度分段，先完整跑通`2021`。

### 代码改动

- 修改股票震荡长历史 Tushare 构建器：
  - `examples/alpha_research/build_stock_range_reversion_tushare_daily_panel.py`
- 关键补强：
  - 新增 `TUSHARE_CALL_SLEEP_SECONDS`，在每次成功调用 Tushare 后主动休眠，减少靠失败重试推进的情况。
  - 继续保留按交易日 parquet 缓存，支持年度分段和中断续跑。
...
- 固定信号归因：
  - 5日`score_oversold_ret_5`：top-bottom平均超额收益`0.1841%`，Rank IC`0.0186`，t值`1.34`，正向日占比`49.54%`。
  ...
  - 10日`score_oversold_ret_10`：top-bottom平均超额收益`0.2716%`，Rank IC`0.0300`，t值`1.07`，正向日占比`50.23%`。
...
- 市场状态分层：
  ...
  - 2021年5日`score_oversold_ret_10`在`market_up_20d`下 top-bottom 为`-0.2530%`，正向日占比`42.76%`。
  - 结论：2021超跌反弹更像下跌/压力状态中的短修复，不是全年无状态稳定 alpha。
- 2018-2021年度对比：
  ...
  - 结论：2021没有恢复到2018/2019强度，只比2020略好；不能直接交易化。

### 输出文件

- 2021研究面板：
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2021/stock_range_reversion_research_panel.parquet`
- 基准：
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2021/stock_range_reversion_benchmark.parquet`
- 历史成分：
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2021/stock_range_reversion_components.parquet`
- 构建报告：
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2021/stock_range_reversion_research_manifest.md`
- 数据审计：
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2021/stock_range_reversion_data_audit_v1_report.md`
- 信号归因：
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2021/stock_range_reversion_signal_attribution_v1_report.md`
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2021/stock_range_reversion_signal_attribution_v1_summary.csv`
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2021/stock_range_reversion_signal_attribution_v1_2018_2021_year_compare.csv`
...
- 判断：否。
- 原因：年度分段是外推复验，不改变信号公式、不扫阈值、不选择交易参数。

### 运行前继续价值反思

- 判断：是。
- 原因：2020已经弱化，必须看2021是否恢复、延续或反转，才能判断股票横截面震荡是否具备穿越周期潜力。

### 运行后过拟合反思

- 判断：否。
- 原因：本阶段使用固定信号，2021弱化是反证而不是拟合收益；市场状态分层只是归因观察，不作为交易过滤器落地。

### 运行后继续价值反思

- 判断：是，但方向要收敛。
- 原因：2021全年弱，但下跌状态中的超跌修复仍明显；下一步应继续补2022并做市场状态/风格分层，而不是直接做全市场无状态长侧策略。
...
- 不接入第78。
- 不进入第78 A/B/C。
- 后续扩展改为年度分段，不再尝试一口气跑完整`2021-2026`。

### 后续规划和TODO

- 第一优先级：
  - 继续跑`2022`年度分段，验证弱化是否延续到熊市/下跌年。
  - 把`market_down_20d`/`market_up_20d`从观察扩展为更细的状态归因，不直接当交易过滤器。
- 第二优先级：
  - 补行业、市值、成交额分层，判断2021弱化是否来自风格或行业集中。
  - 检查`600069`、`600175`、`600677`全期无有效日线的原因。
- 暂不做：
  - 不写资金曲线。
  - 不调阈值。
  - 不把2021市场状态分层直接变成策略规则。

## 2026-04-27 20:58 - 第206阶段：股票震荡2022年度分段面板与固定信号归因

### 本次版本变更

- 当前模式：`day`
- 是否重要突破版本：否。不是交易突破版本；是股票震荡长历史年度扩展与压力年份复验。
- 本阶段为股票震荡独立研究线，与期货第78趋势策略、期货震荡策略隔离。
- 第78影响：无。未修改第78正式趋势策略、配置、回测入口或18品种趋势池。
- 本阶段未跑策略回测：
  - 只构建2022年度历史成分股票研究面板。
...
- 新增代码改动：无。
- 沿用第205阶段新增的 `TUSHARE_CALL_SLEEP_SECONDS` 和按交易日 parquet 缓存机制。
...
- 固定信号归因：
  - 1日`score_down_volume_pressure`：top-bottom平均超额收益`0.1779%`，Rank IC`0.0266`，t值`3.15`，正向日占比`60.18%`。
  - 5日`score_oversold_ret_20`：top-bottom平均超额收益`0.4225%`，Rank IC`0.0430`，t值`2.87`，正向日占比`55.56%`。
  - 5日`score_below_ma20`：top-bottom平均超额收益`0.2815%`，Rank IC`0.0306`，t值`2.03`。
  - 10日`score_oversold_ret_20`：top-bottom平均超额收益`0.7611%`，Rank IC`0.0491`，t值`3.91`，正向日占比`55.92%`。
...
- 2022研究面板：
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2022/stock_range_reversion_research_panel.parquet`
- 基准：
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2022/stock_range_reversion_benchmark.parquet`
- 历史成分：
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2022/stock_range_reversion_components.parquet`
- 构建报告：
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2022/stock_range_reversion_research_manifest.md`
- 数据审计：
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2022/stock_range_reversion_data_audit_v1_report.md`
- 信号归因：
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2022/stock_range_reversion_signal_attribution_v1_report.md`
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2022/stock_range_reversion_signal_attribution_v1_summary.csv`
  - `examples/alpha_research/native_results/stock_range_reversion_cache_tushare_daily_2022/stock_range_reversion_signal_attribution_v1_2018_2022_year_compare.csv`
...
- 判断：否。
- 原因：补`2022`是向外扩样本、找反证，不改信号或阈值，不做交易参数优化。

### 运行前继续价值反思

- 判断：是。
- 原因：2022是压力年份，正好验证2021观察到的“下跌状态中短修复”是否成立。
...
- 第一优先级：
  - 继续跑`2023`年度分段，验证反弹/结构市中20日超跌修复是否延续。
  - 整合2018-2023年度对比，明确哪些年份强、哪些年份钝化。
- 第二优先级：
  - 补行业、市值、成交额分层，判断2022的20日超跌修复是否集中在小盘或特定行业。
  - 单独研究`score_down_volume_pressure`的1日反转，暂不进入交易化。
- 暂不做：
  - 不写资金曲线。
  - 不调阈值。
  - 不把2022强信号直接变成策略规则。
