...
PREFIX: str = "stock_range_reversion_liquid_q3_industry_resid_core_30w_replay_v1"
...
def read_selected() -> pl.DataFrame:
    ...
    ).filter(pl.col("model") == TARGET_MODEL)
...
def build_shaped_selected(
    ...
    scenario = f"{TARGET_MODEL}_h{horizon}_top{top_k}_gross{int(basket_gross_weight * 100)}_ind{max_per_industry}"
    work = (
        selected.with_columns(
            pl.col("model_score").rank("ordinal", descending=True).over(["datetime", "industry"]).alias("_industry_rank")
        )
        .filter(pl.col("_industry_rank") <= max_per_industry)
        .with_columns(pl.col("model_score").rank("ordinal", descending=True).over("datetime").alias("_daily_shape_rank"))
...
def write_report(summary: pl.DataFrame, quality: pl.DataFrame, yearly: pl.DataFrame, paths: dict[str, Path]) -> Path:
    ...
    lines = [
        "# 股票震荡liquid_q3 industry_resid_core 30万整手复放 v1",
        "",
        f"- 记录时间：{datetime.now().strftime('%Y-%m-%d %H:%M CST')}",
        "- 当前模式：day。",
        "- 当前研究线：股票震荡独立策略研究，不接入第78。",
        "- 本阶段性质：对第307阶段通过反证审计的`industry_resid_core`做30万真实整手复放。",
    ...
    lines.extend(
        [
            ...
            "## 场景汇总",
            "",
            markdown_table(summary.select([col for col in display_cols if col in summary.columns]), [col for col in display_cols if col in summary.columns], max_rows=120),
            "",
            "## 回撤20%以内候选",
            "",
            ...
            "## 年度拆分Top候选",
            "",
            markdown_table(yearly.filter(pl.col("scenario").is_in(top_yearly_scenarios)).sort(["scenario", "year"]), yearly.columns, max_rows=160),
            "",
            "## 质量检查",
            "",
            markdown_table(quality, quality.columns, max_rows=220),
            "",
            "## 结论",
            "",
            "- 本阶段用于判断信号层通过审计后，能否通过30万真实整手约束。",
            "- 若回撤/收益仍不达标，说明优势仍不足以覆盖账户颗粒度和持仓路径；后续不应继续扫局部形状。",
        ]
    )
