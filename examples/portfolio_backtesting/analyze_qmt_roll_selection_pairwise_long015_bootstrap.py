...
BASELINE_DAILY_PATH: Path = OUTPUT_DIR / "qmt_roll_selection_pairwise_v2_long015_compare_daily.csv"
LONG015_DAILY_PATH: Path = OUTPUT_DIR / "qmt_roll_selection_pairwise_v2_volume_tilt_long015_sweep_fix1_daily.csv"

ROLLING_SUMMARY_CSV_PATH: Path = OUTPUT_DIR / "qmt_roll_selection_pairwise_v2_volume_tilt_long015_bootstrap_rolling_summary.csv"
BOOTSTRAP_JSON_PATH: Path = OUTPUT_DIR / "qmt_roll_selection_pairwise_v2_volume_tilt_long015_bootstrap_summary.json"

BLOCK_LENGTHS: tuple[int, ...] = (5, 20, 60)
BOOTSTRAP_ITERATIONS: int = 5000
RANDOM_SEED: int = 20260424
ROLLING_WINDOWS: tuple[int, ...] = (126, 252, 504)
CAPITAL_BASE: float = 200_000.0
...
def build_delta_frame() -> pd.DataFrame:
    baseline = load_daily_curve(BASELINE_DAILY_PATH)
    ...
    merged["delta_trade_count"] = merged["trade_count_long015"] - merged["trade_count_base"]
    return merged
...
def compute_window_metrics(window_df: pd.DataFrame) -> dict[str, float]:
    ...
    return {
        "delta_end_balance": long_end - base_end,
        "delta_total_return_pct": long_total_return - base_total_return,
        "delta_max_dd_pct": long_dd - base_dd,
