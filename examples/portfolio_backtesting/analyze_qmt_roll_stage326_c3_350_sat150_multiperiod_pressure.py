...
MODEL_TAG = "stage326_c3_350_sat150_multiperiod_pressure_v1"
OUTPUT_PREFIX = "qmt_roll_stage326_c3_350_sat150_multiperiod_pressure"
...
def _run_c3(window: Window, capital: float, label: str) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    print(f"[stage326] run C3 {label} {window.name} capital={capital:.0f}", flush=True)
    engine, analysis_df, statistics = run_roll_backtest(
        ...
        chart_title=f"Stage326 {label} {window.label}",
    )
    return _daily_from_analysis_with_slippage(analysis_df, capital, label), build_positions_df(engine), statistics


def _run_satellite(window: Window) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    preload_start = max(PRELOAD_START_DT, window.start - timedelta(days=365))
    print(f"[stage326] run satellite {window.name} capital={SATELLITE_CAPITAL:.0f}", flush=True)
    engine, analysis_df, statistics = run_range_backtest(
        ...
        save_artifacts=False,
        file_prefix=f"{OUTPUT_PREFIX}_satellite_{window.name}",
        chart_title=f"Stage326 Satellite {window.label}",
        strategy_tag=f"range_reversion_v8_two_stage_stop_stage326_{window.name}",
        setting_overrides=SATELLITE_OVERRIDES,
    )
    return _daily_from_analysis_with_slippage(analysis_df, SATELLITE_CAPITAL, "satellite"), build_positions_df(engine), statistics


def _combine_candidate_daily(c3_daily: pd.DataFrame, satellite_daily: pd.DataFrame) -> pd.DataFrame:
    dates = sorted(set(c3_daily["date"]).union(set(satellite_daily["date"])))
    if not dates:
        return pd.DataFrame()
    merged = pd.DataFrame({"date": pd.to_datetime(dates)})
    merged = merged.merge(c3_daily, on="date", how="left").merge(satellite_daily, on="date", how="left")
    for column in [
    ...
    merged["combo_net_pnl"] = merged["c3_candidate_net_pnl"] + merged["satellite_net_pnl"]
    merged["combo_slippage"] = merged["c3_candidate_slippage"] + merged["satellite_slippage"]
    ...
    merged["trade_count"] = merged["c3_candidate_trade_count"] + merged["satellite_trade_count"]
    return merged
...
def _window_gate(c3_return: float, combo_return: float, combo_dd: float) -> tuple[int, float]:
    if c3_return > 0:
        ...
        ok = combo_dd >= -30.0 and retention >= 80.0
        return int(ok), retention
    ok = combo_dd >= -30.0 and combo_return >= c3_return
    return int(ok), math.nan


def _run_window(window: Window, metadata: dict[str, Any]) -> tuple[dict[str, Any], pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    satellite_daily, satellite_positions, satellite_stats = _run_satellite(window)
    combo_daily = _combine_candidate_daily(c3_candidate_daily, satellite_daily)
    baseline_daily = _baseline_daily_from_c3(c3_base_daily)
    margin = _combine_margin(combo_daily, c3_candidate_positions, satellite_positions, metadata)
    ...
    row = {
        ...
        "satellite_150_return_pct": _safe_float(satellite_stats.get("total_return")),
        "satellite_150_max_dd_pct": _safe_float(satellite_stats.get("max_ddpercent")),
    ...
    return row, combo_daily, baseline_daily, margin
...
def _decision(summary: pd.DataFrame, slippage: pd.DataFrame) -> str:
    ...
    if pass_windows == strict_window_count and stress_pass_3x:
        return "promote_to_deeper_quarterly_validation"
    if pass_windows >= max(1, strict_window_count - 1) and positive_retention_min >= 75.0:
        return "research_candidate_watch_slippage_or_window_gap"
    return "fail_multiperiod_or_stress"


def _build_report(summary: pd.DataFrame, slippage: pd.DataFrame, decision: str) -> str:
    lines = [
        "# Stage326 C3 35万 + 卫星15万多周期压力复验",
        "",
        "## 目标",
        "",
        "- 固定 Stage025 研究候选 `c3_350_sat_150`，不再搜索资金权重。",
        "- 与 `50万C3` 在相同起点下比较收益保留和最大回撤。",
        "- 同时做全样本滑点倍数压力，检查候选是否只是成本口径下的偶然通过。",
        "",
        "## 多周期结果",
        "",
    ]
    ...
    if decision == "promote_to_deeper_quarterly_validation":
        lines.append("- 多周期和3倍滑点压力均通过，下一步进入季度冷启动和更严保证金压力。")
    elif decision == "research_candidate_watch_slippage_or_window_gap":
    ...
    lines.extend(
        [
            ...
            "## 过拟合反思",
            "",
            "- 本阶段固定 Stage025 候选，只做反证；如果失败，不继续围绕 `35/15` 调小数权重。",
            "",
            "## 继续价值反思",
            "",
            "- 若通过，组合层值得继续做季度冷启动；若失败，回到寻找更稳的低相关卫星或C3状态识别。",
        ]
    )
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    for window in WINDOWS:
        row, combo_daily, baseline_daily, margin = _run_window(window, metadata)
        rows.append(row)
    ...
    print(f"[stage326] summary={summary_path}")
    print(f"[stage326] combo_daily={combo_daily_path}")
    print(f"[stage326] baseline_daily={baseline_daily_path}")
    print(f"[stage326] margin={margin_path}")
    print(f"[stage326] slippage_stress={slippage_path}")
    print(f"[stage326] report={report_path}")
    print(f"[stage326] decision={decision_path}")
