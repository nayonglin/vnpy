...
MODEL_TAG = "stage415_stage103_cffex_index_true_overlay_v2"
OUTPUT_PREFIX = "qmt_roll_stage415_stage103_cffex_index_true_overlay"
...
INDEX_PRODUCT_SPECS = {
    "IF": {"contract_multiplier": 300.0, "margin_ratio": 0.15, "tick_size": 0.2},
    "IH": {"contract_multiplier": 300.0, "margin_ratio": 0.15, "tick_size": 0.2},
    "IC": {"contract_multiplier": 200.0, "margin_ratio": 0.15, "tick_size": 0.2},
    "IM": {"contract_multiplier": 200.0, "margin_ratio": 0.15, "tick_size": 0.2},
}
...
INDEX_PANEL_PATH = OUTPUT_DIR / f"{OUTPUT_PREFIX}_index_panel_{MODEL_TAG}.csv"
...
VARIANTS: tuple[s405.VariantSpec, ...] = (
    ...
    s405.VariantSpec(
        "stage103_plus_cffex_index_tsmom60_min1_guard",
        "C1 Stage103+中金所股指60日动量真实一手",
        "index_true_overlay",
        "index_tsmom",
        60,
        0,
        0,
        "IF/IH/IC/IM 四个股指期货主力合约按60日时间序列动量，各品种最多1手，并受1.10倍保证金闸门约束。",
    ),
    s405.VariantSpec(
        "stage103_plus_cffex_index_tsmom120_min1_guard",
        "C2 Stage103+中金所股指120日动量真实一手",
        "index_true_overlay",
        "index_tsmom",
        120,
        0,
        0,
        "IF/IH/IC/IM 四个股指期货主力合约按120日时间序列动量，各品种最多1手，并受1.10倍保证金闸门约束。",
    ),
    s405.VariantSpec(
        "stage103_plus_cffex_index_best1_tsmom60_guard",
        "C3 Stage103+中金所股指60日动量最强1手",
        "index_true_overlay",
        "index_tsmom_best1",
        60,
        1,
        0,
        "IF/IH/IC/IM 四个股指期货主力合约按60日时间序列动量，全篮子每天只取绝对动量最强的1个方向，并受1.10倍保证金闸门约束。",
    ),
    s405.VariantSpec(
        "stage103_plus_cffex_index_best1_tsmom120_guard",
        "C4 Stage103+中金所股指120日动量最强1手",
        "index_true_overlay",
        "index_tsmom_best1",
        120,
        1,
        0,
        "IF/IH/IC/IM 四个股指期货主力合约按120日时间序列动量，全篮子每天只取绝对动量最强的1个方向，并受1.10倍保证金闸门约束。",
    ),
    s405.VariantSpec(
        "stage103_plus_cffex_index_short1_tsmom60_guard",
        "C5 Stage103+中金所股指60日危机对冲1手",
        "index_true_overlay",
        "index_tsmom_short1",
        60,
        1,
        0,
        "IF/IH/IC/IM 四个股指期货主力合约按60日时间序列动量，只保留下跌趋势做空信号且每天最多1手，并受1.10倍保证金闸门约束。",
    ),
    s405.VariantSpec(
        "stage103_plus_cffex_index_short1_tsmom120_guard",
        "C6 Stage103+中金所股指120日危机对冲1手",
        "index_true_overlay",
        "index_tsmom_short1",
        120,
        1,
        0,
        "IF/IH/IC/IM 四个股指期货主力合约按120日时间序列动量，只保留下跌趋势做空信号且每天最多1手，并受1.10倍保证金闸门约束。",
    ),
)
...
def _build_index_panel() -> pd.DataFrame:
    frames: list[pd.DataFrame] = []
    for product in INDEX_PRODUCTS:
        series = s381._build_main_product_series(product)
        if series.empty:
            continue
        spec = INDEX_PRODUCT_SPECS[product]
        series = series.copy()
        series["contract_multiplier"] = spec["contract_multiplier"]
        series["margin_ratio"] = spec["margin_ratio"]
        series["tick_size"] = spec["tick_size"]
        series["tick_value"] = series["tick_size"] * series["contract_multiplier"]
        series["contract_value"] = pd.to_numeric(series["close"], errors="coerce") * series["contract_multiplier"]
        ...
        for horizon in (60, 120):
            ...
            part["momentum"] = momentum.replace([np.inf, -np.inf], np.nan).fillna(0.0)
...
def _simulate_index_overlay(
    ...
    if spec.direction not in {"index_tsmom", "index_tsmom_best1", "index_tsmom_short1"}:
    ...
    for date in window_frame["date"].sort_values():
        ...
        proposed_margin = 0.0
        pnl = 0.0
        raw_rows = list(by_date.get(date, []))
        if spec.direction == "index_tsmom_best1":
            raw_rows = [
                max(raw_rows, key=lambda row: abs(float(getattr(row, "momentum", 0.0))))
            ] if raw_rows else []
        elif spec.direction == "index_tsmom_short1":
            short_rows = [row for row in raw_rows if float(getattr(row, "position", 0.0)) < 0.0]
            raw_rows = [
                max(short_rows, key=lambda row: abs(float(getattr(row, "momentum", 0.0))))
            ] if short_rows else []
...
def _plot(daily: pd.DataFrame, horizon: pd.DataFrame, cost: pd.DataFrame) -> None:
    try:
        import matplotlib
        ...
        print(f"[stage415] skip chart: {exc}", flush=True)
        return
    ...
    fig.suptitle("Stage115 CFFEX index true overlay", fontsize=14)
    fig.tight_layout()
...
def _write_report(
    ...
    report = [
        ...
        "- 阶段性质：不同资产风险源真实整数手审计；不改 C3、Stage079、Stage103 规则，不增加账户资金。",
        "- A/B/C：A=Stage079；C0=Stage103；C1/C2=四股指各最多1手；C3/C4=全股指篮子最多1手；C5/C6=只做空危机对冲最多1手。",
        f"- 图表：`{CHART_PATH}`",
        "",
        "## 决策",
        "",
        ...
        "## 合约与反过拟合说明",
        "",
        "- 股指期货合约乘数按公开合约规格：IF/IH为每点300元，IC/IM为每点200元；最小变动价位按0.2点。",
        "- 本阶段用 `15%` 保证金率估算单手占用，并在组合执行闸门中再乘 `1.10`，比当前多数交易所最低/期货公司常见口径更保守。",
        "- 只测试 Stage381 已有净值层线索中的60日/120日股指TSMOM，不按结果挑单个指数、调窗口、调保证金比例或调信号阈值。",
        "- `best1` 与 `short1` 是资金粒度约束下的结构形态：股指单合约名义本金过大，61.5万账户不应同时持有四个股指合约；它们不是相邻小数扫描。",
    ]
...
def main() -> None:
    ...
    try:
        ...
        index_panel = _build_index_panel()
    ...
    decision = {
        ...
        "judgement": "股指期货TSMOM若不能同时通过Stage079硬闸门、Stage103增量不劣化、坏窗口和成本/保证金约束，则不晋级。",
    }
    ...
    index_panel.to_csv(INDEX_PANEL_PATH, index=False, encoding="utf-8-sig")
    DECISION_PATH.write_text(json.dumps(_json_safe(decision), ensure_ascii=False, indent=2), encoding="utf-8")
    _plot(full_daily, horizon, cost)
