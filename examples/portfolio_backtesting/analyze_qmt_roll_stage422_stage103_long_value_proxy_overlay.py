...
MODEL_TAG = "stage422_stage103_long_value_proxy_overlay_v1"
OUTPUT_PREFIX = "qmt_roll_stage422_stage103_long_value_proxy_overlay"
...
FEATURE_PATH = OUTPUT_DIR / f"{OUTPUT_PREFIX}_value_scores_{MODEL_TAG}.csv"
...
VARIANTS: tuple[s405.VariantSpec, ...] = (
    ...
    s405.VariantSpec(
        VALUE504_VARIANT,
        "C1 Stage103+504日长期value proxy",
        "long_value_proxy_overlay",
        "reversal",
        504,
        3,
        20,
        "过去504个交易日长期相对跌幅作为便宜代理，月频买长期弱者、卖长期强者，各3个品种。",
    ),
    s405.VariantSpec(
        VALUE756_VARIANT,
        "C2 Stage103+756日长期value proxy",
        "long_value_proxy_overlay",
        "reversal",
        756,
        3,
        20,
        "过去756个交易日长期相对跌幅作为便宜代理，月频买长期弱者、卖长期强者，各3个品种。",
    ),
)
...
def _build_rank_tables(price_frame: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    result: dict[str, pd.DataFrame] = {BASELINE_VARIANT: pd.DataFrame(), STAGE103_VARIANT: pd.DataFrame()}
    feature_parts = []
    for variant, lookback in ((VALUE504_VARIANT, 504), (VALUE756_VARIANT, 756)):
        trailing_return = one_plus.rolling(lookback, min_periods=lookback).apply(np.prod, raw=True).shift(1) - 1.0
        value_score = -trailing_return
        result[variant] = value_score
        long = value_score.reset_index().melt(id_vars="date", var_name="product_vt_symbol", value_name="value_score")
        long["variant"] = variant
        long["lookback_days"] = lookback
...
def _plot(full_daily: pd.DataFrame, score: pd.DataFrame, pairwise: pd.DataFrame) -> None:
    ...
    labels = ["Stage079", "Stage103", "Value504", "Value756"]
    ...
    fig.suptitle("Stage122 long value proxy overlay", fontsize=14)
    fig.tight_layout()
...
def _write_report(
    ...
    report = [
        "# Stage122 Stage103 Long Value Proxy Overlay审计",
        "",
        f"- 生成时间：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        "- 阶段性质：文献驱动固定结构；不改 Stage079/Stage103/C3 规则，不增加账户资金，不扫品种、阈值或坏窗口。",
        "- A/B/C：A=Stage079；C0=Stage103；C1=Stage103+504日value proxy；C2=Stage103+756日value proxy。",
        "- 候选假设：长期value/contrarian 与趋势不同源，理论上可能覆盖趋势暴涨后反转和长水下路径。",
        "- 关键限制：外部资料通常要求value使用5年以上时间尺度，本地商品连续数据自2020起，756日也只是较短代理，不能因局部有效直接晋级。",
        f"- 图表：`{CHART_PATH}`",
        "",
        "## 决策",
        "",
        ...
        "- 504/756日长期value proxy、月频、top/bottom各3均为预声明粗结构。",
        "- 若候选只靠短样本后半段有效、冷启动失败或相对Stage103收益不足，则不晋级。",
    ]
...
def main() -> None:
    ...
    decision = {
        ...
        "judgement": "长期value proxy若不能同时通过Stage079硬闸门、3/6个月体验、冷启动和相对Stage103鲁棒性，则只保留为研究经验。",
    }
