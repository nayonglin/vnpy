...
import analyze_qmt_roll_stage433_xsmom_trend_quality_filter as s433  # noqa: E402


MODEL_TAG = "stage434_xsmom_jump_concentration_filter_v1"
OUTPUT_PREFIX = "qmt_roll_stage434_xsmom_jump_concentration_filter"
...
VARIANTS: tuple[s403.AuditVariant, ...] = (
    ...
    s403.AuditVariant(
        "xsmom_jump63_drop_worst1_broker10_guard",
        "C1 xsmom 去掉跳跃集中度最高1个信号",
        "jump63_drop_worst1",
        "每天若有2个以上xsmom信号，则去掉过去63日最大单日绝对波动占比最高的1个信号，沿用Stage103保证金闸门。",
    ),
    s403.AuditVariant(
        "xsmom_jump63_lowhalf_broker10_guard",
        "C2 xsmom 跳跃集中度最低半数",
        "jump63_lowhalf",
        "每天只执行过去63日最大单日绝对波动占比最低的半数xsmom信号，沿用Stage103保证金闸门。",
    ),
)
...
def _candidate(spec: s403.AuditVariant, equity: pd.Series) -> Any:
    return s402.s087.Candidate(
        ...
        candidate_class="xsmom_jump_concentration_filter" if spec.variant != BASELINE_VARIANT else "baseline",
        eligible_for_promotion=True,
        note=spec.note,
    )


def _filter_desired_by_jump(
    date: pd.Timestamp,
    ...
    for item in desired:
        _contract, product, _direction, _margin = item
        _long_score, _short_score, jump_share = quality.get((date, product), (float("nan"), float("nan"), float("nan")))
        if math.isnan(jump_share):
            continue
        scored.append((jump_share, item))
    if not scored:
        return [], float("nan")
    ranked = sorted(scored, key=lambda pair: (pair[0], pair[1][3]))
    if mode == "jump63_drop_worst1":
        keep = max(len(ranked) - 1, 1)
        selected = ranked[:keep]
    elif mode == "jump63_lowhalf":
        keep = int(math.ceil(len(ranked) / 2.0))
        selected = ranked[:keep]
    else:
        selected = ranked
    return [item for _score, item in selected], float(np.mean([score for score, _item in selected]))


def _simulate_jump_satellite(
    ...
    for signal_row in window_signals.sort_values("date").itertuples(index=False):
        ...
        desired, avg_jump_share63 = _filter_desired_by_jump(date, desired_raw, spec.mode, quality)
        ...
        rows.append(
            {
                ...
                "avg_selected_jump_share63": avg_jump_share63,
...
def _returns_by_variant(full_daily: pd.DataFrame) -> pd.DataFrame:
    return s433._returns_by_variant(full_daily)


def _metrics_from_returns(returns: np.ndarray) -> dict[str, float]:
    return s433._metrics_from_returns(returns)
...
def _plot(summary: pd.DataFrame, score: pd.DataFrame, margin_audit: pd.DataFrame, topday: pd.DataFrame) -> None:
    try:
        ...
        print(f"[stage434] skip chart: {exc}", flush=True)
        return

    variants = [spec.variant for spec in VARIANTS]
    labels = ["Stage079", "Stage103", "Drop worst1", "Low half"]
    ...
    fig.suptitle("Stage134 xsmom jump-concentration filter audit", fontsize=14)
    fig.tight_layout()
...
def _write_report(
    ...
    lines = [
        ...
        "- A/B/C：A=Stage079；C0=Stage103；C1=去掉跳跃集中度最高1个信号；C2=跳跃集中度最低半数。",
        f"- 图表：`{CHART_PATH}`",
        "",
        "## 决策",
        "",
        ...
        "- `drop_worst1` 和 `lowhalf` 是离散结构，不设置跳跃占比阈值。",
        "- 若反跳跃集中度过滤没有改善短持有体验或损害长期收益，不继续调 `25%/30%`、`20/126日` 或Top比例。",
    ]
...
def main() -> None:
    ...
    try:
        ...
        quality_panel = s433._build_quality_panel(price_frame)
        quality = s433._quality_lookup(quality_panel)
    ...
    jump_variants = [VARIANTS[2].variant, VARIANTS[3].variant]
    jump_gate = gate[gate["variant"].isin(jump_variants)].copy()
    research_ready = jump_gate[jump_gate["research_promotion_pass"].eq(1)]
    execution_ready = jump_gate[jump_gate["execution_relative_pass"].eq(1)]
    abs_ready = jump_gate[jump_gate["deployment_absolute_margin_pass"].eq(1)]
    best = jump_gate.iloc[0] if not jump_gate.empty else None
    decision_key = "jump_quality_candidate_found" if len(execution_ready) else "no_jump_quality_promotion"
    decision = {
        "stage": "Stage134",
        "model_tag": MODEL_TAG,
        "line_id": LINE_ID,
        ...
        "best_jump_variant_by_gate_order": str(best["variant"]) if best is not None else "",
        "chart": str(CHART_PATH),
        "report": str(REPORT_PATH),
        "judgement": "反跳跃集中度过滤若不能在不损害Stage079硬约束的前提下改善短持有体验，则停止该方向，不继续调跳跃占比阈值或窗口。",
    }
