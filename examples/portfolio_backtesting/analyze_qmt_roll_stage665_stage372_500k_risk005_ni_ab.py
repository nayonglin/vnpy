...
MODEL_TAG = "stage665_stage372_500k_risk005_ni_ab_v1"
OUTPUT_PREFIX = "qmt_roll_stage665_stage372_500k_risk005_ni_ab"
...
VARIANT_NO_NI = "stage372_500k_risk005_no_ni"
VARIANT_PLUS_NI = "stage372_500k_risk005_plus_ni"

GENERATED_DIR = OUTPUT_DIR / "stage665_generated_inputs"
UNIVERSE_PLUS_NI_PATH = GENERATED_DIR / f"{OUTPUT_PREFIX}_plus_ni_universe_{MODEL_TAG}.csv"
HIST_ELIGIBILITY_PLUS_NI_PATH = GENERATED_DIR / f"{OUTPUT_PREFIX}_historical_plus_ni_eligibility_{MODEL_TAG}.csv"
LATEST_ELIGIBILITY_PLUS_NI_PATH = GENERATED_DIR / f"{OUTPUT_PREFIX}_latest_plus_ni_eligibility_{MODEL_TAG}.csv"
...
def _write_plus_ni_universe(base_symbols: list[str]) -> None:
    GENERATED_DIR.mkdir(parents=True, exist_ok=True)
    rows = []
    for symbol in sorted(set(base_symbols) | {NI_PRODUCT}):
        ...
        rows.append(
            {
                ...
                "exchange": exchange,
                "eligible": 1,
                "source": "stage665_500k_risk005_plus_ni_fixed_add_one",
            }
        )
    pd.DataFrame(rows).to_csv(UNIVERSE_PLUS_NI_PATH, index=False, encoding="utf-8-sig")
                        ...
                        [
                            {
                                ...
                                "score_type": "stage665_fixed_add_one_ni",
...
def _prepare_inputs() -> dict[str, Any]:
    ...
    plus_symbols = sorted(set(base_symbols) | {NI_PRODUCT})
    ...
    _write_plus_ni_eligibility(historical_source, HIST_ELIGIBILITY_PLUS_NI_PATH)
    _write_plus_ni_eligibility(latest_source, LATEST_ELIGIBILITY_PLUS_NI_PATH)
    return {
...
def _base_500k_spec(metadata: dict[str, Any]) -> s653.ForcedVariant:
    ...
    capital = replace(
        official.capital,
        variant=VARIANT_NO_NI,
        label="50w Stage372 recovery sleeve risk005 no-ni",
        account_capital=CAPITAL,
        c3_capital=CAPITAL,
        risk_multiplier=RISK_MULTIPLIER,
        note=(
            "Stage665 B: keep Stage372 official logic and product pool, set account/c3 capital to 500k "
            "and capital risk_multiplier to 0.05."
        ),
    )
    return replace(official, capital=capital, profile="stage372_500k_risk005_no_ni")


def _plus_ni_500k_spec(metadata: dict[str, Any]) -> s653.ForcedVariant:
    official = s660._official_spec(metadata)
    capital = replace(
        official.capital,
        variant=VARIANT_PLUS_NI,
        label="50w Stage372 recovery sleeve risk005 plus-ni",
        account_capital=CAPITAL,
        c3_capital=CAPITAL,
        risk_multiplier=RISK_MULTIPLIER,
        note=(
            "Stage665 C: 500k/risk005 plus fixed ni.SHFE in product universe and every AI eligibility snapshot."
        ),
    )
    overrides = {
        **official.overrides,
        "product_universe_csv_path": str(UNIVERSE_PLUS_NI_PATH),
        "enable_ai_product_pool_filter": True,
        "ai_product_pool_eligibility_path": str(HIST_ELIGIBILITY_PLUS_NI_PATH),
        "ai_product_pool_strategy": PLUS_NI_STRATEGY,
    }
    return replace(official, capital=capital, overrides=overrides, profile="stage372_500k_risk005_plus_ni")
...
def _comparison(summary: pd.DataFrame, cost: pd.DataFrame) -> pd.DataFrame:
    base = summary[summary["variant"].eq(VARIANT_NO_NI)].set_index("window_name")
    plus = summary[summary["variant"].eq(VARIANT_PLUS_NI)].set_index("window_name")
    rows: list[dict[str, Any]] = []
        ...
        rows.append(
            {
                ...
                "no_ni_return_pct": float(brow["rebased_total_return_pct"]),
                "plus_ni_return_pct": float(crow["rebased_total_return_pct"]),
                "delta_return_pct": float(crow["rebased_total_return_pct"] - brow["rebased_total_return_pct"]),
                "no_ni_max_dd_pct": float(brow["rebased_max_dd_pct"]),
                "plus_ni_max_dd_pct": float(crow["rebased_max_dd_pct"]),
                "delta_max_dd_pct": float(crow["rebased_max_dd_pct"] - brow["rebased_max_dd_pct"]),
                "no_ni_sharpe": float(brow["rebased_sharpe"]),
                "plus_ni_sharpe": float(crow["rebased_sharpe"]),
                "delta_sharpe": float(crow["rebased_sharpe"] - brow["rebased_sharpe"]),
                "no_ni_trades": float(brow["total_trade_count"]),
                "plus_ni_trades": float(crow["total_trade_count"]),
                "delta_trades": float(crow["total_trade_count"] - brow["total_trade_count"]),
                "no_ni_slippage": float(brow["total_slippage"]),
                "plus_ni_slippage": float(crow["total_slippage"]),
                "delta_slippage": float(crow["total_slippage"] - brow["total_slippage"]),
                "no_ni_margin_peak_pct": float(brow["max_broker10_margin_to_rebased_equity_pct"]),
                "plus_ni_margin_peak_pct": float(crow["max_broker10_margin_to_rebased_equity_pct"]),
    ...
    base_cost = cost[(cost["variant"].eq(VARIANT_NO_NI)) & (cost["cost_multiplier"].eq(2.0))].set_index("window_name")
    plus_cost = cost[(cost["variant"].eq(VARIANT_PLUS_NI)) & (cost["cost_multiplier"].eq(2.0))].set_index("window_name")
    for row in rows:
        name = str(row["window_name"])
        if name in base_cost.index and name in plus_cost.index:
            row["no_ni_2x_max_dd_pct"] = float(base_cost.loc[name, "max_dd_pct"])
            row["plus_ni_2x_max_dd_pct"] = float(plus_cost.loc[name, "max_dd_pct"])
        ...
        rows.append(
            {
                ...
                "ni_active_days": int(active["date"].nunique()),
                "ni_first_active_date": active["date"].min().date().isoformat() if not active.empty else "",
                "ni_last_active_date": active["date"].max().date().isoformat() if not active.empty else "",
            }
        )
    if not usage.empty:
...
def _checks(summary: pd.DataFrame, cost: pd.DataFrame, comparison: pd.DataFrame, rolling: pd.DataFrame) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
    for variant in (VARIANT_NO_NI, VARIANT_PLUS_NI):
...
def _plot(curves: pd.DataFrame, comparison: pd.DataFrame) -> None:
    ...
    ax_delta.set_title("Plus ni return delta")
    ...
    fig.suptitle("Stage665 500k risk005 no-ni vs plus-ni", fontsize=14)
    fig.tight_layout()
...
def _write_report(
    ...
    lines = [
        "# Stage665 50万 risk0.05 加/不加 ni 多周期审计",
        "",
        f"- 生成时间：`{datetime.now().strftime('%Y-%m-%d %H:%M CST')}`",
        f"- line_id：`{LINE_ID}`",
        f"- 当前 official live：`{OFFICIAL_LIVE_VERSION}` / `{OFFICIAL_LIVE_PROFILE_NAME}`",
        "- B：Stage372逻辑，50万，`risk_multiplier=0.05`，不加 `ni`。",
        "- C：B基础上固定加入 `ni.SHFE` 到产品宇宙和每月AI eligibility。",
...
def _run_variant_suite(
    *,
    ...
    for window_name, window_label, group, start, end in s660.WINDOWS:
        ...
        print(f"[stage665] {spec.capital.variant} {window_name}: {analysis_start.date()} -> {analysis_end.date()}", flush=True)
    ...
    print(f"[stage665] {spec.capital.variant} ytd latest-ai", flush=True)
...
def main() -> None:
    ...
    no_ni_spec = _base_500k_spec(prepared["base_metadata"])
    plus_ni_spec = _plus_ni_500k_spec(prepared["plus_metadata"])
    ...
    plus_summary, plus_curves, plus_cost, plus_positions, plus_usage, plus_annual, plus_monthly = _run_variant_suite(
        ...
        latest_ai_path=LATEST_ELIGIBILITY_PLUS_NI_PATH,
    )
    ...
    ni_activity = _ni_activity(plus_positions, plus_usage)
    ...
    full_cmp = comparison[comparison["window_name"].eq("full_2020_20260430")]
    plus_decision = "plus_ni_rejected"
    if not full_cmp.empty and float(full_cmp.iloc[0]["delta_return_pct"]) > 0 and float(full_cmp.iloc[0]["delta_sharpe"]) >= -0.03:
        plus_decision = "plus_ni_watch_not_auto_promote" if watch_checks else "plus_ni_passes_first_gate"
    decision = {
        "stage": "Stage377",
        "script_stage": "Stage665",
        "line_id": LINE_ID,
        "model_tag": MODEL_TAG,
        ...
        "arms": {
            "B": f"{VARIANT_NO_NI}: 500k, risk_multiplier=0.05, official Stage372 pool",
            "C": f"{VARIANT_PLUS_NI}: B plus fixed {NI_PRODUCT}",
        },
        "decision": plus_decision,
        "hard_fail_checks": hard_fail_checks,
        "watch_checks": watch_checks,
        "overfitting_reflection": (
            "The 500k/risk005 capital profile is a structural sizing test. Adding ni still has selection-after-review "
            "risk, so it must beat the no-ni arm across broad windows before any promotion."
        ),
        "continued_value_reflection": (
            "The low-risk capital profile is worth measuring because it may address margin/path risk. Fixed ni is only "
            "worth continuing if it improves the no-ni 500k/risk005 control without path damage."
        ),
        "inputs": {
            ...
            "plus_ni_universe": str(UNIVERSE_PLUS_NI_PATH),
            "plus_ni_historical_eligibility": str(HIST_ELIGIBILITY_PLUS_NI_PATH),
            "plus_ni_latest_eligibility": str(LATEST_ELIGIBILITY_PLUS_NI_PATH),
        },
        "outputs": {
    ...
    _write_report(summary, cost, comparison, rolling, checks, ni_activity, decision)
