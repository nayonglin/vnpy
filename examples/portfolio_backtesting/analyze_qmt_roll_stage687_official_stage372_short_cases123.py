...
MODEL_TAG = "stage687_official_stage372_short_cases123_v1"
OUTPUT_PREFIX = "qmt_roll_stage687_official_stage372_short_cases123"
...
def _short_cases123_spec(identity_map: str) -> s653.ForcedVariant:
    base = _official_spec(identity_map)
    capital = replace(
        base.capital,
        variant=TARGET_VARIANT,
        label="Stage400 official Stage372 20w recovery sleeve short_case1a/2/3",
        note=(
            "Stage687: keep current official Stage372 20w recovery sleeve profile, "
            "but allow fresh short entries from short_case1a, short_case2 and short_case3."
        ),
    )
    return replace(base, capital=capital, profile="official_stage372_recovery_sleeve_short_cases123")
...
def _comparison(summary: pd.DataFrame, cost: pd.DataFrame) -> pd.DataFrame:
    ...
    for source, metric in fields:
        ...
        rows.append({"metric": metric, "official": base_value, "short_cases123": target_value, "delta": target_value - base_value})
    for multiplier in (2.0, 3.0):
        if multiplier in base_cost.index and multiplier in target_cost.index:
            ...
            rows.append({"metric": f"{multiplier}x_cost_max_dd_pct", "official": base_dd, "short_cases123": target_dd, "delta": target_dd - base_dd})
            base_equity = float(base_cost.loc[multiplier, "end_equity"])
            target_equity = float(target_cost.loc[multiplier, "end_equity"])
            rows.append({"metric": f"{multiplier}x_cost_end_equity", "official": base_equity, "short_cases123": target_equity, "delta": target_equity - base_equity})
...
def _annual_monthly(daily: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    data["net_pnl"] = pd.to_numeric(data.get("net_pnl", 0.0), errors="coerce").fillna(0.0)
...
def _plot(daily: pd.DataFrame) -> None:
    ...
    data["label"] = data["variant"].map(
        {
            ...
            TARGET_VARIANT: "Official + short cases 1a/2/3",
        }
    ).fillna(data["variant"])
    ...
    for label, group in data.sort_values("date").groupby("label"):
        axes[0].plot(group["date"], group["account_equity"], label=label, linewidth=1.25)
        axes[1].plot(group["date"], group["broker10_margin_to_equity_pct"], label=label, linewidth=1.05)
    axes[0].axhline(200_000, color="#94a3b8", linestyle="--", linewidth=0.8)
    axes[0].set_title("Stage400 official Stage372 short-case expansion")
    axes[0].set_ylabel("Equity")
    axes[0].grid(True, linestyle="--", linewidth=0.5, alpha=0.35)
    axes[0].legend(loc="upper left")
    axes[1].axhline(90, color="#ef4444", linestyle="--", linewidth=0.8, alpha=0.55)
    axes[1].axhline(100, color="#991b1b", linestyle="--", linewidth=0.8, alpha=0.55)
...
def _write_report(
    ...
    lines = [
        "# Stage687 Official Stage372 Short Cases 1a/2/3",
        "",
        f"- 生成时间：`{datetime.now().strftime('%Y-%m-%d %H:%M CST')}`",
        f"- line_id：`{LINE_ID}`",
        "- 基准：当前正式版 `official_live_stage372_20w_recovery_sleeve`。",
        "- 新分支：只把 fresh short entry 从 `short_case1a` 放宽为 `short_case1a/short_case2/short_case3`。",
...
def main() -> None:
    ...
    specs = [_official_spec(identity_map), _short_cases123_spec(identity_map)]
    ...
    for spec in specs:
        ...
        print(f"[stage687] running {spec.capital.variant}", flush=True)
        daily, positions, product_margin, usage, forced_events = _run_spec(
            spec,
            metadata,
            allow_short_cases123=is_target,
        )
    ...
    official_return = float(
        summary.loc[summary["variant"].eq(BASE_VARIANT), "total_return_pct"].iloc[0]
    )
    ...
    decision = {
        "stage": "Stage400",
        "script_stage": "Stage687",
        "line_id": LINE_ID,
        "model_tag": MODEL_TAG,
        "baseline": BASE_VARIANT,
        "target": TARGET_VARIANT,
        "allowed_short_signals": sorted(ALLOWED_SHORT_SIGNALS),
        "summary": summary.to_dict("records"),
        "cost": cost.to_dict("records"),
        "comparison": comparison.to_dict("records"),
        "forced_summary": forced_summary.to_dict("records"),
        "decision": "official_stage372_short_cases123_pending_review",
    ...
    product_margin_all.to_csv(PRODUCT_PATH.with_name(f"{OUTPUT_PREFIX}_product_margin_{MODEL_TAG}.csv"), index=False, encoding="utf-8-sig")
