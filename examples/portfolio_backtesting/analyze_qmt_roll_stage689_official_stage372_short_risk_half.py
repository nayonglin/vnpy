...
MODEL_TAG = "stage689_official_stage372_short_risk_half_v1"
OUTPUT_PREFIX = "qmt_roll_stage689_official_stage372_short_risk_half"

LINE_ID = "futures_trend_drawdown30_preserve_return"
BASE_VARIANT = "stage526_200k_force95_to80_recovery_sleeve_r080_pc25_maxpos4"
TARGET_VARIANT = "stage526_200k_force95_to80_recovery_sleeve_r080_pc25_maxpos4_short_risk_half"
            ...
            rows.append(
                {
                    ...
                    "official": base_equity,
                    "short_risk_half": target_equity,
...
def _plot(daily: pd.DataFrame) -> None:
    ...
    data["label"] = data["variant"].map(
        {
            ...
            TARGET_VARIANT: "Official short risk x0.5",
        }
    ).fillna(data["variant"])
    colors = {"Official Stage372": "#f97316", "Official short risk x0.5": "#2563eb"}
    ...
    axes[0].set_title("Stage402 official Stage372 short-risk-half ablation")
...
def _write_report(
    ...
    lines = [
        "# Stage689 Official Stage372 Short Risk Half",
        "",
        f"- 生成时间：`{datetime.now().strftime('%Y-%m-%d %H:%M CST')}`",
        f"- line_id：`{LINE_ID}`",
        "- 基准：当前正式版 `official_live_stage372_20w_recovery_sleeve`。",
        "- 新分支：保留空头开仓路径，但把空头开仓 sizing 的 `risk_amount` 乘以 `0.5` 后重新限制手数。",
...
def main() -> None:
    ...
    specs = [_official_spec(identity_map), _short_risk_half_spec(identity_map)]
    ...
    for spec in specs:
        ...
        print(f"[stage689] running {spec.capital.variant}", flush=True)
        daily, positions, product_margin, usage, forced_events = _run_spec(
            spec,
            metadata,
            short_risk_half=is_target,
        )
    ...
    decision = {
        ...
        "decision": "official_stage372_short_risk_half_pending_review",
