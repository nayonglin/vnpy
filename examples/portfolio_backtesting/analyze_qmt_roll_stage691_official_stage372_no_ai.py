...
MODEL_TAG = "stage691_official_stage372_no_ai_v1"
OUTPUT_PREFIX = "qmt_roll_stage691_official_stage372_no_ai"

LINE_ID = "futures_trend_drawdown30_preserve_return"
BASE_VARIANT = "stage526_200k_force95_to80_recovery_sleeve_r080_pc25_maxpos4"
TARGET_VARIANT = "stage526_200k_force95_to80_recovery_sleeve_r080_pc25_maxpos4_no_ai"
...
def _no_ai_spec(identity_map: str) -> s653.ForcedVariant:
    base = _official_spec(identity_map)
    capital = replace(
        base.capital,
        variant=TARGET_VARIANT,
        label="Stage404 official Stage372 20w recovery sleeve no AI",
        note=(
            "Stage691: keep current official Stage372 20w recovery sleeve profile, "
            "but disable the AI product pool filter."
        ),
    )
    overrides = {
        **base.overrides,
    ...
    return replace(base, capital=capital, overrides=overrides, profile="official_stage372_recovery_sleeve_no_ai")
...
def _comparison(summary: pd.DataFrame, cost: pd.DataFrame) -> pd.DataFrame:
    ...
    for source, metric in fields:
        ...
        rows.append({"metric": metric, "official": base_value, "no_ai": target_value, "delta": target_value - base_value})
    for multiplier in (2.0, 3.0):
        if multiplier in base_cost.index and multiplier in target_cost.index:
            ...
            rows.append({"metric": f"{multiplier}x_cost_max_dd_pct", "official": base_dd, "no_ai": target_dd, "delta": target_dd - base_dd})
            base_equity = float(base_cost.loc[multiplier, "end_equity"])
            target_equity = float(target_cost.loc[multiplier, "end_equity"])
            rows.append({"metric": f"{multiplier}x_cost_end_equity", "official": base_equity, "no_ai": target_equity, "delta": target_equity - base_equity})
...
def _plot(daily: pd.DataFrame) -> None:
    ...
    data["label"] = data["variant"].map(
        {
            ...
            TARGET_VARIANT: "Official no AI",
        }
    ).fillna(data["variant"])
    colors = {"Official Stage372": "#f97316", "Official no AI": "#2563eb"}
    ...
    axes[0].set_title("Stage404 official Stage372 no-AI ablation")
...
def _write_report(
    ...
    lines = [
        "# Stage691 Official Stage372 No AI",
        "",
        f"- 生成时间：`{datetime.now().strftime('%Y-%m-%d %H:%M CST')}`",
        f"- line_id：`{LINE_ID}`",
        "- 基准：当前正式版 `official_live_stage372_20w_recovery_sleeve`。",
        "- 新分支：仅关闭 AI product pool filter，保留正式版品种池、入场、空头、恢复仓和强制减仓逻辑。",
...
def main() -> None:
    ...
    specs = [_official_spec(identity_map), _no_ai_spec(identity_map)]
    ...
    for spec in specs:
        print(f"[stage691] running {spec.capital.variant}", flush=True)
    ...
    decision = {
        ...
        "decision": "official_stage372_no_ai_pending_review",
