...
MODEL_TAG = "stage699_stage407_streak_time_decay60_v1"
OUTPUT_PREFIX = "qmt_roll_stage699_stage407_streak_time_decay60"
OFFICIAL_DECAY_VARIANT = "stage526_200k_force95_to80_recovery_sleeve_r080_pc25_maxpos4_streak_time_decay60"
TARGET_VARIANT = (
    "stage526_200k_force95_to80_recovery_sleeve_r080_pc25_original_ai_plus_jd_ai_rerank_top9_"
    "maxpos5_streak_time_decay60"
)
AI_STRATEGY = "stage699_original_ai_pool_plus_jd_probability_rerank_top9_entry_filter"
AI_SCORE_TYPE = "stage699_original_ai_pool_plus_jd_probability_rerank_top9"
AI_PRE_COVERAGE_SCORE_TYPE = "stage699_official_ai_pre_full_market_coverage"
STREAK_DECAY_CALENDAR_DAYS = 60

_ORIGINAL_CURRENT_STREAK_MULTIPLIER = QmtRollPortfolioStrategy._current_streak_multiplier
_ORIGINAL_UPDATE_STREAK = QmtRollPortfolioStrategy._update_streak_risk_state
...
def _reconfigure_paths() -> None:
    ...
    s696.OFFICIAL_SOFT_VARIANT = OFFICIAL_DECAY_VARIANT
    ...
    s696.GENERATED_DIR = s696.OUTPUT_DIR / "stage699_generated_inputs"
...
def _official_decay_spec(identity_map: str) -> s696.s692.s653.ForcedVariant:
    base = s696._official_spec(identity_map)
    capital = replace(
        base.capital,
        variant=OFFICIAL_DECAY_VARIANT,
        label="Stage412 official streak time-decay 60d",
        note=(
            "Official Stage372 unchanged except the 0.1 loss-streak floor automatically decays back to normal "
            "risk if no further realized loss has occurred for 60 calendar days."
        ),
    )
    overrides = {
        **base.overrides,
    ...
    return replace(base, capital=capital, overrides=overrides, profile="official_stage372_streak_time_decay60")
...
def _stage407_spec(identity_map: str, *, soft_streak: bool) -> s696.s692.s653.ForcedVariant:
    ...
    label = "Stage412 Stage407 streak time-decay 60d" if soft_streak else "Stage407 baseline rerun"
    note = (
        "Stage412 C: Stage407 original AI pool plus jd AI rerank top9 maxpos5, with the 0.1 loss-streak floor "
        "decaying back to normal risk after 60 calendar days without another realized loss."
        if soft_streak
    ...
    profile = "stage407_original_ai_plus_jd_ai_rerank_top9_maxpos5_streak_time_decay60" if soft_streak else "stage407_original_ai_plus_jd_ai_rerank_top9_maxpos5"
    return replace(base, capital=capital, overrides=overrides, profile=profile)


def _patched_update_streak(self: QmtRollPortfolioStrategy, realized_pnl: float, *args: Any, **kwargs: Any) -> None:
    _ORIGINAL_UPDATE_STREAK(self, realized_pnl, *args, **kwargs)
    current_date = getattr(self, "current_bar_date", None)
    if current_date is not None and realized_pnl < 0:
        self._stage699_last_realized_loss_date = pd.Timestamp(current_date).tz_localize(None).normalize()
    if current_date is not None and realized_pnl > 0 and int(getattr(self, "loss_streak", 0) or 0) <= 0:
        self._stage699_last_realized_loss_date = None
...
def _patched_current_streak_multiplier(self: QmtRollPortfolioStrategy) -> float:
    ...
    if raw_multiplier > 0.1000001:
        return raw_multiplier
    current_date = getattr(self, "current_bar_date", None)
    last_loss_date = getattr(self, "_stage699_last_realized_loss_date", None)
    if current_date is None or last_loss_date is None:
        return raw_multiplier
    days_since_loss = int(
        (
            pd.Timestamp(current_date).tz_localize(None).normalize()
            - pd.Timestamp(last_loss_date).tz_localize(None).normalize()
        ).days
    )
    if days_since_loss >= STREAK_DECAY_CALENDAR_DAYS:
        return 1.0
    return raw_multiplier
...
def _run_variant_with_patch(
    ...
    patched_variants = {OFFICIAL_DECAY_VARIANT, TARGET_VARIANT}
    if spec.capital.variant in patched_variants:
        QmtRollPortfolioStrategy._update_streak_risk_state = _patched_update_streak
        QmtRollPortfolioStrategy._current_streak_multiplier = _patched_current_streak_multiplier
    else:
        QmtRollPortfolioStrategy._update_streak_risk_state = _ORIGINAL_UPDATE_STREAK
...
def _entry_risk_summary(candidates: pd.DataFrame, entry_risk: pd.DataFrame) -> pd.DataFrame:
    ...
    for (variant, scope), group in data.groupby(["variant", "scope"], sort=True):
        inferred = group[group["loss_streak"].ge(3) & group["risk_multiplier"].ge(0.999)]
        rows.append(
            {
                "variant": variant,
                "scope": scope,
                "streak_decay_inferred_rows": int(len(inferred)),
                "streak_decay_inferred_opened_rows": int(
                    pd.to_numeric(inferred.get("is_opened", 0.0), errors="coerce").fillna(0.0).sum()
                ),
                "streak_decay_inferred_selected_volume_sum": float(inferred["selected_volume"].sum()),
            }
        )
    inferred_frame = pd.DataFrame(rows)
...
def _plot(daily: pd.DataFrame) -> None:
    ...
    data["label"] = data["variant"].map(
        {
            ...
            OFFICIAL_DECAY_VARIANT: "D Official decay60",
            s696.STAGE407_VARIANT: "B Stage407 current",
            TARGET_VARIANT: "C Stage407 decay60",
        }
    ).fillna(data["variant"])
    colors = {
        "A Official current": "#ea580c",
        "D Official decay60": "#a855f7",
        "B Stage407 current": "#16a34a",
        "C Stage407 decay60": "#2563eb",
    }
    ...
    axes[0].set_title("Stage412 / Script699: loss-streak time decay 60d")
    ...
    ax.axvspan(pd.Timestamp(s696.WINDOW_START), pd.Timestamp(s696.WINDOW_END), color="#ef4444", alpha=0.10)
    ax.set_title("Stage412 Equity Curves: loss-streak time decay 60d")
    ax.set_xlabel("Date")
...
def _decision(
    ...
    decision["stage"] = "Stage412"
    decision["script_stage"] = "Stage699"
    decision["model_tag"] = MODEL_TAG
    decision["official_soft"] = OFFICIAL_DECAY_VARIANT
    decision["target"] = TARGET_VARIANT
    change = dict(decision.get("change", {}))
    change.update(
        {
            "candidate_hypothesis": (
                "Keep the existing 1,1,1,0.1 loss-streak cliff, but treat it as a temporary cooldown: "
                "after 60 calendar days without another realized loss, restore normal risk even if no winning "
                "trade has occurred to reset the loss_streak counter."
            ),
            "streak_risk_multipliers_before": s696.BASE_STREAK_MULTIPLIERS,
            "streak_risk_multipliers_after": s696.BASE_STREAK_MULTIPLIERS,
            "streak_decay_calendar_days": STREAK_DECAY_CALENDAR_DAYS,
        }
    )
    decision["change"] = change
    hard_fail = list(decision.get("hard_fail_checks", []))
    decision["decision"] = "stage407_streak_time_decay60_not_promoted" if hard_fail else "stage407_streak_time_decay60_watch"
    return decision
...
def main() -> None:
    _reconfigure_paths()
    s696._official_soft_spec = _official_decay_spec
