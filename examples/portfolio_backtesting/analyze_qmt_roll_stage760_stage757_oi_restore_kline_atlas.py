...
SOURCE_PREFIX = "qmt_roll_stage757_c50_oi_confirm_risk_restore"
SOURCE_TAG = "stage757_c50_oi_confirm_risk_restore_v1"
SOURCE_RESTORE_LOTS = OUTPUT_DIR / f"{SOURCE_PREFIX}_restore_lots_{SOURCE_TAG}.csv"

OUTPUT_PREFIX = "qmt_roll_stage760_stage757_oi_restore_kline_atlas"
MODEL_TAG = "stage760_stage757_oi_restore_kline_atlas_v1"
...
def _load_oi_restore_lots() -> pd.DataFrame:
    lots = pd.read_csv(SOURCE_RESTORE_LOTS)
    ...
    lots = lots[
        lots["oi_price_confirm_risk_restore_applied"].fillna(0.0).eq(1.0)
    ].copy()
    lots = lots.dropna(subset=["entry_date", "exit_date", "entry_price", "exit_price"])
    lots.sort_values(["entry_date", "exit_date", "lot_id"], inplace=True)
    lots.reset_index(drop=True, inplace=True)
    lots["oi_restore_rank"] = np.arange(1, len(lots) + 1)
...
def _plot_one(price_ax: plt.Axes, volume_ax: plt.Axes, row: pd.Series, bars: pd.DataFrame) -> dict[str, Any]:
    ...
    hold_color = "#dcfce7" if pnl > 0 else "#fee2e2" if pnl < 0 else "#e0f2fe"
    price_ax.axvspan(local_entry_idx, local_exit_idx, color=hold_color, alpha=0.30)
    ...
    title = (
        f"#{int(row['oi_restore_rank'])} lot{int(row['lot_id'])} {row['vt_symbol']} "
        f"{row['direction']} ret={ret:.2f}% R={r_multiple:.2f} "
        f"pnl={_format_money(pnl)} bars={int(row['holding_bar_count'])}"
    )
    subtitle = (
        f"{entry_date:%Y-%m-%d}->{exit_date:%Y-%m-%d} "
        f"{row.get('signal', '')} exit={row.get('exit_reason', '')} "
        f"risk={risk_multiplier:.2f} loss={loss_streak:.0f} "
        f"OI {oi1:.0f}->{oi0:.0f} close {close1:.2f}->{close0:.2f}"
    )
    price_ax.set_title(title + "\n" + subtitle, fontsize=8.6, loc="left")
    price_ax.text(
        0.01,
        0.02,
        "blue=entry purple=exit shaded=holding | lower: volume bars + OI line | pre50/post50",
...
def main() -> None:
    ...
    for page_idx in range(page_count):
        ...
        for ax_idx in range(PER_PAGE):
            ...
            if bars.empty:
                ...
                price_ax.text(
                    ...
                    0.5,
                    (
                        f"missing bars\n#{int(row['oi_restore_rank'])} lot{int(row['lot_id'])}\n"
        ...
        fig.suptitle(
            (
                "Stage760 Stage757 OI restore trades K-line atlas "
                f"(all causal OI restore lots, sorted by entry date, pre{PRE_BARS}/post{POST_BARS}) "
        ...
        [
            {
                ...
                "line_id": LINE_ID,
                "source_restore_lots": str(SOURCE_RESTORE_LOTS),
                "selected_oi_restore_lot_count": int(len(lots)),
                "profit_count": int(lots["realized_pnl"].gt(0.0).sum()),
                "loss_count": int(lots["realized_pnl"].lt(0.0).sum()),
                "flat_count": int(lots["realized_pnl"].eq(0.0).sum()),
                "total_realized_pnl": float(lots["realized_pnl"].sum()),
                "median_r_multiple": float(lots["r_multiple"].median()),
                "median_theory_return_pct": float(lots["theory_return_pct"].median()),
