...
import analyze_qmt_roll_stage766_2019_start_stage757_stage764 as s766


PROJECT_DIR = Path(__file__).resolve().parent
OUTPUT_DIR = PROJECT_DIR / "backtest_outputs"

MODEL_TAG = "stage768_2018_start_stage757_stage764_v1"
OUTPUT_PREFIX = "qmt_roll_stage768_2018_start_stage757_stage764"
...
STAGE767_SUMMARY_PATH = (
    OUTPUT_DIR
    / "qmt_roll_stage767_2018_data_repair_readiness_summary_stage767_2018_data_repair_readiness_v1.json"
)
...
def _row_and_curve(
    ...
    *,
    ...
    row["execution_proxy_note"] = "2018/2019 entries use daily-next-open fallback until Stage149 proxy starts in 2020"
    curve["arm"] = arm
...
def _load_stage767_summary() -> dict[str, Any]:
    if not STAGE767_SUMMARY_PATH.exists():
        return {}
    return json.loads(STAGE767_SUMMARY_PATH.read_text(encoding="utf-8"))
...
def _plot_curves(curves: pd.DataFrame) -> None:
    ...
    fig, axes = plt.subplots(2, 1, figsize=(17, 9.5), sharex=True, gridspec_kw={"height_ratios": [2.25, 1]})
    for label, group in data.groupby("label_for_plot", sort=False):
        group = group.sort_values("date")
        axes[0].plot(group["date"], group["account_equity"], linewidth=1.75, label=label)
        axes[1].plot(group["date"], group["drawdown_pct"], linewidth=1.15, label=label)
    axes[0].set_title("Stage768 2018 start: Stage757 vs Stage764")
...
def _write_report(summary: pd.DataFrame, source_counts: pd.DataFrame, decision: dict[str, Any]) -> None:
    ...
    stage767 = decision.get("stage767_data_repair", {})
    lines = [
        "# Stage768 2018 起点可回测性验证",
        "",
        f"- line_id：`{LINE_ID}`",
        f"- model_tag：`{MODEL_TAG}`",
        f"- 终点：`{ANALYSIS_END.date()}`",
        "- 本阶段只读回测，不修改正式配置，不连接 CTP。",
        "",
        "## 数据和执行 caveat",
        "",
        f"- Stage767 数据修复：`{stage767}`",
        f"- next-real-open proxy 覆盖：`{decision['proxy_coverage']}`",
        f"- AI coverage：`{decision['ai_coverage']}`",
        "- 2018/2019 年内没有 Stage149 next-real-open 分钟代理，成交价格使用 daily next open fallback；因此 2018 起点用于路径敏感性验证，不能和 2020 后执行代理完全等权。",
        "",
        "## 结果摘要",
        "",
        ...
        "## 成交价格来源",
        "",
        _md_table(source_counts, max_rows=120),
        "",
        "## 结论",
        "",
...
def main() -> None:
    ...
    _plot_curves(curves)
    stage767 = _load_stage767_summary()
    decision = {
        "stage": "Stage768",
        "line_id": LINE_ID,
        "model_tag": MODEL_TAG,
        "decision": "2018_start_backtest_completed_with_fu_terminal_gap_and_execution_proxy_caveats",
        "analysis_end": ANALYSIS_END.date().isoformat(),
        "arms": [STAGE757_ARM, STAGE764_ARM],
        "starts": [start.date().isoformat() for start in STARTS],
        "stage767_data_repair": {
            "decision": stage767.get("decision", ""),
            "before_missing_mapped_days": stage767.get("before_missing_mapped_days", None),
            "after_missing_mapped_days": stage767.get("after_missing_mapped_days", None),
            "remaining_missing_days_by_contract": stage767.get("remaining_missing_days_by_contract", {}),
        },
        "proxy_coverage": s766._proxy_coverage(),
        "ai_coverage": s766._ai_coverage(),
        "overfit_judgment": "low: parameters and universe are frozen; only data coverage and independent start date are extended",
        "continue_value": "yes: 2018 adds a materially different path for robustness, but any conclusion must account for 2018/2019 execution proxy caveat",
        "outputs": {
