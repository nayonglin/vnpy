...
OUTPUT_PREFIX = "qmt_roll_stage819_stage813_30w_yearly"
LINE_ID = "futures_trend_2019_data_extension"

CAPITAL_30W = 300_000.0
YEAR_STARTS = s800.YEAR_STARTS
MAX_WORKERS = max(1, min(4, int(os.environ.get("STAGE819_MAX_WORKERS", "4"))))

VARIANT = "stage819_stage813_30w_am41_oi08_old_ai_long_tighter_stop_rsi95_yearly"
LABEL = "Stage819 Stage813 30w AM41 OI0.8 oldAI long-tight RSI95 yearly"
...
COMPARISON_50W_PATH = OUTPUT_DIR / f"{OUTPUT_PREFIX}_comparison_vs_stage813_50w_{MODEL_TAG}.csv"
AGG_50W_PATH = OUTPUT_DIR / f"{OUTPUT_PREFIX}_aggregate_vs_stage813_50w_{MODEL_TAG}.csv"
...
def _profile_30w(metadata: dict[str, Any], start: pd.Timestamp) -> dict[str, Any]:
    ...
    capital = replace(
        ...
        note=(
            f"{spec.capital.note} | Stage819 capital-only stress: account_capital and "
            f"c3_capital changed from 500000 to {CAPITAL_30W:.0f}; all Stage813 logic unchanged."
        ),
    )
    profile = dict(base)
    profile["profile"] = "stage819_stage813_30w_rsi_on"
    profile["spec"] = replace(spec, capital=capital, profile=profile["profile"])
    profile["note"] = (
        "Stage813 official-candidate logic with only account/c3 capital changed to 30w. "
...
def _run_one(start_text: str) -> tuple[dict[str, Any], pd.DataFrame, pd.DataFrame]:
    ...
    row, curve, rsi_events = s813._run_profile_once(
        profile=_profile_30w(metadata, start),
        start=start,
        metadata=metadata,
...
def _comparison(candidate: pd.DataFrame, base: pd.DataFrame) -> pd.DataFrame:
    ...
    comparison["rsi_partial_exit_count_30w"] = comparison["start_month"].map(count_map).fillna(0).astype(int)
    comparison["rsi_partial_exit_volume_30w"] = comparison["start_month"].map(volume_map).fillna(0).astype(int)
...
def _aggregate(comparison: pd.DataFrame) -> pd.DataFrame:
    ...
    for bucket in aggregate["bucket"].astype(str).tolist():
        ...
        aggregate.loc[idx, "total_rsi_partial_exit_count_30w"] = int(frame["rsi_partial_exit_count_30w"].sum())
        aggregate.loc[idx, "total_rsi_partial_exit_volume_30w"] = int(frame["rsi_partial_exit_volume_30w"].sum())
...
def _write_report(comparison: pd.DataFrame, aggregate: pd.DataFrame, decision: dict[str, Any]) -> None:
    display_cols = [
        ...
        "rsi_partial_exit_count_30w",
    ]
    lines = [
        "# Stage819 Stage813 30w本金年度起点回测",
        "",
        f"- line_id：`{LINE_ID}`",
        f"- model_tag：`{MODEL_TAG}`",
        "- A：已登记 Stage813 50w 候选：AM41、基础风险0.40、OI命中恢复0.80、旧正式AI、maxpos4、多头更紧初始止损、RSI95半平。",
        "- C：同 A，仅把 `account_capital/c3_capital` 改成 `300000`。",
...
def main() -> None:
    ...
    print(f"[stage819] launching {len(tasks)} yearly Stage813 30w runs workers={MAX_WORKERS}", flush=True)
    if MAX_WORKERS == 1:
        for idx, task in enumerate(tasks, start=1):
            print(f"[stage819] running {idx}/{len(tasks)} {task}", flush=True)
        ...
        with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for idx, future in enumerate(as_completed(future_map), start=1):
                ...
                curves.append(curve)
                rsi_events.append(events)
                print(f"[stage819] completed {idx}/{len(tasks)} {task}", flush=True)
    ...
    comparison.to_csv(COMPARISON_50W_PATH, index=False, encoding="utf-8-sig")
    aggregate.to_csv(AGG_50W_PATH, index=False, encoding="utf-8-sig")
    ...
    decision_label = "stage819_stage813_30w_yearly_not_live_ready" if hard_fail else "stage819_stage813_30w_yearly_watch"
    decision = {
        "stage": "Stage819",
        "line_id": LINE_ID,
        "model_tag": MODEL_TAG,
        "base": "official_candidate_stage813_50w_am41_oi08_old_ai_long_tighter_stop_rsi95_v1 yearly starts",
        "candidate": "Stage813 logic with account_capital/c3_capital changed to 300000",
        "change": {
            "account_capital_before": 500_000.0,
            "account_capital_after": CAPITAL_30W,
            "c3_capital_before": 500_000.0,
            "c3_capital_after": CAPITAL_30W,
            "strategy_logic_changed": False,
        },
        "decision": decision_label,
        "judgment": (
            "Capital-only deployment stress. This tests whether the Stage813 candidate survives 30w integer lots, "
        ...
        "outputs": {
            ...
            "comparison_vs_50w": str(COMPARISON_50W_PATH),
            "aggregate_vs_50w": str(AGG_50W_PATH),
