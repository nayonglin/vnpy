...
STAGE = "Stage871"
MODEL_TAG = "stage871_stage870_broker10_path_forensics_v1"
OUTPUT_PREFIX = "qmt_roll_stage871_stage870_broker10_path_forensics"
...
C4_ARM = s870.C4_ARM
C9_ARM = s870.C9_ARM
C13_ARM = s870.C13_ARM
ARMS = [C4_ARM, C9_ARM, C13_ARM]
...
CURVE_IN = OUTPUT_DIR / f"{STAGE870_PREFIX}_curve_{STAGE870_TAG}.csv"
CLOSED_LOTS_IN = OUTPUT_DIR / f"{STAGE870_PREFIX}_closed_lots_{STAGE870_TAG}.csv"
ENTRY_RISK_IN = OUTPUT_DIR / f"{STAGE870_PREFIX}_entry_risk_{STAGE870_TAG}.csv"
...
def _peak_dates(curve: pd.DataFrame) -> pd.DataFrame:
    ...
    for focus_date in focus_dates:
        ...
        row["c13_minus_c9_broker10_pct"] = row[f"{C13_ARM}_broker10_pct"] - row[f"{C9_ARM}_broker10_pct"]
        row["c13_minus_c4_broker10_pct"] = row[f"{C13_ARM}_broker10_pct"] - row[f"{C4_ARM}_broker10_pct"]
        output.append(row)
    return pd.DataFrame(output).sort_values(
        [f"{C13_ARM}_peak_rank", "c13_minus_c9_broker10_pct"],
    ...
    for metric in values:
        for arm in ARMS:
            ...
            if column not in wide.columns:
                wide[column] = 0.0
    wide["c13_minus_c9_broker10_pct"] = (
        wide[f"estimated_broker10_margin_to_equity_pct__{C13_ARM}"]
        - wide[f"estimated_broker10_margin_to_equity_pct__{C9_ARM}"]
    )
    wide["c13_minus_c4_broker10_pct"] = (
        wide[f"estimated_broker10_margin_to_equity_pct__{C13_ARM}"]
        - wide[f"estimated_broker10_margin_to_equity_pct__{C4_ARM}"]
    )
    wide["c13_minus_c9_broker10_margin"] = (
        wide[f"estimated_broker10_margin__{C13_ARM}"] - wide[f"estimated_broker10_margin__{C9_ARM}"]
    )
    wide["c13_minus_c9_volume"] = wide[f"volume__{C13_ARM}"] - wide[f"volume__{C9_ARM}"]
    return wide.sort_values(["focus_date", "c13_minus_c9_broker10_pct"], ascending=[True, False]).reset_index(drop=True)


def _denominator_decomposition(curve: pd.DataFrame, peaks: pd.DataFrame) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
    c13_peak_dates = peaks[pd.to_numeric(peaks[f"{C13_ARM}_peak_rank"], errors="coerce").fillna(0).gt(0)].copy()
    for _, item in c13_peak_dates.sort_values(f"{C13_ARM}_peak_rank").iterrows():
        focus_date = pd.Timestamp(item["focus_date"])
        c13 = _curve_snapshot(curve, C13_ARM, focus_date)
        row: dict[str, Any] = {
            "focus_date": focus_date.date().isoformat(),
            "c13_peak_rank": int(item[f"{C13_ARM}_peak_rank"]),
            "c13_broker10_pct": _safe_float(c13.get("broker10_margin_to_equity_pct")),
            "c13_equity": _safe_float(c13.get("account_equity")),
            "c13_broker10_margin": _safe_float(c13.get("estimated_broker10_margin_amount")),
            "c13_drawdown_pct": _safe_float(c13.get("drawdown_pct")),
        }
        for base_arm, prefix in [(C9_ARM, "vs_c9"), (C4_ARM, "vs_c4")]:
            ...
            c13_equity = row["c13_equity"]
            if c13_equity > 0 and np.isfinite(base_margin):
                base_margin_on_c13_equity_pct = base_margin / c13_equity * 100.0
                denominator_effect = base_margin_on_c13_equity_pct - base_broker10_pct
                exposure_effect = row["c13_broker10_pct"] - base_margin_on_c13_equity_pct
            else:
                base_margin_on_c13_equity_pct = np.nan
            ...
            row[f"{prefix}_broker10_delta_pct"] = row["c13_broker10_pct"] - base_broker10_pct
            row[f"{prefix}_equity_ratio_c13_to_base"] = c13_equity / base_equity if base_equity > 0 else np.nan
            row[f"{prefix}_margin_delta"] = row["c13_broker10_margin"] - base_margin
            row[f"{prefix}_base_margin_on_c13_equity_pct"] = base_margin_on_c13_equity_pct
        ...
        rows.append(row)
    return pd.DataFrame(rows).sort_values("c13_peak_rank").reset_index(drop=True)
...
def _cumulative_pnl_delta(closed_lots: pd.DataFrame, focus_dates: list[pd.Timestamp]) -> pd.DataFrame:
    ...
    for focus_date in focus_dates:
        ...
        grouped["c13_minus_c9_realized_pnl"] = grouped[C13_ARM] - grouped[C9_ARM]
        grouped["c13_minus_c4_realized_pnl"] = grouped[C13_ARM] - grouped[C4_ARM]
        rows.extend(grouped.to_dict("records"))
    if not rows:
        return pd.DataFrame()
    return pd.DataFrame(rows).sort_values(["focus_date", "c13_minus_c9_realized_pnl"]).reset_index(drop=True)


def _entry_context(entry_risk: pd.DataFrame, focus_dates: list[pd.Timestamp]) -> pd.DataFrame:
    if entry_risk.empty:
        return pd.DataFrame()
    data = entry_risk[entry_risk["profile"].isin([C9_ARM, C13_ARM])].copy()
...
def _plot_summary(decomposition: pd.DataFrame) -> None:
    if decomposition.empty:
        return
    data = decomposition.sort_values("c13_peak_rank").head(10).copy()
    ...
    axes[0].bar(
        ...
        width=width,
        label="C13-C9 broker10 delta",
        color="#7c3aed",
    )
    axes[0].axhline(0, color="#111827", linewidth=0.8)
    axes[0].set_title("Stage871 C13 broker10 delta decomposition vs C9")
    axes[0].set_xticks(x)
    ...
    axes[1].bar(x, data["c13_broker10_margin"] / 1_000_000.0, label="C13 broker10 margin", color="#2563eb")
    axes[1].bar(
        x,
    ...
    ax2 = axes[1].twinx()
    ax2.plot(x, data["vs_c9_equity_ratio_c13_to_base"], color="#dc2626", marker="s", label="C13/C9 equity ratio")
    axes[1].set_title("Margin numerator and equity denominator")
    axes[1].set_xticks(x)
    axes[1].set_xticklabels(labels, rotation=35, ha="right")
    axes[1].set_ylabel("broker10 margin million")
    ax2.set_ylabel("C13/C9 equity ratio")
...
def _plot_atlas(
    ...
    positive_pairs = pair_delta[pair_delta["c13_minus_c9_broker10_pct"].gt(0)].sort_values(
        "c13_minus_c9_broker10_pct",
        ascending=False,
    )
    selected_rows: list[pd.DataFrame] = []
    for _, pair in positive_pairs.head(12).iterrows():
        rows = available_lots[
            available_lots["arm"].eq(C13_ARM)
    ...
    if selected_rows:
        ...
        selected = available_lots[available_lots["arm"].eq(C13_ARM)].copy()
    selected = (
    ...
    for page_start in range(0, len(selected), PER_PAGE):
        ...
        for ax, (_, row) in zip(axes_arr, page_rows.iterrows(), strict=False):
            ...
            ax.set_title(
                ...
                f"C13 broker10 {row.get('estimated_broker10_margin_to_equity_pct'):.2f}%",
                fontsize=9,
            )
            manifest.append(
                {
        ...
        fig.suptitle("Stage871 C13 broker10 peak active-lot minute-K atlas", fontsize=13)
...
def _decision(decomposition: pd.DataFrame) -> str:
    if decomposition.empty:
        return "stage871_broker10_path_forensics_failed_no_decomposition"
    ...
    if denominator_dominant >= 7 and exposure_expansion <= 2:
        return "stage871_broker10_worse_mainly_equity_denominator_no_engine"
    return "stage871_broker10_has_exposure_signal_needs_readonly_followup"
...
def _write_report(
    ...
    top_positive_pairs = pair_delta[pair_delta["c13_minus_c9_broker10_pct"].gt(0)].sort_values(
        "c13_minus_c9_broker10_pct",
        ascending=False,
    ).head(12)
    top_pnl_deficits = cumulative_pnl_delta.sort_values("c13_minus_c9_realized_pnl").head(12)
    lines = [
        "# Stage871 Stage870 broker10 路径归因",
        "",
        f"- line_id：`{LINE_ID}`",
        ...
        "- 我的判断：Stage870 不是继续调 `progress R` 的问题，而要拆开 broker10 的分子和分母：若主要是权益分母被削低，就不能用简单缩手或入场过滤解决。",
        "",
        "## Decomposition",
        "",
        ...
        "",
        "## C13 Positive Product-Direction Broker10 Deltas",
        "",
        ...
        *[f"- atlas page：`{path}`" for path in atlas_paths],
        ...
        "",
        f"- 决策：`{decision}`。",
        "- 如果 broker10 恶化主要来自 C13 前序权益分母被削低，而不是当下持仓分子扩大，则不应把 Stage871 变成一个新的仓位缩手引擎。",
    ]
...
def main() -> None:
    ...
    c13_peak_dates = [
        ...
        for item in peaks.loc[pd.to_numeric(peaks[f"{C13_ARM}_peak_rank"], errors="coerce").fillna(0).gt(0), "focus_date"]
    ]
    ...
    cumulative_pnl_delta = _cumulative_pnl_delta(closed_lots, c13_peak_dates)
    entry_context = _entry_context(entry_risk, c13_peak_dates)
