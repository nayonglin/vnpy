...
STAGE = "Stage882"
MODEL_TAG = "stage882_stage881_progress_pyramid_engine_v1"
OUTPUT_PREFIX = "qmt_roll_stage882_stage881_progress_pyramid_engine"

C4_ARM = s830.CAP_ARM
C9_ARM = s847.C9_ARM
C16_ARM = "stage882_stage819_c9_progress_pyramid_once"
...
class QmtRollPortfolioStrategyStage882ProgressPyramid(s847.QmtRollPortfolioStrategyStage847C9StopRetry):
    enable_stage882_progress_pyramid_once: bool = False
    stage882_pyramid_progress_r: float = PYRAMID_PROGRESS_R
    stage882_pyramid_add_volume_multiplier: float = PYRAMID_ADD_VOLUME_MULTIPLIER

    parameters = s847.QmtRollPortfolioStrategyStage847C9StopRetry.parameters + [
        "enable_stage882_progress_pyramid_once",
        "stage882_pyramid_progress_r",
        "stage882_pyramid_add_volume_multiplier",
    ]
    variables = s847.QmtRollPortfolioStrategyStage847C9StopRetry.variables + [
        "stage882_pyramid_event_count",
        "stage882_pyramid_stop_count",
    ]

    def __init__(self, strategy_engine, strategy_name: str, vt_symbols: list[str], setting: dict) -> None:
        super().__init__(strategy_engine, strategy_name, vt_symbols, setting)
        self.stage882_pyramid_events: list[dict[str, Any]] = []
        self.stage882_pyramid_event_count: int = 0
        self.stage882_pyramid_stop_count: int = 0
    ...
    def _stage847_stop_retry_event_after_open_trade(self, trade: s827.TradeData) -> dict[str, Any] | None:
        ...
        if c9_event:
            c9_event["stage882_pyramid_checked"] = 0
            return c9_event
        if not bool(self.enable_stage882_progress_pyramid_once):
            return None
        return self._stage882_progress_pyramid_event_after_open_trade(trade)

    def _stage882_record_add_diagnostic(
        self,
        *,
        state: Any,
        ...
        sizing_snapshot = {
            ...
            "sizing_method": "stage882_progress_pyramid",
        }
        self._record_entry_risk_diagnostic(
            ...
            direction=direction,
            bar=fake_bar,
            signal="stage882_progress_pyramid",
            layer_kind="stage882_pyramid",
            volume=add_volume,
            stop_price=stop_price,
            risk_mode=state.risk_mode,
    ...
    def _stage882_append_pyramid_layer(
        self,
        *,
        state: Any,
        direction: str,
    ...
    def _stage882_progress_pyramid_event_after_open_trade(self, trade: s827.TradeData) -> dict[str, Any] | None:
        ...
        progress_r = float(self.stage882_pyramid_progress_r)
        ...
        add_volume = int(round(float(base_volume) * float(self.stage882_pyramid_add_volume_multiplier)))
        if add_volume <= 0:
            return None
        ...
        self._stage882_append_pyramid_layer(
            state=state,
        ...
        self._stage882_record_add_diagnostic(
            state=state,
            contract_vt_symbol=contract_vt_symbol,
            direction=position_direction,
            add_price=add_price,
        ...
        self._record_trade_event(
            ...
            offset="Open",
            reason="stage882_progress_pyramid_open",
            volume=add_volume,
            price=add_price,
        )
        synthetic_trades: list[dict[str, Any]] = [
            {
                "action": "open",
                "source": "stage882_progress_pyramid_open",
                "price": add_price,
                "volume": add_volume,
                "time": progress_time,
            }
        ...
        if stop_idx >= 0:
            ...
            self.stage882_pyramid_stop_count += 1
        else:
            self._apply_state_target(state, execution_price_override=add_price)

        self.stage882_pyramid_event_count += 1
        ...
        event = {
            ...
            "stage882_pyramid_checked": 1,
            "note": "entry-day +0.5R progress pyramid; addon stop is original entry price",
            "synthetic_trades": synthetic_trades,
        }
        self.stage882_pyramid_events.append(event)
        return event


def _c16_profile(metadata: dict[str, Any]) -> dict[str, Any]:
    profile = s847._c9_profile(metadata)
    spec = profile["spec"]
    capital = replace(
        spec.capital,
        variant=f"{C16_ARM}_2018",
        label="Stage882 Stage819 C9 plus one +0.5R progress pyramid",
        note=(
            f"{spec.capital.note} | Stage882 C16. Keep C9 unchanged. If the entry-day path first reaches "
            "+0.5R progress before -0.5R adverse, synthesize one same-volume add at +0.5R. The add-on layer "
            "has a hard stop at the original entry price; no threshold, volume, product, direction, or year scan."
        ),
    )
    overrides = {
        **spec.overrides,
        "enable_stage882_progress_pyramid_once": True,
        "stage882_pyramid_progress_r": PYRAMID_PROGRESS_R,
        "stage882_pyramid_add_volume_multiplier": PYRAMID_ADD_VOLUME_MULTIPLIER,
    }
    result = dict(profile)
    result["profile"] = C16_ARM
    result["strategy_cls"] = QmtRollPortfolioStrategyStage882ProgressPyramid
...
def _run_profile(profile: dict[str, Any], metadata: dict[str, Any]) -> tuple[pd.DataFrame, dict[str, pd.DataFrame]]:
    ...
    try:
        ...
        pyramid_events = pd.DataFrame(getattr(strategy, "stage882_pyramid_events", []) if strategy else [])
...
def _path_diagnostics(curve: pd.DataFrame) -> pd.DataFrame:
    data = curve[curve["arm"].isin([C4_ARM, C9_ARM, C16_ARM])].copy()
...
def _plot_path(curve: pd.DataFrame) -> None:
    ...
    colors = {C4_ARM: "#16a34a", C9_ARM: "#7c3aed", C16_ARM: "#0f766e"}
    labels = {
        C4_ARM: "C4 broker10 cap",
        C9_ARM: "C9 stop/retry",
        C16_ARM: "C16 C9 + +0.5R pyramid",
    }
    for arm, group in data.groupby("arm"):
        group = group.sort_values("date")
    ...
    axes[0].set_title("Stage882 equity path")
...
def _plot_atlas(events: pd.DataFrame, minute_bars: pd.DataFrame) -> tuple[list[Path], pd.DataFrame]:
    ...
    for page in range(1, pages + 1):
        ...
        fig.suptitle("Stage882 +0.5R progress pyramid true-engine entry-day atlas", fontsize=12)
...
def _write_report(
    ...
    lines = [
        ...
        "- 我的判断：Stage881 的代理增量足够大，但只有真实引擎才能检验新增仓是否被保证金、复利路径、后续退出和弱窗口吞掉。",
        "",
        "## 冻结规则",
        "",
        ...
        *[f"- atlas：`{path}`" for path in atlas_paths],
        ...
        "",
        f"- 决策：`{decision_label}`",
        "- 通过线：C16 必须在 C9 上同时改善收益、Sharpe，且不恶化最大回撤和 broker10 峰值，才允许进入滚动起点和成本压力。",
        "- 若不通过，说明 Stage881 代理上限被真实组合路径抵消，停止本 pyramiding 分支，不做小数救参。",
    ]
...
def main() -> None:
    ...
    profile = _c16_profile(metadata)
    combined, frames = _run_profile(profile, metadata)
    c16_summary, c16_curve = s827._metric(profile, combined)
    c16_summary["arm"] = C16_ARM
    c16_curve["arm"] = C16_ARM
    ...
    c16_row = comparison[comparison["arm"].eq(C16_ARM)].iloc[0].to_dict()
    c9_row = comparison[comparison["arm"].eq(C9_ARM)].iloc[0].to_dict()
    c16_passes = (
        float(c16_row["end_equity_delta_vs_C9"]) > 0
        and float(c16_row["sharpe_delta_vs_C9"]) > 0
        and float(c16_row["max_dd_delta_vs_C9"]) >= 0
        and float(c16_row["max_broker10_margin_to_equity_pct"])
        <= float(c9_row["max_broker10_margin_to_equity_pct"])
    )
    decision_label = (
        "stage882_progress_pyramid_true_engine_promising_needs_rolling_cost_stress"
        if c16_passes
        else "stage882_progress_pyramid_true_engine_not_promoted"
    )
    ...
    decision = {
        ...
        "rule": {
            ...
            "add_volume_multiplier": PYRAMID_ADD_VOLUME_MULTIPLIER,
        ...
        "candidate_result": c16_row,
        "overfit_reflection": (
            "不是本轮调参式过拟合。规则只用 Stage881 预声明的 +0.5R、同手数、原入场价止损，"
            "没有扫阈值、比例、品种、方向或年份。但如果本轮失败后继续微调 0.25R/0.75R 或止损位置，就会过拟合。"
        ),
        "continue_value": (
            "只有当 C16 同时改善 C9 的收益、Sharpe、最大回撤和 broker10，才有继续滚动起点和成本压力的价值；"
            "否则 pyramiding 代理上限应视为被真实资金路径反证。"
        ),
        "outputs": {
