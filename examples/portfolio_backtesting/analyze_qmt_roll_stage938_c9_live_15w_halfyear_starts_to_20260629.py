...
REQUESTED_END = pd.Timestamp("2026-06-29")
START_MONTHS = (1, 7)
...
def _write_report(summary: pd.DataFrame, stats: pd.DataFrame, decision: dict[str, Any]) -> None:
    ...
    lines = [
        "# Stage938 C9 当前实盘 15万 2018起逐半年冷启动到2026-06-29",
        "",
        ...
        f"- 统一结束日：`{REQUESTED_END.date()}`；实际结束日取回测曲线最后可用交易日。",
...
def main() -> None:
    ...
    print(
        f"[stage938] live={OFFICIAL_LIVE_VERSION} starts={REQUESTED_START.date()} "
    ...
    for idx, start in enumerate(starts, start=1):
        print(f"[stage938] running {idx}/{len(starts)} start={_date_text(start)}", flush=True)
    ...
    decision = {
        ...
        "decision": "stage938_live_c9_15w_halfyear_starts_to_20260629_measured_no_order_api",
        "order_api_called": False,
        "send_order_api_called_count": 0,
        "cancel_order_api_called_count": 0,
        "ctp_connected": False,
        "external_research_judgment": (
            "Use the existing repository live-wrapper instead of introducing another backtesting engine; "
            "C9 has path-dependent sizing, retry, minute-stop, AI-pool, and broker10 state."
        ),
        "overfit_reflection_before": (
            "否。起点、结束日、资金和当前 live override 都由用户请求固定；本次不调任何策略参数。"
        ),
        "continue_value_before": (
            "是。统一结束日的半年度冷启动曲线能观察不同入场时点到当前版本末端的路径差异。"
        ),
        "overfit_reflection_after": (
            "否。本次只对固定线上版本做多起点冷启动回放；不得用某个起点的好坏反向调整 AI 池或 C9 参数。"
        ),
        "continue_value_after": (
            "是。结果可以作为当前重建版本的路径风险基准；下一步应固定 AI 池与关键派生产物 hash，而不是救参。"
        ),
        "outputs": {
