...
QUALITY_COLUMN_V1: str = "label_selection_quality_score_v1"
QUALITY_BUCKET_COLUMN_V1: str = "label_selection_quality_bucket_v1"
QUALITY_RANK_PCT_COLUMN_V1: str = "label_selection_quality_score_v1_rank_pct_1d"
QUALITY_RANK_CENTERED_COLUMN_V1: str = "label_selection_quality_score_v1_rank_centered_1d"
...
def add_selection_quality_v1_labels(samples_df: pd.DataFrame) -> pd.DataFrame:
    if samples_df.empty:
    ...
    range_tail_penalty = _excess_positive(enriched_df["feature_range_pct_zscore_120"], 0.8, 3.5)
    momentum_tail_penalty = _excess_positive(enriched_df["feature_ret_20d_zscore_120"], 1.0, 3.5)
    trend_extreme_penalty = _excess_positive(
        pd.to_numeric(enriched_df["feature_trend_ma20_gap_pct_cs_rank_centered_1d"], errors="coerce").abs(),
        0.5,
        1.0,
    )
    structure_extreme_penalty = _excess_positive(
        pd.to_numeric(enriched_df["feature_ma20_ma40_gap_pct_cs_zscore_1d"], errors="coerce").abs(),
        1.0,
        4.0,
    )
    close_position_extreme_penalty = _excess_positive(
    ...
    quality_score_v1 = (
        ...
        + 0.15 * mfe_20d
        - 0.22 * mae_20d
        - 0.06 * range_tail_penalty
        - 0.05 * momentum_tail_penalty
        - 0.04 * trend_extreme_penalty
        - 0.03 * structure_extreme_penalty
        - 0.02 * close_position_extreme_penalty
    )
    enriched_df[QUALITY_COLUMN_V1] = quality_score_v1.astype("float64")

    enriched_df[QUALITY_BUCKET_COLUMN_V1] = "small"
    enriched_df.loc[enriched_df[QUALITY_COLUMN_V1] >= 0.0, QUALITY_BUCKET_COLUMN_V1] = "normal"
    enriched_df.loc[enriched_df[QUALITY_COLUMN_V1] >= 1.0, QUALITY_BUCKET_COLUMN_V1] = "large"

    group_size = enriched_df.groupby("candidate_date")["sample_id"].transform("count").astype("float64")
    quality_rank = enriched_df.groupby("candidate_date")[QUALITY_COLUMN_V1].rank(method="average").astype("float64")
    ...
    enriched_df[QUALITY_RANK_PCT_COLUMN_V1] = quality_rank_pct
    enriched_df[QUALITY_RANK_CENTERED_COLUMN_V1] = (quality_rank_pct - 0.5) * 2.0
