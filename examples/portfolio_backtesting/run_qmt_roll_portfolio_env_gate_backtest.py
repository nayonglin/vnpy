...
EXPERIMENT_TAG: str = "qmt_roll_portfolio_env_gate_v1"
...
EXPERIMENTS: tuple[dict[str, Any], ...] = (
    {
        ...
        "file_prefix": "qmt_roll_portfolio_gate_ungated_baseline",
        "chart_title": "QMT Roll Portfolio Gate Ungated Baseline",
        "strategy_overrides": {},
    },
    {
        "experiment_name": "portfolio_env_gate_v1",
        "file_prefix": "qmt_roll_portfolio_env_gate_v1",
        "chart_title": "QMT Roll Portfolio Env Gate v1",
        "strategy_overrides": {
            "enable_portfolio_env_gate": True,
...
def run_experiments() -> pd.DataFrame:
    ...
    for experiment in EXPERIMENTS:
        ...
        print(f"[portfolio-env-gate] running {experiment_name}")
...
def build_summary_payload(summary_df: pd.DataFrame) -> dict[str, Any]:
    ...
    if len(summary_df) >= 2:
        ...
        gated = summary_df[summary_df["experiment_name"] == "portfolio_env_gate_v1"]
...
def main() -> None:
    ...
    print(f"[portfolio-env-gate] summary csv: {SUMMARY_CSV_PATH}")
    print(f"[portfolio-env-gate] summary json: {SUMMARY_JSON_PATH}")
