...
# Low-base specialized sweep for the current 1.2 / 0.9 open-interest regime.
# Base risk explicitly scans below 0.04, while the other three parameters stay
# near the currently relevant ranges observed in prior coarse/refined runs.
BASE_GRID: list[float] = [0.010, 0.015, 0.020, 0.025, 0.030]
OPEN_INTEREST_SURGE_GRID: list[float] = [0.050, 0.060]
VOLUME_OPEN_INTEREST_SURGE_GRID: list[float] = [0.060, 0.070, 0.080]
OPEN_INTEREST_DECLINE_GRID: list[float] = [0.005, 0.010, 0.015, 0.020, 0.025, 0.030]
...
def run_grid() -> pd.DataFrame:
    ...
    print(f"[grid-low-base] valid combos: {len(combos)}")
    ...
    for index, (base_risk, oi_surge_risk, vol_oi_surge_risk, oi_decline_risk) in enumerate(combos, start=1):
        ...
        engine, _, statistics = run_backtest(
            ...
            file_prefix=f"qmt_roll_grid_low_base_{combo_label}",
            chart_title=f"QMT Roll 4-Param Low Base Grid {combo_label}",
        )
...
def main() -> None:
    ...
    result_df = run_grid()
    summary_path: Path = (OUTPUT_DIR / "qmt_roll_risk_4param_grid_low_base_summary.csv").resolve()
    result_df.to_csv(summary_path, index=False, encoding="utf-8-sig")
    print(f"[grid-low-base] summary csv: {summary_path}")
    print("[grid-low-base] top10:")
    print(
        result_df.head(10)[
            [
                "rank",
