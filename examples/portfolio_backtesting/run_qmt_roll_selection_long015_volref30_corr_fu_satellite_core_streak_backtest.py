...
EXPERIMENT_TAG: str = "qmt_roll_selection_long015_volref30_corr_fu_satellite_core_streak"
FORMAL_PREFIX: str = f"{EXPERIMENT_TAG}_formal"
EXPERIMENT_NAME: str = "ai_top8_plus_fu_satellite_post_signal_core_streak"
...
def run_cycle_backtests() -> tuple[pd.DataFrame, pd.DataFrame]:
    strategy_overrides, universe_path, eligibility_path = build_strategy_overrides()
    rows: list[dict[str, Any]] = []
    full_analysis_df: pd.DataFrame | None = None
    ...
    for window in CYCLE_WINDOWS:
        ...
        print(
            f"[fu-satellite-core-streak] {window_name}: "
        ...
        _, analysis_df, statistics = run_backtest(
            ...
            chart_title="QMT Roll AI Top8 + Fu Satellite Core Streak" if save_artifacts else None,
        )
        if save_artifacts:
            full_analysis_df = analysis_df
        rows.append(
            build_summary_row(
                statistics,
                analysis_start=analysis_start,
    ...
    return pd.DataFrame(rows), build_slippage_stress(full_analysis_df)
...
def build_report(summary: pd.DataFrame, slippage_stress: pd.DataFrame, payload: dict[str, Any]) -> str:
    ...
    lines: list[str] = [
        "# AI Top8 + fu卫星：核心连续亏损状态隔离",
        "",
        "## 设计",
        "",
        f"- 新增参数：`streak_risk_state_excluded_products={FU_PRODUCT}`",
        "- 含义：`fu.SHFE` 的平仓盈亏不更新组合连续亏损状态，避免卫星品种盈利把核心池风险降档状态重置。",
        "- 默认行为不变；这是结构性风控隔离，不是按2026单品种亏损拟合。",
        "",
        "## 回测结果",
        "",
        to_markdown_table(
    ...
    lines.extend(
        [
            ...
            "## 判断",
            "",
            "- 该版本不升级正式候选：它修复了2026尾部的一部分路径问题，但明显牺牲了`post_signal_2022_2026`和`early_ai_2022_2023`的风险质量。",
            "- 第75阶段`ai_top8_plus_fu_satellite_post_signal`仍是当前研究第一候选。",
            "- 有价值的结论是：卫星品种会污染组合级连续亏损状态，后续应研究更稳健的状态治理，而不是直接隔离`fu.SHFE`。",
        ]
    )
    lines.extend(
        [
...
def main() -> None:
    ...
    summary, slippage_stress = run_cycle_backtests()
    payload = build_payload(summary, slippage_stress)
    report = build_report(summary, slippage_stress, payload)
    ...
    print(f"[fu-satellite-core-streak] summary csv: {SUMMARY_CSV_PATH}")
    print(f"[fu-satellite-core-streak] summary json: {SUMMARY_JSON_PATH}")
    print(f"[fu-satellite-core-streak] report: {REPORT_PATH}")
