...
EXPERIMENT_TAG: str = "qmt_roll_selection_long015_volref30_corr_fu_satellite_gradual_streak"
FORMAL_PREFIX: str = f"{EXPERIMENT_TAG}_formal"
EXPERIMENT_NAME: str = "ai_top8_plus_fu_satellite_post_signal_gradual_streak"
RECOVERY_MODE: str = "decrement"
...
def run_cycle_backtests() -> pd.DataFrame:
    ...
    for window in CYCLE_WINDOWS:
        ...
        print(
            f"[fu-satellite-gradual-streak] {window_name}: "
        ...
        _, _, statistics = run_backtest(
            ...
            chart_title="QMT Roll AI Top8 + Fu Satellite Gradual Streak" if save_artifacts else None,
        )
        rows.append(
            build_summary_row(
                statistics,
...
def build_report(summary: pd.DataFrame, payload: dict[str, Any]) -> str:
    ...
    lines: list[str] = [
        "# AI Top8 + fu卫星：连续亏损状态渐进恢复",
        "",
        "## 设计",
        "",
        f"- 新增参数：`streak_profit_recovery_mode={RECOVERY_MODE}`",
        "- 含义：盈利平仓不再一次性清零`loss_streak`，而是每次盈利只降低一级连续亏损状态。",
        "- 这是品种无关的风险状态治理，目标是避免单笔盈利把系统从风险收缩状态直接拉回满风险。",
        "",
        "## 回测结果",
        "",
        to_markdown_table(
    ...
    lines.extend(
        [
            ...
            "## 判断",
            "",
            "- 该版本不升级正式候选：2026尾部回撤明显改善，但全周期期末权益和Sharpe被大幅压低。",
            "- `decrement`让连续亏损状态过度黏滞，系统长期处于风险收缩状态，错过了趋势策略最需要承接的主升段。",
            "- 当前研究第一候选仍是第75阶段`ai_top8_plus_fu_satellite_post_signal`；后续应研究“恢复条件”而不是简单慢恢复。",
        ]
    )
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(f"[fu-satellite-gradual-streak] summary csv: {SUMMARY_CSV_PATH}")
    print(f"[fu-satellite-gradual-streak] summary json: {SUMMARY_JSON_PATH}")
    print(f"[fu-satellite-gradual-streak] report: {REPORT_PATH}")
