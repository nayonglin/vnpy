...
EXPERIMENT_TAG: str = (
    ...
    "entry_structure_rsi_recovery_dd_guard"
)
FORMAL_PREFIX: str = f"{EXPERIMENT_TAG}_formal"
EXPERIMENT_NAME: str = (
    "ai_top8_plus_fu_satellite_post_signal_profit_shield_entry_structure_rsi_recovery_dd_guard"
)

ENTRY_STRUCTURE_RECOVERY_MAX_PORTFOLIO_DRAWDOWN_PCT: float = 0.05
    ...
    strategy_overrides = {
        **strategy_overrides,
        ...
        "streak_entry_structure_recovery_max_portfolio_drawdown_pct": (
            ENTRY_STRUCTURE_RECOVERY_MAX_PORTFOLIO_DRAWDOWN_PCT
        ),
    }
...
def run_cycle_backtests() -> pd.DataFrame:
    ...
    for window in CYCLE_WINDOWS:
        ...
        print(
            f"[fu-satellite-profit-shield-entry-structure-rsi-recovery-dd-guard] {window_name}: "
        ...
        _, _, statistics = run_backtest(
            ...
            chart_title="QMT Roll AI Top8 + Fu Satellite Profit Shield Entry Structure RSI Recovery DD Guard"
            if save_artifacts
            else None,
        )
        rows.append(
...
def build_report(summary: pd.DataFrame, payload: dict[str, Any]) -> str:
    ...
    lines: list[str] = [
        "# AI Top8 + fu卫星：入场结构RSI恢复 + 回撤保护",
        "",
        "## 设计",
        "",
        "- 延续第86阶段全部规则，不修改RSI阈值。",
        "- 新增低自由度恢复风险回撤保护：只有组合当前回撤不超过`5%`时，才允许入场结构恢复把风险乘数提高到`1.0`。",
        "- 目标是验证第88阶段发现的2026冷启动尾部是否能被组合状态保护拦截，同时检查全周期收益是否被过度牺牲。",
        "",
        "## 回测结果",
        "",
        to_markdown_table(
        ...
        f"- 相比第86阶段收益增强候选：期末权益差额`{_safe_float(stage86.get('end_balance_diff')):,.0f}`，Sharpe差额`{_safe_float(stage86.get('sharpe_ratio_diff')):.4f}`，最大回撤差额`{_safe_float(stage86.get('max_dd_percent_diff')):.4f}`。",
        "",
        "## 判断",
        "",
        "- 如果`latest_2026`明显改善但全周期收益大幅回吐，说明硬回撤门槛过于粗糙。",
        "- 如果`latest_2026`改善且全周期仍高于第78，可进入起始年份反证。",
        "- 该版本只测试一个原则型阈值`5%`，不做阈值扫描。",
    ]
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(f"[fu-satellite-profit-shield-entry-structure-rsi-recovery-dd-guard] summary csv: {SUMMARY_CSV_PATH}")
    print(f"[fu-satellite-profit-shield-entry-structure-rsi-recovery-dd-guard] summary json: {SUMMARY_JSON_PATH}")
    print(f"[fu-satellite-profit-shield-entry-structure-rsi-recovery-dd-guard] report: {REPORT_PATH}")
