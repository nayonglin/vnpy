...
EXPERIMENT_TAG: str = (
    "qmt_roll_selection_long015_volref30_corr_fu_satellite_profit_shield_entry_structure_rsi_recovery_start_year"
)
...
def run_start_year_backtests() -> pd.DataFrame:
    ...
    for window_name, display_label, analysis_start, analysis_end in START_YEAR_WINDOWS:
        print(
            f"[entry-structure-rsi-recovery-start-year] {window_name}: "
        ...
        rows.append(
            build_summary_row(
                ...
                analysis_start=analysis_start,
                analysis_end=analysis_end,
                strategy_name=STAGE86_EXPERIMENT_NAME,
...
def build_comparison(stage86: pd.DataFrame, reference: pd.DataFrame) -> pd.DataFrame:
    key_columns = ["window_name", "display_label", "analysis_start", "analysis_end"]
    stage86_view = stage86.copy()
    stage86_view["analysis_start"] = pd.to_datetime(stage86_view["analysis_start"]).dt.date.astype(str)
    stage86_view["analysis_end"] = pd.to_datetime(stage86_view["analysis_end"]).dt.date.astype(str)
    ...
    comparison = stage86_view.merge(stage75, on=key_columns, suffixes=("_stage86", "_stage75"))
    ...
    for reference_name in ("stage75", "stage78"):
        for column in (
            ...
            comparison[f"{column}_diff_vs_{reference_name}"] = (
                comparison[f"{column}_stage86"] - comparison[f"{column}_{reference_name}"]
            )
        comparison[f"stage86_beats_{reference_name}_end_balance"] = (
            comparison[f"end_balance_diff_vs_{reference_name}"] > 0.0
        ).astype(int)
        comparison[f"stage86_beats_{reference_name}_sharpe"] = (
            comparison[f"sharpe_ratio_diff_vs_{reference_name}"] > 0.0
        ).astype(int)
    return comparison


def build_payload(stage86: pd.DataFrame, comparison: pd.DataFrame) -> dict[str, Any]:
    aggregate: dict[str, Any] = {"windows": int(len(comparison))}
    for reference_name in ("stage75", "stage78"):
        aggregate[f"stage86_end_balance_wins_vs_{reference_name}"] = int(
            comparison[f"stage86_beats_{reference_name}_end_balance"].sum()
        )
        aggregate[f"stage86_sharpe_wins_vs_{reference_name}"] = int(
            comparison[f"stage86_beats_{reference_name}_sharpe"].sum()
        )
    ...
    return {
        ...
        "stage86_summary": stage86.to_dict(orient="records"),
...
def build_report(stage86: pd.DataFrame, comparison: pd.DataFrame, payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## 设计",
        "",
        "- 只运行第86阶段同一套参数的起始年份窗口，不做再优化。",
        "- 第75阶段与第78阶段结果直接复用既有起始年份稳健性产物，避免重复回测。",
        "- 目标是确认第86阶段全周期创新高是否依赖单一起点。",
        "",
        "## 汇总",
        "",
        f"- 起始年份窗口数：`{int(_safe_float(aggregate.get('windows'))):,}`",
        f"- 第86相对第78期末权益胜出窗口：`{int(_safe_float(aggregate.get('stage86_end_balance_wins_vs_stage78'))):,}`",
        f"- 第86相对第78 Sharpe胜出窗口：`{int(_safe_float(aggregate.get('stage86_sharpe_wins_vs_stage78'))):,}`",
        f"- 第86相对第78平均期末权益差额：`{_safe_float(aggregate.get('avg_end_balance_diff_vs_stage78')):,.0f}`",
        f"- 第86相对第78平均Sharpe差额：`{_safe_float(aggregate.get('avg_sharpe_diff_vs_stage78')):.4f}`",
        f"- 第86相对第78最差期末权益差额：`{_safe_float(aggregate.get('worst_end_balance_diff_vs_stage78')):,.0f}`",
        "",
        "## 起始年份对比",
        "",
        ...
        "## 判断",
        "",
        "- 如果第86只在`since_2020`胜出，但较晚起点明显恶化，不能升级为正式版本。",
        "- 如果多个起点稳定胜出但`latest_2026`仍崩坏，应保留收益增强身份，并继续做尾部暂停机制。",
    ]
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    stage86 = run_start_year_backtests()
    reference = load_reference_summary()
    comparison = build_comparison(stage86, reference)
    payload = build_payload(stage86, comparison)
    report = build_report(stage86, comparison, payload)

    stage86.to_csv(SUMMARY_CSV_PATH, index=False, encoding="utf-8-sig")
    comparison.to_csv(COMPARISON_CSV_PATH, index=False, encoding="utf-8-sig")
    SUMMARY_JSON_PATH.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    REPORT_PATH.write_text(report, encoding="utf-8")

    print(f"[entry-structure-rsi-recovery-start-year] summary csv: {SUMMARY_CSV_PATH}")
    print(f"[entry-structure-rsi-recovery-start-year] comparison csv: {COMPARISON_CSV_PATH}")
    print(f"[entry-structure-rsi-recovery-start-year] summary json: {SUMMARY_JSON_PATH}")
    print(f"[entry-structure-rsi-recovery-start-year] report: {REPORT_PATH}")
