...
EXPERIMENT_TAG: str = "qmt_roll_selection_long015_volref30_corr_fu_satellite_profit_shield_streak"
FORMAL_PREFIX: str = f"{EXPERIMENT_TAG}_formal"
EXPERIMENT_NAME: str = "ai_top8_plus_fu_satellite_post_signal_profit_shield_streak"
...
def run_cycle_backtests() -> pd.DataFrame:
    ...
    for window in CYCLE_WINDOWS:
        ...
        print(
            f"[fu-satellite-profit-shield-streak] {window_name}: "
        ...
        _, _, statistics = run_backtest(
            ...
            chart_title="QMT Roll AI Top8 + Fu Satellite Profit Shield Streak" if save_artifacts else None,
        )
        rows.append(
            build_summary_row(
                statistics,
...
def build_payload(summary: pd.DataFrame) -> dict[str, Any]:
    references = {
        ...
        "stage76_fu_satellite_core_streak": {
            "end_balance": 4_260_025,
            "total_return_pct": 2030.0125,
            "max_dd_percent": -36.990703,
            "sharpe_ratio": 1.2083,
            "total_slippage": 275_900,
            "total_trade_count": 787,
        },
...
def build_report(summary: pd.DataFrame, payload: dict[str, Any]) -> str:
    ...
    stage76 = payload.get("comparison_vs_stage76_fu_satellite_core_streak", {})
    ai_top8 = payload.get("comparison_vs_stage68_71_ai_top8_product_pool", {})

    lines: list[str] = [
        "# AI Top8 + fu卫星：卫星盈利屏蔽连续亏损恢复",
        "",
        "## 设计",
        "",
        f"- 新增参数验证：`streak_risk_state_excluded_products={FU_PRODUCT}`，`streak_risk_state_exclusion_mode={EXCLUSION_MODE}`",
        "- 含义：`fu.SHFE`盈利平仓不清零`loss_streak`，但`fu.SHFE`亏损仍会增加连续亏损状态。",
        "- 这比第76阶段完全隔离`fu`更保守，也比全局慢恢复更聚焦，目标是防止卫星盈利替核心池解除风险。",
        "",
        "## 回测结果",
        "",
        to_markdown_table(
    ...
    if not full.empty:
        lines.extend(
            [
                ...
                f"- 相比第76阶段`fu`完全隔离：期末权益差额`{_safe_float(stage76.get('end_balance_diff')):,.0f}`，Sharpe差额`{_safe_float(stage76.get('sharpe_ratio_diff')):.4f}`，最大回撤差额`{_safe_float(stage76.get('max_dd_percent_diff')):.4f}`。",
    ...
    lines.extend(
        [
            ...
            "## 判断",
            "",
            "- 该版本升级为风险治理第一候选：全周期收益几乎不输第75阶段，同时显著改善2026尾部并降低交易成本。",
            "- 第75阶段仍是收益上限基准；本版本更适合作为穿越周期的风控候选，因为它用很小收益代价约束了卫星品种对组合状态的污染。",
            "- 后续仍需做起始年份、极端交易和实盘可执行性复核，不能只凭单次全周期结果定版。",
        ]
    )
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(f"[fu-satellite-profit-shield-streak] summary csv: {SUMMARY_CSV_PATH}")
    print(f"[fu-satellite-profit-shield-streak] summary json: {SUMMARY_JSON_PATH}")
    print(f"[fu-satellite-profit-shield-streak] report: {REPORT_PATH}")
