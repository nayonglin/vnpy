...
EXPERIMENT_TAG: str = "qmt_roll_selection_pairwise_long015_base_volume_neighbors_backtest"
...
EXPERIMENTS: tuple[dict[str, Any], ...] = (
    {
        ...
        "chart_title": "QMT Roll Selection Pairwise v2 Volume Tilt Long015 Neighbors",
        "strategy_overrides": BASE_LONG015,
        "reuse_existing_statistics": True,
    },
    {
        "experiment_name": "selection_pairwise_v2_volume_tilt_long015_volref25",
        "file_prefix": "qmt_roll_selection_pairwise_v2_volume_tilt_long015_volref25_neighbors",
        "chart_title": "QMT Roll Selection Pairwise v2 Volume Tilt Long015 VolRef25 Neighbors",
    ...
    },
    {
        ...
        "chart_title": "QMT Roll Selection Pairwise v2 Volume Tilt Long015 VolRef30 Neighbors",
        "strategy_overrides": build_base_volume_overrides(30.0),
        "reuse_existing_statistics": True,
    },
    {
        "experiment_name": "selection_pairwise_v2_volume_tilt_long015_volref35",
        "file_prefix": "qmt_roll_selection_pairwise_v2_volume_tilt_long015_volref35_neighbors",
        "chart_title": "QMT Roll Selection Pairwise v2 Volume Tilt Long015 VolRef35 Neighbors",
...
def load_existing_statistics(file_prefix: str) -> dict[str, Any] | None:
    statistics_path = OUTPUT_DIR / f"{file_prefix}_statistics.json"
...
def run_experiments() -> pd.DataFrame:
    ...
    for experiment in EXPERIMENTS:
        ...
        if bool(experiment.get("reuse_existing_statistics", False)):
            statistics = load_existing_statistics(file_prefix)
            if statistics is not None:
                print(
                    "[selection-pairwise-long015-base-volume-neighbors] "
                    f"reuse existing statistics for {experiment_name}: {file_prefix}"
                )

        if statistics is None:
            print(f"[selection-pairwise-long015-base-volume-neighbors] running {experiment_name}")
...
def main() -> None:
    ...
    print(f"[selection-pairwise-long015-base-volume-neighbors] summary csv: {SUMMARY_CSV_PATH}")
    print(f"[selection-pairwise-long015-base-volume-neighbors] summary json: {SUMMARY_JSON_PATH}")
