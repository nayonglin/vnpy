...
EXPERIMENT_TAG: str = "qmt_roll_selection_pairwise_long015_volref30_corr_crowding_neighbors_fast"
...
EXPERIMENTS: tuple[dict[str, Any], ...] = (
    ...
    {
        "experiment_name": "corr20_060_080_floor35",
    ...
    {
        "experiment_name": "corr20_060_080_floor25",
        "strategy_overrides": correlation_gate_overrides(
            lookback=20,
            start=0.60,
            full=0.80,
    ...
    {
        "experiment_name": "corr20_060_080_floor50",
        "strategy_overrides": correlation_gate_overrides(
            lookback=20,
            start=0.60,
            full=0.80,
    ...
    {
        "experiment_name": "corr15_060_080_floor35",
        "strategy_overrides": correlation_gate_overrides(
            lookback=15,
            start=0.60,
            full=0.80,
    ...
    {
        "experiment_name": "corr30_060_080_floor35",
        "strategy_overrides": correlation_gate_overrides(
            lookback=30,
            start=0.60,
            full=0.80,
    ...
    {
        "experiment_name": "corr20_055_080_floor35",
        "strategy_overrides": correlation_gate_overrides(
            lookback=20,
            start=0.55,
            full=0.80,
    ...
    {
        "experiment_name": "corr20_060_085_floor35",
        "strategy_overrides": correlation_gate_overrides(
            lookback=20,
            start=0.60,
            full=0.85,
...
def run_experiments() -> pd.DataFrame:
    ...
    for experiment in EXPERIMENTS:
        ...
        print(f"[volref30-corr-crowding-neighbors-fast] running {experiment_name}")
    ...
    summary_df.sort_values("end_balance", ascending=False, inplace=True)
...
def build_comparison_payload(summary_df: pd.DataFrame) -> dict[str, Any]:
    ...
    current_df = summary_df[summary_df["experiment_name"] == "volref30_current"]
    anchor_df = summary_df[summary_df["experiment_name"] == "corr20_060_080_floor35"]
    if not current_df.empty:
        current_row = current_df.iloc[0]
        payload["comparison_vs_volref30_current"] = [
            _comparison_row(current_row, row)
            for _, row in summary_df.iterrows()
            if str(row["experiment_name"]) != "volref30_current"
        ]
    if not anchor_df.empty:
        anchor_row = anchor_df.iloc[0]
        payload["comparison_vs_corr20_060_080_floor35"] = [
            _comparison_row(anchor_row, row)
            for _, row in summary_df.iterrows()
            if str(row["experiment_name"]) != "corr20_060_080_floor35"
        ]
    return payload


def _comparison_row(base_row: pd.Series, compare_row: pd.Series) -> dict[str, Any]:
    return {
...
def main() -> None:
    ...
    print(f"[volref30-corr-crowding-neighbors-fast] summary csv: {SUMMARY_CSV_PATH}")
    print(f"[volref30-corr-crowding-neighbors-fast] summary json: {SUMMARY_JSON_PATH}")
