...
def run_grid() -> pd.DataFrame:
    ...
    for index, single_cap_ratio in enumerate(SINGLE_CAP_GRID, start=1):
        ...
        print(f"[single-cap-grid] {index}/{len(SINGLE_CAP_GRID)} single_cap_ratio={single_cap_ratio:.2f}")
        _, _, statistics = run_backtest(
            ...
            file_prefix=f"qmt_roll_single_cap_{ratio_label}",
            chart_title=f"QMT Roll Single Cap Grid {ratio_label}",
        )
...
def main() -> None:
    ...
    result_df = run_grid()
    summary_path: Path = (OUTPUT_DIR / "qmt_roll_single_cap_grid_summary.csv").resolve()
    result_df.to_csv(summary_path, index=False, encoding="utf-8-sig")
    print(f"[single-cap-grid] summary csv: {summary_path}")
    print("[single-cap-grid] full ranking:")
