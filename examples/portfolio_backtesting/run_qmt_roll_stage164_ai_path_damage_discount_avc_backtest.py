...
MODEL_TAG: str = "stage164_ai_path_damage_discount_avc_v1"
OUTPUT_PREFIX: str = "qmt_roll_stage164_ai_path_damage_discount_avc"
...
PROFILE_C: str = "C_stage78_ai_path_damage_discount80_oos2023"

DISCOUNT_START_DATE: str = "2023-01-01"
DISCOUNT_PROBABILITY_START: float = 0.25
DISCOUNT_PROBABILITY_FULL: float = 0.75
DISCOUNT_WEIGHT_FLOOR: float = 0.80
    ...
    candidate.update(
        {
            ...
            "ai_path_damage_discount_start_date": DISCOUNT_START_DATE,
            "ai_path_damage_discount_probability_start": DISCOUNT_PROBABILITY_START,
            "ai_path_damage_discount_probability_full": DISCOUNT_PROBABILITY_FULL,
            "ai_path_damage_discount_weight_floor": DISCOUNT_WEIGHT_FLOOR,
        }
    )
    return (
        ExperimentArm(
        ...
        ExperimentArm(
            ...
            arm="C",
            hypothesis=(
                "第78加AI纯路径损伤概率连续风险折扣；不改信号，不过滤品种，"
                "只在2023年后把高坏路径概率候选最低降至80%体量。"
            ),
...
def _candidate_summary(engine: Any, arm: ExperimentArm, window_name: str) -> dict[str, Any]:
    ...
    base = {
        ...
        "ai_path_damage_enabled_count": 0,
        "ai_path_damage_feature_available_count": 0,
        "ai_path_damage_discount_applied_count": 0,
        "ai_path_damage_avg_probability": 0.0,
        "ai_path_damage_median_weight": 1.0,
        "ai_path_damage_avg_weight": 1.0,
        "ai_path_damage_total_volume_before": 0,
        "ai_path_damage_total_volume_after": 0,
        "ai_path_damage_volume_delta": 0,
    }
    if candidates.empty:
        return base
    frame = candidates[candidates.get("entry_context", "").astype(str).eq("flat_entry")].copy()
    if frame.empty:
        return base
    ...
    enabled = frame[frame["ai_path_damage_enabled"] > 0].copy()
    base.update(
        {
            "flat_candidate_count": int(len(frame)),
            "opened_flat_entry_count": int(len(opened)),
            "ai_path_damage_enabled_count": int((frame["ai_path_damage_enabled"] > 0).sum()),
            "ai_path_damage_feature_available_count": int((frame["ai_path_damage_feature_available"] > 0).sum()),
            "ai_path_damage_discount_applied_count": int((frame["ai_path_damage_discount_applied"] > 0).sum()),
            "ai_path_damage_avg_probability": _safe_float(enabled["ai_path_damage_probability"].mean()) if not enabled.empty else 0.0,
            "ai_path_damage_median_weight": _safe_float(enabled["ai_path_damage_discount_weight"].median(), 1.0)
            if not enabled.empty
            else 1.0,
            "ai_path_damage_avg_weight": _safe_float(enabled["ai_path_damage_discount_weight"].mean(), 1.0)
            if not enabled.empty
            else 1.0,
            "ai_path_damage_total_volume_before": int(_safe_float(enabled["ai_path_damage_selected_volume_before"].sum())),
            "ai_path_damage_total_volume_after": int(_safe_float(enabled["ai_path_damage_selected_volume_after"].sum())),
            "ai_path_damage_volume_delta": int(
                _safe_float(enabled["ai_path_damage_selected_volume_after"].sum())
                - _safe_float(enabled["ai_path_damage_selected_volume_before"].sum())
            ),
        }
    )
    return base
...
def _run_one(arm: ExperimentArm, window: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any], pd.DataFrame, pd.DataFrame]:
    window_name = str(window["window_name"])
    analysis_start: datetime = window["analysis_start"]
    analysis_end: datetime = window["analysis_end"]
    print(f"[stage164-ai-path-damage] {window_name} / {arm.profile_name}: {analysis_start.date()} -> {analysis_end.date()}", flush=True)
    ...
    candidate_row = _candidate_summary(engine, arm, window_name)
...
def _experiment_decision(comparison: pd.DataFrame, candidate_summary: pd.DataFrame) -> str:
    if comparison.empty:
    ...
    applied_count = int(
        _safe_float(
            candidate_summary[
                (candidate_summary["profile_name"].astype(str).eq(PROFILE_C))
                & (candidate_summary["window_name"].astype(str).eq("full_2020_2026"))
            ]["ai_path_damage_discount_applied_count"].sum()
        )
    )
    if applied_count <= 0:
...
def _build_report(summary: pd.DataFrame, comparison: pd.DataFrame, candidate_summary: pd.DataFrame, decision: str) -> str:
    result_columns = [
        "profile_name",
        "window_name",
        "end_balance",
    ...
    candidate_columns = [
        ...
        "ai_path_damage_enabled_count",
        "ai_path_damage_feature_available_count",
        "ai_path_damage_discount_applied_count",
        "ai_path_damage_avg_probability",
        "ai_path_damage_median_weight",
        "ai_path_damage_volume_delta",
    ]
    return "\n".join(
        [
            "# Stage164 AI Path-Damage Discount A Vs C",
            "",
            "## Boundary",
            "",
            f"- A = `{PROFILE_A}`.",
            f"- C = `{PROFILE_C}`.",
            "- B is not meaningful because this is a deployment risk-budget layer.",
            "- The model is frozen from Stage163 and the discount starts from 2023-01-01 to avoid using train-period evidence as deployment proof.",
            "- No product blacklist, date patch, signal change, or threshold sweep is used.",
            "",
            "## Predeclared Candidate",
            "",
            f"- Probability start: `{DISCOUNT_PROBABILITY_START:.2f}`",
            f"- Probability full: `{DISCOUNT_PROBABILITY_FULL:.2f}`",
            f"- Weight floor: `{DISCOUNT_WEIGHT_FLOOR:.2f}`",
            "",
            "## Results",
            "",
            ...
            "",
            to_markdown_table(candidate_summary[candidate_columns]) if not candidate_summary.empty else "_empty_",
            "",
            "## Decision",
            "",
            f"- `{decision}`",
...
def main() -> None:
    ...
    for window in windows:
        for arm in arms:
            row, candidate_row, daily_margin, run_log = _run_one(arm, window)
    ...
    candidate_summary = pd.DataFrame(candidate_rows).sort_values(["window_name", "profile_name"]).reset_index(drop=True)
    ...
    decision = _experiment_decision(comparison, candidate_summary)
    ...
    REPORT_PATH.write_text(_build_report(summary, comparison, candidate_summary, decision), encoding="utf-8")
        ...
        json.dumps(
            {
                ...
                "discount_profile": {
                    ...
                    "probability_start": DISCOUNT_PROBABILITY_START,
                    "probability_full": DISCOUNT_PROBABILITY_FULL,
                    "weight_floor": DISCOUNT_WEIGHT_FLOOR,
                },
                "windows": [
                    {
                        "window_name": str(window["window_name"]),
    ...
    print(f"[stage164-ai-path-damage] summary: {SUMMARY_CSV_PATH}")
    print(f"[stage164-ai-path-damage] comparison: {COMPARISON_CSV_PATH}")
    print(f"[stage164-ai-path-damage] candidate summary: {CANDIDATE_SUMMARY_CSV_PATH}")
    print(f"[stage164-ai-path-damage] report: {REPORT_PATH}")
    print(f"[stage164-ai-path-damage] decision: {decision}")
