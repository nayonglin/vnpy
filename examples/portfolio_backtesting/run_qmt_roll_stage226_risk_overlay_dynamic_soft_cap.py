...
MODEL_TAG = "stage226_risk_overlay_dynamic_soft_cap_v1"
OUTPUT_PREFIX = "qmt_roll_stage226_risk_overlay_dynamic_soft_cap"
...
def _variant_overrides(variant: str, analysis_start: datetime) -> dict[str, Any]:
    ...
    if variant == "dynamic_soft_cap_v1":
        overrides.update(
            {
...
def _run_window(variant: str, window_name: str, display_label: str, analysis_start: datetime, analysis_end: datetime) -> tuple[Any, pd.DataFrame, dict[str, Any]]:
    ...
    return run_backtest(
        ...
        chart_title=f"Stage226 risk overlay {variant} {display_label}",
    )
...
def run_variants() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for variant in ("baseline_78_1", "dynamic_soft_cap_v1"):
        for window_name, display_label, analysis_start, analysis_end in WINDOWS:
            print(f"[stage226] {variant} {window_name}", flush=True)
...
def _comparison(summary_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for window_name, group in summary_df.groupby("window_name", sort=False):
        ...
        candidate = group[group["variant"].eq("dynamic_soft_cap_v1")]
...
def _build_report(summary_df: pd.DataFrame, comparison_df: pd.DataFrame, slippage_df: pd.DataFrame, mc_summary_df: pd.DataFrame, paths: dict[str, str]) -> str:
    lines = [
        "# Stage226 风险覆盖层：动态sizing软上限最小验证",
        "",
        "## 口径",
        "",
        ...
        "- C：`dynamic_soft_cap_v1`，只启用动态sizing软上限。",
...
def main() -> None:
    ...
    slippage_df = pd.concat(
        [_slippage_stress(variant, full_daily[full_daily["variant"].eq(variant)]) for variant in ("baseline_78_1", "dynamic_soft_cap_v1")],
        ignore_index=True,
    )
        ...
        json.dumps(
            {
                ...
                "candidate_overrides": _variant_overrides("dynamic_soft_cap_v1", START_DT),
                "paths": paths,
            },
            ensure_ascii=False,
            indent=2,
