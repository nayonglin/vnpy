...
MODEL_TAG = "stage228_risk_overlay_layered_profit_lock_v2"
OUTPUT_PREFIX = "qmt_roll_stage228_risk_overlay_layered_profit_lock_v2"
...
def _variant_overrides(variant: str, analysis_start: datetime) -> dict[str, Any]:
    ...
    if variant == "layered_profit_lock_v2":
        overrides.update(
            {
...
def _run_window(variant: str, window_name: str, display_label: str, analysis_start: datetime, analysis_end: datetime) -> tuple[Any, pd.DataFrame, dict[str, Any]]:
    ...
    return run_backtest(
        ...
        chart_title=f"Stage228 risk overlay {variant} {display_label}",
    )
...
def run_variants() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for variant in ("baseline_78_1", "layered_profit_lock_v2"):
        for window_name, display_label, analysis_start, analysis_end in WINDOWS:
            print(f"[stage228] {variant} {window_name}", flush=True)
...
def _comparison(summary_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for window_name, group in summary_df.groupby("window_name", sort=False):
        ...
        candidate = group[group["variant"].eq("layered_profit_lock_v2")]
...
def _build_report(summary_df: pd.DataFrame, comparison_df: pd.DataFrame, slippage_df: pd.DataFrame, mc_summary_df: pd.DataFrame, paths: dict[str, str]) -> str:
    lines = [
        ...
        "- C：`layered_profit_lock_v2`，启用多层阶梯分层出金/权益锁定sizing覆盖层。",
...
def main() -> None:
    ...
    slippage_df = pd.concat(
        [_slippage_stress(variant, full_daily[full_daily["variant"].eq(variant)]) for variant in ("baseline_78_1", "layered_profit_lock_v2")],
        ignore_index=True,
    )
        ...
        json.dumps(
            {
                ...
                "candidate_overrides": _variant_overrides("layered_profit_lock_v2", START_DT),
                "paths": paths,
            },
            ensure_ascii=False,
            indent=2,
