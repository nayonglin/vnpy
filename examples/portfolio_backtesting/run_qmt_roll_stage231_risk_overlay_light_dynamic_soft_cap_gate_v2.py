...
MODEL_TAG = "stage231_risk_overlay_light_dynamic_soft_cap_gate_v2"
OUTPUT_PREFIX = "qmt_roll_stage231_risk_overlay_light_dynamic_soft_cap_gate_v2"
...
def _variant_overrides(variant: str, analysis_start: datetime) -> dict[str, Any]:
    ...
    if variant == "light_dynamic_soft_cap_gate_v2":
        overrides.update(
            {
                "enable_dynamic_sizing_equity_soft_cap": True,
                "dynamic_sizing_equity_soft_cap_base": 2_000_000.0,
                "dynamic_sizing_equity_soft_cap_max": 7_500_000.0,
...
def _run_window(variant: str, window_name: str, display_label: str, analysis_start: datetime, analysis_end: datetime) -> tuple[Any, pd.DataFrame, dict[str, Any]]:
    ...
    return run_backtest(
        ...
        chart_title=f"Stage231 risk overlay {variant} {display_label}",
    )
...
def run_variants() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for variant in ("baseline_78_1", "light_dynamic_soft_cap_gate_v2"):
        for window_name, display_label, analysis_start, analysis_end in WINDOWS:
            print(f"[stage231] {variant} {window_name}", flush=True)
...
def _comparison(summary_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for window_name, group in summary_df.groupby("window_name", sort=False):
        ...
        candidate = group[group["variant"].eq("light_dynamic_soft_cap_gate_v2")]
...
def _build_report(summary_df: pd.DataFrame, comparison_df: pd.DataFrame, slippage_df: pd.DataFrame, mc_summary_df: pd.DataFrame, paths: dict[str, str]) -> str:
    lines = [
        "# Stage231 风险覆盖层：轻量动态软上限收益恢复版v2验证",
        "",
        "## 口径",
        "",
        ...
        "- C：`light_dynamic_soft_cap_gate_v2`，启用收益恢复版轻量动态sizing软上限，并叠加同日新增仓保证金预算门禁。",
...
def main() -> None:
    ...
    slippage_df = pd.concat(
        [_slippage_stress(variant, full_daily[full_daily["variant"].eq(variant)]) for variant in ("baseline_78_1", "light_dynamic_soft_cap_gate_v2")],
        ignore_index=True,
    )
        ...
        json.dumps(
            {
                ...
                "candidate_overrides": _variant_overrides("light_dynamic_soft_cap_gate_v2", START_DT),
                "paths": paths,
            },
            ensure_ascii=False,
            indent=2,
