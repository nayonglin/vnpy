...
MODEL_TAG = "stage296_stage78_1_loss_streak_risk_v1"
OUTPUT_PREFIX = "qmt_roll_stage296_stage78_1_loss_streak_risk"
...
PROFILES: tuple[Profile, ...] = (
    ...
    Profile(
        ...
        "连续亏损温和降风险，盈利即恢复",
        {"streak_risk_multipliers": "1.0,0.8,0.5,0.2"},
    ),
    Profile(
        "C_loss_streak_aggressive_reset",
        "连续亏损积极降风险，盈利即恢复",
        {"streak_risk_multipliers": "1.0,0.6,0.3,0.1"},
    ),
    Profile(
...
def _run_profile(profile: Profile, window_name: str, display_label: str, analysis_start: datetime, analysis_end: datetime) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    _, analysis_df, statistics = run_backtest(
        ...
        chart_title=f"Stage296 {profile.label} {display_label}",
    )
...
def _run_suite() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for window_name, display_label, analysis_start, analysis_end in WINDOWS:
        for profile in PROFILES:
            print(f"[stage296] {window_name} {profile.name}", flush=True)
            ...
            rows.append(
                {
                    ...
                    "research_pass": int(dd_ok and retention >= 0.65 and float(c["sharpe_ratio"]) >= baseline_sharpe),
                }
            )
...
def _build_report(summary_df: pd.DataFrame, comparison_df: pd.DataFrame) -> str:
    ...
    lines = [
        "# Stage296 第78-1连续亏损风险倍率验证",
        "",
        "## 口径",
        "",
        "- 基于Stage294发现：最大回撤期多次新增开仓发生在 `loss_streak` 2-4。",
        "- 本轮不改信号和品种，只让连续亏损后的下一笔风险更早下降。",
        "",
        "## 全样本候选排序",
        "",
        full_cmp[
    ...
    if strict_count:
        ...
        lines.append("- 未出现通过候选；连续亏损风险倍率可以改善风险质量，但不足以单独把最大回撤压进30以内。")
    return "\n".join(lines) + "\n"
