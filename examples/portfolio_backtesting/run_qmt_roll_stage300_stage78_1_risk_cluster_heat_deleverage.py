...
MODEL_TAG = "stage300_stage78_1_risk_cluster_heat_deleverage_v1"
OUTPUT_PREFIX = "qmt_roll_stage300_stage78_1_risk_cluster_heat_deleverage"
...
def _heat_deleverage_overrides(
    *,
    target_clusters: str = "",
    layer_kinds: str = "add,donchian",
    ...
    return {
        ...
        "risk_cluster_heat_deleverage_layer_kinds": layer_kinds,
...
PROFILES: tuple[Profile, ...] = (
    ...
    Profile(
        "C_delev_all_add_soft",
        "全风险簇加仓层热度降暴露",
        _heat_deleverage_overrides(min_pressure=0.35),
    ),
    Profile(
        "C_delev_energy_add_soft",
        "能化工业加仓层热度降暴露",
        _heat_deleverage_overrides(
...
def _run_profile(
    ...
    _, analysis_df, statistics = run_backtest(
        ...
        chart_title=f"Stage300 {profile.label} {display_label}",
    )
...
def _run_suite() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for window_name, display_label, analysis_start, analysis_end in WINDOWS:
        for profile in PROFILES:
            print(f"[stage300] {window_name} {profile.name}", flush=True)
...
def _build_report(comparison_df: pd.DataFrame) -> str:
    ...
    lines = [
        "# Stage300 第78-1风险簇热度降暴露A/C验证",
        "",
        "## 目标",
        "",
        "- A：第78-1正式基准。",
        "- C：只增加风险簇热度降暴露模块，不修改入场、AI池和基础趋势仓。",
        "- 降暴露只处理已有 `add/donchian` 加仓层，保留基础趋势仓。",
    ...
    lines.append("- 如果降已有加仓层仍压不动全样本回撤，说明回撤主要来自基础趋势仓路径或趋势失效期。")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(f"[stage300] summary={summary_path}")
    print(f"[stage300] comparison={comparison_path}")
    print(f"[stage300] report={report_path}")
    print(f"[stage300] decision={decision_path}")
