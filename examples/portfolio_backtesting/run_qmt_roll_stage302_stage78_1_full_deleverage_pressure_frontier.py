...
MODEL_TAG = "stage302_stage78_1_full_deleverage_pressure_frontier_v1"
OUTPUT_PREFIX = "qmt_roll_stage302_stage78_1_full_deleverage_pressure_frontier"
...
PROFILES: tuple[Profile, ...] = (
    ...
    Profile(
        "C_full_delev_pressure025",
        "全层热度降暴露-压力阈值0.25",
        _full_deleverage_overrides(min_pressure=0.25),
    ),
    Profile(
        "C_full_delev_pressure030",
        "全层热度降暴露-压力阈值0.30",
        _full_deleverage_overrides(min_pressure=0.30),
    ),
    Profile(
        "C_full_delev_pressure035",
        "全层热度降暴露-压力阈值0.35",
        _full_deleverage_overrides(min_pressure=0.35),
    ),
    Profile(
        "C_full_delev_pressure040",
        "全层热度降暴露-压力阈值0.40",
        _full_deleverage_overrides(min_pressure=0.40),
    ),
    Profile(
        "C_full_delev_pressure045",
        "全层热度降暴露-压力阈值0.45",
        _full_deleverage_overrides(min_pressure=0.45),
    ),
)
...
def _run_profile(
    ...
    _, analysis_df, statistics = run_backtest(
        ...
        chart_title=f"Stage302 {profile.label} {display_label}",
    )
...
def _run_suite() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for window_name, display_label, analysis_start, analysis_end in WINDOWS:
        for profile in PROFILES:
            print(f"[stage302] {window_name} {profile.name}", flush=True)
...
def _build_report(comparison_df: pd.DataFrame) -> str:
    ...
    pass_count = int(full_cmp["strict_pass"].sum() + full_cmp["research_pass"].sum()) if not full_cmp.empty else 0
    lines = [
        "# Stage302 第78-1全层热度降暴露压力阈值前沿扫描",
        "",
        "## 目标",
        "",
        "- A：第78-1正式基准。",
        "- C：只改变热度压力触发阈值；其它热度定义保持 Stage301 最强线索口径。",
        "- 这是粗粒度前沿扫描，不作为默认实盘建议。",
    ...
    if pass_count:
        lines.append("- 至少一个全样本候选达到回撤30以内，下一步必须做起始年份、季度冷启动、弱窗口和滑点压力。")
        lines.append("- 若通过点只集中在单一阈值，不能直接认定可用，必须视为疑似窄参数。")
    else:
        lines.append("- 全样本没有候选同时满足回撤30以内和收益保留要求。")
        lines.append("- 如果粗粒度前沿仍过不了30，单策略内部继续扫小数阈值的意义下降，应转向资金分层或组合层风险预算。")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(f"[stage302] summary={summary_path}")
    print(f"[stage302] comparison={comparison_path}")
    print(f"[stage302] report={report_path}")
    print(f"[stage302] decision={decision_path}")
