...
MODEL_TAG = "stage311_stage78_1_drawdown_deleverage_engine_validation_v1"
OUTPUT_PREFIX = "qmt_roll_stage311_stage78_1_drawdown_deleverage_engine_validation"
...
def _drawdown_gate_base() -> dict[str, Any]:
    return {
...
def _drawdown_deleverage_only() -> dict[str, Any]:
    return {
        **_drawdown_gate_base(),
...
def _drawdown_gate_and_deleverage() -> dict[str, Any]:
    return {
        **_drawdown_deleverage_only(),
        "enable_portfolio_drawdown_gate": True,
        "portfolio_drawdown_gate_entry_contexts": ALL_ENTRY_CONTEXTS,
    }
...
PROFILES: tuple[Profile, ...] = (
    ...
    Profile(
        ...
        "热度降暴露0.40+组合回撤降已有持仓",
        _merge(_pressure040_overrides(), _drawdown_deleverage_only()),
    ),
    Profile(
        "C_pressure040_dd_gate_deleverage",
        "热度降暴露0.40+开仓门禁+组合回撤降已有持仓",
        _merge(_pressure040_overrides(), _drawdown_gate_and_deleverage()),
    ),
)
...
def _run_profile(
    ...
    _, analysis_df, statistics = run_backtest(
        ...
        chart_title=f"Stage311 {profile.label} {display_label}",
    )
...
def _run_suite() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for window_name, display_label, analysis_start, analysis_end in WINDOWS:
        for profile in PROFILES:
            print(f"[stage311] {window_name} {profile.name}", flush=True)
...
def _build_report(comparison_df: pd.DataFrame) -> str:
    ...
    lines = [
        "# Stage311 组合回撤降仓真实引擎验证",
        "",
        "## 目标",
        "",
        "- A：78-1正式基准。",
        "- C：沿用10%-30%-85%动态回撤门禁，但把门禁落到已有持仓降仓路径，避免只在日权益曲线上好看。",
        "- 本阶段不改alpha、AI池、品种池，也不做小数扫参。",
        "",
        "## 全样本对比",
        "",
        full_cmp[
