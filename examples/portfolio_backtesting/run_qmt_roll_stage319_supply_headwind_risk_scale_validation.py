...
MODEL_TAG = "stage319_supply_headwind_risk_scale_validation_v1"
OUTPUT_PREFIX = "qmt_roll_stage319_supply_headwind_risk_scale_validation"
...
PROFILES: tuple[Profile, ...] = (
    Profile("C_pressure040", "热度降暴露0.40", 1.00, _pressure040_overrides()),
    Profile("C3_supply_headwind", "热度0.40+供需强逆风", 1.00, C3_OVERRIDES),
    Profile("C3_supply_headwind_risk095", "热度0.40+供需强逆风+风险0.95", 0.95, C3_OVERRIDES),
    Profile("C3_supply_headwind_risk090", "热度0.40+供需强逆风+风险0.90", 0.90, C3_OVERRIDES),
)
...
def _run_profile(
    ...
    risk_ratio = BASE_RISK_RATIO * profile.risk_scale
    _, analysis_df, statistics = run_backtest(
        ...
        chart_title=f"Stage319 {profile.label} {display_label}",
    )
...
def _run_suite() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for window_name, display_label, analysis_start, analysis_end in WINDOWS:
        for profile in PROFILES:
            print(f"[stage319] {window_name} {profile.name}", flush=True)
            ...
            summary_rows.append(
                build_summary_row(
                    ...
                    effective_risk_ratio=risk_ratio,
            ...
            rows.append(
                {
                    ...
                    "return_ok_vs_c3_85": int(c3_retention >= 85.0),
                    "strict_pass": int(dd_ok and pressure_retention >= 100.0 and c3_retention >= 85.0),
                    "research_pass": int(dd_ok and pressure_retention >= 95.0 and c3_retention >= 80.0),
                }
            )
...
def _build_report(summary_df: pd.DataFrame, comparison_df: pd.DataFrame) -> str:
    ...
    multi = (
        comparison_df[comparison_df["variant"].str.contains("risk", regex=False)]
    ...
    lines = [
        "# Stage319 供需过滤叠加统一风险预算缩放",
        "",
        "## 目标",
        "",
        "- 延续 Stage318 的 C3：`C_pressure040 + 供需强逆风过滤`。",
        "- 只测试账户级风险预算缩放 `0.95/0.90`，不改入场、不改供需阈值、不做品种黑名单。",
        "- 严格通过：全样本最大回撤进入30%以内，收益不低于 `C_pressure040`，且保留 C3 至少85%收益。",
        "",
        "## 全样本对比",
        "",
        to_markdown_table(
    ...
    full_candidates = full_cmp[full_cmp["variant"].str.contains("risk", regex=False)]
    passed = full_candidates[full_candidates["strict_pass"].eq(1)]
    if not passed.empty:
        best = passed.iloc[0]
        lines.append(
            f"- `{best['variant']}` 全样本严格通过；下一步必须做起始年份、季度冷启动和滑点压力，暂不直接替代78-1。"
        )
    else:
        lines.append("- 风险缩放候选未在全样本严格通过；不进入后续稳健性。")
    return "\n".join(lines) + "\n"
        ...
        "profiles": [
            {
                ...
                "label": profile.label,
                "risk_scale": profile.risk_scale,
                "effective_risk_ratio": BASE_RISK_RATIO * profile.risk_scale,
