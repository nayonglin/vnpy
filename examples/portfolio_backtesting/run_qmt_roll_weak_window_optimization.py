...
EXPERIMENTS: list[dict[str, Any]] = [
    {
        "label": "baseline_cap070",
        "description": "当前默认配置基线",
        "strategy_overrides": {},
    },
    {
        "label": "angle_reversal_on",
        "description": "打开 MA5 角度反转过滤",
        "strategy_overrides": {
    ...
    {
        "label": "wick_tight_4",
        "description": "收紧 wick chop 过滤阈值到 4 天",
        "strategy_overrides": {
    ...
    {
        "label": "wick_tight_3",
        "description": "收紧 wick chop 过滤阈值到 3 天",
        "strategy_overrides": {
            "wick_chop_filter_max_days": 3,
        },
    },
    {
        "label": "alignment_break_on",
        "description": "打开排列破坏即退出",
        "strategy_overrides": {
            "exit_on_alignment_break": True,
        },
    },
    {
        "label": "streak_defensive",
        "description": "连败后更激进降风险",
        "strategy_overrides": {
    ...
    {
        "label": "concurrent_3",
        "description": "最大并发持仓从 4 降到 3",
        "strategy_overrides": {
            "max_concurrent_positions": 3,
        },
    },
    {
        "label": "portfolio_cap_0p7",
        "description": "组合总资金使用上限从 0.9 降到 0.7",
        "strategy_overrides": {
            "max_capital_usage_ratio": 0.7,
        },
    },
    {
        "label": "long_only_reference",
        "description": "诊断参考：关闭全部新开空",
        "strategy_overrides": {
            "short_entry_enabled": False,
        },
    },
    {
        "label": "combo_defensive",
        "description": "角度过滤 + 排列破坏退出 + 连败降风险 + 并发降到 3",
        "strategy_overrides": {
            "ma5_angle_reversal_filter_enabled": True,
            "exit_on_alignment_break": True,
            "streak_risk_multipliers": "1.0,1.0,0.5,0.25",
...
def run_experiments() -> pd.DataFrame:
    ...
    for index, experiment in enumerate(EXPERIMENTS, start=1):
        ...
        print(f"[weak-window-opt] {index}/{len(EXPERIMENTS)} {label}: {description}")
        _, _, statistics = run_backtest(
            ...
            file_prefix=f"qmt_roll_weak_opt_{label}",
            chart_title=f"QMT Roll Weak Window Optimization {label}",
        )
...
def main() -> None:
    ...
    summary_path: Path = (OUTPUT_DIR / "qmt_roll_weak_window_optimization_summary.csv").resolve()
    result_df.to_csv(summary_path, index=False, encoding="utf-8-sig")
    print(f"[weak-window-opt] summary csv: {summary_path}")
    print("[weak-window-opt] full ranking:")
