...
SAMPLES_PATH: Path = OUTPUT_DIR / "qmt_roll_ai_candidate_pairwise_horizon_samples.csv"

MODEL_TAG: str = "pairwise_horizon_cls_v1"
MODEL_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_pairwise_horizon_classifier_{MODEL_TAG}.joblib"
SUMMARY_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_pairwise_horizon_classifier_summary_{MODEL_TAG}.json"
COEFFICIENT_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_pairwise_horizon_classifier_coefficients_{MODEL_TAG}.csv"
PREDICTIONS_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_pairwise_horizon_classifier_predictions_{MODEL_TAG}.csv"
BUCKET_ANALYSIS_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_pairwise_horizon_classifier_bucket_analysis_{MODEL_TAG}.csv"
...
def build_bucket_analysis(df: pd.DataFrame, probability_column: str) -> pd.DataFrame:
    ...
    summary = (
        ...
        .agg(
            ...
            support_5d_rate=("label_horizon_5d_support_primary", "mean"),
    ...
    summary["support_5d_rate"] = summary["support_5d_rate"] * 100.0
...
def main() -> None:
    ...
    summary = {
        ...
        "model_judgement": {
            "why_horizon_label": "主标签直接使用 10d/20d horizon 结果，不再依赖 quality_score 聚合值。",
            "why_linear_baseline": "先验证标签本身能否提供稳定方向性，再决定是否值得用更复杂模型。",
        },
    }
    ...
    print(f"[ai-candidate-pairwise-horizon-classifier] model: {MODEL_OUTPUT_PATH}")
    print(f"[ai-candidate-pairwise-horizon-classifier] summary: {SUMMARY_OUTPUT_PATH}")
    print(f"[ai-candidate-pairwise-horizon-classifier] coefficients: {COEFFICIENT_OUTPUT_PATH}")
    print(f"[ai-candidate-pairwise-horizon-classifier] predictions: {PREDICTIONS_OUTPUT_PATH}")
    print(f"[ai-candidate-pairwise-horizon-classifier] bucket analysis: {BUCKET_ANALYSIS_OUTPUT_PATH}")
