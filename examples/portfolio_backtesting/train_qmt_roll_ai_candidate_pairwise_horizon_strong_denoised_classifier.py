...
SAMPLES_PATH: Path = OUTPUT_DIR / "qmt_roll_ai_candidate_pairwise_horizon_strong_denoised_samples.csv"

MODEL_TAG: str = "pairwise_horizon_cls_v2_strong_denoised"
MODEL_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_pairwise_horizon_strong_denoised_classifier_{MODEL_TAG}.joblib"
SUMMARY_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_pairwise_horizon_strong_denoised_classifier_summary_{MODEL_TAG}.json"
COEFFICIENT_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_pairwise_horizon_strong_denoised_classifier_coefficients_{MODEL_TAG}.csv"
PREDICTIONS_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_pairwise_horizon_strong_denoised_classifier_predictions_{MODEL_TAG}.csv"
BUCKET_ANALYSIS_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_pairwise_horizon_strong_denoised_classifier_bucket_analysis_{MODEL_TAG}.csv"
...
def build_bucket_analysis(df: pd.DataFrame, probability_column: str) -> pd.DataFrame:
    ...
    summary = (
        ...
        .agg(
            ...
            avg_oi_noise=("abs_delta_feature_oi_delta_1d_pct_zscore_120", "mean"),
...
def main() -> None:
    ...
    summary = {
        ...
        "model_judgement": {
            "why_strong_denoise": "只移除 strong 组中 OI 冲击过大的 pair，避免把 medium 组刚出现的正向信号一并误伤。",
            "why_linear_baseline": "继续固定低自由度模型，确保本轮比较的变化主要来自标签和样本去噪，而不是模型复杂度。",
        },
    }
    ...
    prediction_columns = [
        ...
        "abs_delta_feature_oi_delta_1d_pct_zscore_120",
    ...
    print(f"[ai-candidate-pairwise-horizon-strong-denoised-classifier] model: {MODEL_OUTPUT_PATH}")
    print(f"[ai-candidate-pairwise-horizon-strong-denoised-classifier] summary: {SUMMARY_OUTPUT_PATH}")
    print(f"[ai-candidate-pairwise-horizon-strong-denoised-classifier] coefficients: {COEFFICIENT_OUTPUT_PATH}")
    print(f"[ai-candidate-pairwise-horizon-strong-denoised-classifier] predictions: {PREDICTIONS_OUTPUT_PATH}")
    print(f"[ai-candidate-pairwise-horizon-strong-denoised-classifier] bucket analysis: {BUCKET_ANALYSIS_OUTPUT_PATH}")
