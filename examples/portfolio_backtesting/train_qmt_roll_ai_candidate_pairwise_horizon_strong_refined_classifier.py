...
MODEL_TAG: str = "pairwise_horizon_cls_v3_strong_refined"
MODEL_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_pairwise_horizon_strong_refined_classifier_{MODEL_TAG}.joblib"
SUMMARY_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_pairwise_horizon_strong_refined_classifier_summary_{MODEL_TAG}.json"
COEFFICIENT_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_pairwise_horizon_strong_refined_classifier_coefficients_{MODEL_TAG}.csv"
PREDICTIONS_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_pairwise_horizon_strong_refined_classifier_predictions_{MODEL_TAG}.csv"
BUCKET_ANALYSIS_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_pairwise_horizon_strong_refined_classifier_bucket_analysis_{MODEL_TAG}.csv"
...
def main() -> None:
    ...
    subtype_metrics = {
        "train": compute_subtype_metrics(
            scored_df[scored_df["dataset_split"] == "train"], "predicted_left_win_probability"
        ),
        "valid": compute_subtype_metrics(
            scored_df[scored_df["dataset_split"] == "valid"], "predicted_left_win_probability"
        ),
        "test": compute_subtype_metrics(
            scored_df[scored_df["dataset_split"] == "test"], "predicted_left_win_probability"
        ),
    }
    ...
    summary = {
        ...
        "model_judgement": {
            "why_strong_refinement": "把 strong 从单一强度桶细分成结构原型，有助于区分拥挤噪声和趋势延续。",
            "why_linear_baseline": "继续固定低自由度模型，确保收益主要来自标签层次重构，而不是模型变化。",
        },
    }
    ...
    prediction_columns = [
        ...
        "abs_delta_feature_trend_ma20_gap_pct",
        "abs_delta_feature_ret_20d_zscore_120",
    ...
    print(f"[ai-candidate-pairwise-horizon-strong-refined-classifier] model: {MODEL_OUTPUT_PATH}")
    print(f"[ai-candidate-pairwise-horizon-strong-refined-classifier] summary: {SUMMARY_OUTPUT_PATH}")
    print(f"[ai-candidate-pairwise-horizon-strong-refined-classifier] coefficients: {COEFFICIENT_OUTPUT_PATH}")
    print(f"[ai-candidate-pairwise-horizon-strong-refined-classifier] predictions: {PREDICTIONS_OUTPUT_PATH}")
    print(f"[ai-candidate-pairwise-horizon-strong-refined-classifier] bucket analysis: {BUCKET_ANALYSIS_OUTPUT_PATH}")
    print(json.dumps(split_metrics, ensure_ascii=False, indent=2))
    print(json.dumps(strength_metrics, ensure_ascii=False, indent=2))
    print(json.dumps(subtype_metrics, ensure_ascii=False, indent=2))
    if not bucket_df.empty:
