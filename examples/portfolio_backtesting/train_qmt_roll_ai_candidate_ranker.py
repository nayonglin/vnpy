...
def compute_cross_section_metrics(df: pd.DataFrame, prediction_column: str) -> dict[str, Any]:
    ...
    for _, group_df in df.groupby(GROUP_COLUMN, sort=False):
        ...
        top1_target = float(actual_target.loc[predicted_top_index])
        group_target = float(actual_target.mean())
        top1_quality = float(group_df.loc[predicted_top_index, QUALITY_COLUMN])
        group_quality = float(group_df[QUALITY_COLUMN].mean())
        top1_selected = float(group_df.loc[predicted_top_index, SELECTED_COLUMN])
        group_selected = float(group_df[SELECTED_COLUMN].mean())

        top1_targets.append(top1_target)
        group_avg_targets.append(group_target)
...
def main() -> None:
    ...
    print(f"[ai-candidate-ranker] model: {MODEL_OUTPUT_PATH}")
    print(f"[ai-candidate-ranker] summary: {SUMMARY_OUTPUT_PATH}")
    print(f"[ai-candidate-ranker] feature importance: {FEATURE_IMPORTANCE_OUTPUT_PATH}")
    print(f"[ai-candidate-ranker] predictions: {PREDICTIONS_OUTPUT_PATH}")
    print(f"[ai-candidate-ranker] bucket analysis: {BUCKET_ANALYSIS_OUTPUT_PATH}")
