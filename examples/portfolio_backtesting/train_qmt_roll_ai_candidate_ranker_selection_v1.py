...
from qmt_roll_ai_candidate_selection_label_v1 import (
    QUALITY_COLUMN_V1,
    QUALITY_RANK_CENTERED_COLUMN_V1,
    add_selection_quality_v1_labels,
)
...
TARGET_COLUMN: str = QUALITY_RANK_CENTERED_COLUMN_V1
QUALITY_COLUMN: str = QUALITY_COLUMN_V1
...
def main() -> None:
    ...
    df = load_samples()
    df = add_selection_quality_v1_labels(df)
    ...
    summary = {
        ...
        "label_notes": {
            "goal": "selection_rights_label_v1",
            "target_column": TARGET_COLUMN,
            "quality_column": QUALITY_COLUMN,
            "changes": [
                "keep future return core",
                "add 20d tail bonus to break high-end ties",
                "penalize high MAE and extreme volatility/trend tails",
            ],
        },
    }
    ...
    print(f"[ai-candidate-ranker-selection-v1] model: {MODEL_OUTPUT_PATH}")
    print(f"[ai-candidate-ranker-selection-v1] summary: {SUMMARY_OUTPUT_PATH}")
    print(f"[ai-candidate-ranker-selection-v1] relabeled samples: {RELABEL_OUTPUT_PATH}")
    print(f"[ai-candidate-ranker-selection-v1] feature importance: {FEATURE_IMPORTANCE_OUTPUT_PATH}")
    print(f"[ai-candidate-ranker-selection-v1] predictions: {PREDICTIONS_OUTPUT_PATH}")
    print(f"[ai-candidate-ranker-selection-v1] bucket analysis: {BUCKET_ANALYSIS_OUTPUT_PATH}")
