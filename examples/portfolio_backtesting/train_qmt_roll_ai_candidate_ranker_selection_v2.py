...
from qmt_roll_ai_candidate_selection_label_v2 import (
    QUALITY_COLUMN_V2,
    QUALITY_RANK_CENTERED_COLUMN_V2,
    SAMPLE_WEIGHT_COLUMN_V2,
    add_selection_quality_v2_labels,
)
...
MODEL_TAG: str = "selection_v2"
MODEL_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_ranker_{MODEL_TAG}.joblib"
SUMMARY_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_ranker_summary_{MODEL_TAG}.json"
FEATURE_IMPORTANCE_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_ranker_feature_importance_{MODEL_TAG}.csv"
PREDICTIONS_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_ranker_predictions_{MODEL_TAG}.csv"
BUCKET_ANALYSIS_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_ranker_bucket_analysis_{MODEL_TAG}.csv"
RELABEL_OUTPUT_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_training_samples_{MODEL_TAG}.csv"

TARGET_COLUMN: str = QUALITY_RANK_CENTERED_COLUMN_V2
QUALITY_COLUMN: str = QUALITY_COLUMN_V2
SAMPLE_WEIGHT_COLUMN: str = SAMPLE_WEIGHT_COLUMN_V2
...
def build_feature_columns(df: pd.DataFrame) -> tuple[list[str], list[str]]:
    ...
    cs_feature_columns = [
        ...
        if column.startswith("feature_") and column.endswith(CS_FEATURE_SUFFIXES)
    ]
    numeric_columns = [column for column in BASE_NUMERIC_COLUMNS if column in df.columns]
    numeric_columns.extend(cs_feature_columns)
    numeric_columns = list(dict.fromkeys(numeric_columns))
    return categorical_columns + numeric_columns, categorical_columns
...
def train_model(
    ...
    weight_train = train_df[SAMPLE_WEIGHT_COLUMN].astype("float64")
    ...
    weight_valid = valid_df[SAMPLE_WEIGHT_COLUMN].astype("float64")
    ...
    if not valid_df.empty:
        ...
        fit_kwargs["eval_sample_weight"] = [weight_valid]
...
def compute_cross_section_metrics(df: pd.DataFrame, prediction_column: str) -> dict[str, Any]:
    if df.empty:
        return {
            ...
            "top1_avg_sample_weight": 0.0,
            "group_avg_sample_weight": 0.0,
        }
    ...
    for _, group_df in df.groupby(GROUP_COLUMN, sort=False):
        ...
        actual_top_index = actual_target.idxmax()
        ...
        top1_quality_values.append(float(group_df.loc[predicted_top_index, QUALITY_COLUMN]))
        group_quality_values.append(float(group_df[QUALITY_COLUMN].mean()))
        ...
        top1_weight_values.append(float(group_df.loc[predicted_top_index, SAMPLE_WEIGHT_COLUMN]))
        group_weight_values.append(float(group_df[SAMPLE_WEIGHT_COLUMN].mean()))
        group_count += 1
    ...
    top1_avg_weight = float(np.mean(top1_weight_values)) if top1_weight_values else 0.0
    group_avg_weight = float(np.mean(group_weight_values)) if group_weight_values else 0.0
    ...
    return {
        ...
        "top1_avg_sample_weight": top1_avg_weight,
        "group_avg_sample_weight": group_avg_weight,
    }
...
def build_bucket_analysis(df: pd.DataFrame, prediction_column: str) -> pd.DataFrame:
    ...
    summary = (
        ...
        .agg(
            ...
            avg_sample_weight=(SAMPLE_WEIGHT_COLUMN, "mean"),
...
def main() -> None:
    ...
    summary = {
        ...
        "label_notes": {
            "goal": "selection_rights_label_v2",
            "target_column": TARGET_COLUMN,
            "quality_column": QUALITY_COLUMN,
            "sample_weight_column": SAMPLE_WEIGHT_COLUMN,
            "changes": [
                "keep future return core and 20d tail bonus in label",
                "keep mae penalty in label",
                "move extreme volatility/trend penalties to sample weights",
            ],
        },
    }
    ...
    scored_df.to_csv(RELABEL_OUTPUT_PATH, index=False, encoding="utf-8-sig")

    if not bucket_df.empty:
        bucket_df.to_csv(BUCKET_ANALYSIS_OUTPUT_PATH, index=False, encoding="utf-8-sig")

    print(f"[ai-candidate-ranker-selection-v2] model: {MODEL_OUTPUT_PATH}")
    print(f"[ai-candidate-ranker-selection-v2] summary: {SUMMARY_OUTPUT_PATH}")
    print(f"[ai-candidate-ranker-selection-v2] relabeled samples: {RELABEL_OUTPUT_PATH}")
    print(f"[ai-candidate-ranker-selection-v2] feature importance: {FEATURE_IMPORTANCE_OUTPUT_PATH}")
    print(f"[ai-candidate-ranker-selection-v2] predictions: {PREDICTIONS_OUTPUT_PATH}")
    print(f"[ai-candidate-ranker-selection-v2] bucket analysis: {BUCKET_ANALYSIS_OUTPUT_PATH}")
