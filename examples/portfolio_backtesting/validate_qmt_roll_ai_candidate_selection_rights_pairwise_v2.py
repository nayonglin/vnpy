...
from build_qmt_roll_ai_candidate_selection_pairwise_samples_v2 import build_pair_row_v2
...
def predict_daily_scores(group_df: pd.DataFrame, model: Any, feature_columns: list[str]) -> pd.DataFrame:
    ...
    for pair_index, (idx_a, idx_b) in enumerate(combinations(range(len(ordered_rows)), 2)):
        ...
        pair_rows.append(build_pair_row_v2(f"score__{pair_index:03d}", left_row, right_row))
...
def main() -> None:
    ...
    print(f"[selection-rights-pairwise-v2] summary json: {SELECTION_SUMMARY_PATH}")
    print(f"[selection-rights-pairwise-v2] window csv: {WINDOW_SUMMARY_CSV_PATH}")
    print(f"[selection-rights-pairwise-v2] day csv: {DAY_DETAIL_CSV_PATH}")
