...
from qmt_roll_ai_candidate_selection_label_v1 import QUALITY_COLUMN_V1, add_selection_quality_v1_labels
...
MODEL_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_ranker_{MODEL_TAG}.joblib"
MODEL_SUMMARY_PATH: Path = OUTPUT_DIR / f"qmt_roll_ai_candidate_ranker_summary_{MODEL_TAG}.json"
...
PREDICTION_COLUMN: str = "predicted_rank_score"
...
def load_model_bundle() -> tuple[Any, list[str], list[str]]:
...
def score_samples(df: pd.DataFrame, model: Any, feature_columns: list[str], categorical_columns: list[str]) -> pd.DataFrame:
    scored_df = df.copy()
    x = prepare_lightgbm_frames(scored_df, feature_columns, categorical_columns)
    scored_df[PREDICTION_COLUMN] = np.asarray(model.predict(x), dtype="float64")
    return scored_df
...
def filter_rerankable_pool(df: pd.DataFrame) -> pd.DataFrame:
    ...
    pool_df = pool_df[(selected_count >= 1) & (selected_count < candidate_count)].copy()
...
def evaluate_window(scored_df: pd.DataFrame, window: EvaluationWindow) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    day_rows = [evaluate_group(group_df) for _, group_df in window_df.groupby(GROUP_COLUMN, sort=False)]
...
def main() -> None:
    ...
    model, feature_columns, categorical_columns = load_model_bundle()
    scored_df = score_samples(samples_df, model, feature_columns, categorical_columns)
    scored_df = filter_rerankable_pool(scored_df)
    ...
    print(f"[selection-rights-v1] summary json: {SELECTION_SUMMARY_PATH}")
    print(f"[selection-rights-v1] window csv: {WINDOW_SUMMARY_CSV_PATH}")
    print(f"[selection-rights-v1] day csv: {DAY_DETAIL_CSV_PATH}")
