...
def main() -> None:
    ...
    fig = plt.figure(figsize=(14, 17), dpi=180)
    gs = fig.add_gridspec(4, 1, height_ratios=[3.2, 3.0, 2.35, 1.35], hspace=0.42)
    fig.suptitle(
        "C9当前重建版15万：2018起逐年冷启动到2026-06-30",
        fontsize=23,
        fontweight="bold",
        x=0.5,
        y=0.985,
    ...
    fig.text(
        0.5,
        0.962,
        "official_live_stage847_c9_15w；每个起点独立重跑；Stage182月更AI池；订单API=0",
        ha="center",
        va="top",
        fontsize=11,
        color="#4a5568",
    ...
    for idx, start in enumerate(starts):
        ...
        linewidth = 2.4 if start in {"2018-01", "2022-01", "2026-01"} else 1.45
        alpha = 0.95 if linewidth > 2 else 0.72
        ax1.plot(part["date"], part["nav"], label=start, color=color, linewidth=linewidth, alpha=alpha)
    ax1.set_yscale("log")
    ax1.set_title("绝对日历净值曲线（log，统一请求结束日）", fontsize=15, loc="left", pad=12)
    ax1.set_ylabel("归一净值")
    ax1.grid(color="#d9dee7", linewidth=0.7, alpha=0.75)
    ax1.legend(loc="upper left", ncols=5, fontsize=9, frameon=False)
    ...
    for idx, start in enumerate(starts):
        part = curves[curves["requested_start_month"].astype(str).eq(start)].copy()
        color = _start_color(idx, len(starts))
        linewidth = 2.4 if start in {"2018-01", "2022-01", "2026-01"} else 1.35
        alpha = 0.95 if linewidth > 2 else 0.72
    ...
    bars_ret = ax3.bar(x - width / 2, returns, width, color=return_colors, alpha=0.9, label="期末收益")
