...
MODEL_TAG = "stage506_next_real_forward_risk_signal_frontier_v1"
OUTPUT_PREFIX = "qmt_roll_stage506_next_real_forward_risk_signal_frontier"
...
MAX_ITERATIONS = 3
RAW_ROOT = PROJECT_DIR / "downloaded_futures" / "tqsdk_stage506_next_real_forward_risk_signal_frontier"
...
CONFIGS: tuple[SignalConfig, ...] = (
    ...
    SignalConfig(
        "stage079_next_real_r070_corr20_f50",
        "Stage079 next-real r0.70 same-dir corr gate",
        0.7,
        "same_direction_correlation_gate",
        {
    ...
    SignalConfig(
        "stage079_next_real_r080_corr20_f50",
        "Stage079 next-real r0.80 same-dir corr gate",
        0.8,
        "same_direction_correlation_gate",
        {
            "enable_same_direction_correlation_gate": True,
    ...
    SignalConfig(
        ...
        "Stage079 next-real r0.80 vol budget 20d target60 floor50 entry",
        0.8,
        "portfolio_volatility_budget_entry_only",
        {
            ...
            "enable_portfolio_volatility_budget_deleverage": False,
        },
    ),
    SignalConfig(
        "stage079_next_real_r080_vol60_t60_min50_entry",
        "Stage079 next-real r0.80 vol budget 60d target60 floor50 entry",
        0.8,
        "portfolio_volatility_budget_entry_only",
        {
            "enable_portfolio_volatility_budget": True,
    ...
    SignalConfig(
        "stage079_next_real_r080_corr20_vol20_entry",
        "Stage079 next-real r0.80 corr gate + vol20 entry",
        0.8,
        "corr_plus_vol_entry_only",
        {
            "enable_same_direction_correlation_gate": True,
            "same_direction_correlation_gate_lookback": 20,
            "same_direction_correlation_gate_start": 0.60,
...
def _patch_stage506_raw_roots() -> None:
    s504.RAW_ROOT = RAW_ROOT
    s504._patch_raw_roots()


def _run_config(config: SignalConfig) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    setting.update(config.overrides)
    ...
    daily["signal_family"] = config.signal_family
    ...
    if not usage.empty:
        ...
        usage["signal_family"] = config.signal_family
    return daily, usage
...
def _frontier(summary: pd.DataFrame, usage: pd.DataFrame) -> pd.DataFrame:
    ...
    frame["signal_family"] = frame["variant"].map(lambda value: config_map[str(value)].signal_family)
    ...
    ].sort_values(["fallback_trade_count", "dd40_pass", "return65_pass", "total_return_pct"], ascending=[True, False, False, False])
...
def _plot(long_daily: pd.DataFrame, frontier: pd.DataFrame) -> None:
    ...
    clean_dd40_return65 = frontier[
        frontier["fallback_trade_count"].eq(0)
        & frontier["dd40_pass"].eq(1)
        & frontier["return65_pass"].eq(1)
    ]["variant"].tolist()
    for variant in clean_dd40_return65[:3]:
        if variant not in keep:
            keep.append(variant)
    if len(keep) <= 3:
        for variant in frontier[frontier["fallback_trade_count"].eq(0)]["variant"].head(4).tolist():
            if variant not in keep:
                keep.append(variant)
    ...
    axes[0].set_title("Stage506 next-real forward risk-signal frontier")
...
def _write_report(
    ...
    report = [
        ...
        "- 阶段性质：真实可成交前置风险信号粗前沿；不改 Stage079/C3 信号和品种池。",
        "- A/B/C判断：A 为 Stage079 原始日线 baseline；B 无独立意义；C 为 Stage079 下一真实窗口 + 固定风险预算 + 前置风险信号。",
        "",
        "## 外部调研判断",
        "",
        "- CTA/趋势跟随常用波动缩放和相关性/拥挤度控制；但此前仓库已经反证亏损后降仓和多次失败信号冷却，所以本阶段只测前置入场/加仓侧粗结构。",
        "- 本阶段使用上一日已形成的权益波动、当前同方向持仓相关性和当前候选历史数据，不使用未来收益。",
        "",
        "## 决策",
        "",
        ...
        "## 成交价格来源",
        "",
        _md_table(source_counts.sort_values(["variant", "trade_count"], ascending=[True, False]), max_rows=100),
        "",
        "## 补数状态",
        "",
        _md_table(backfill_status.sort_values(["iteration", "vt_symbol"]), max_rows=100),
        "",
        "## 剩余 fallback 审计",
        "",
        _md_table(fallback_audit.sort_values(["variant", "signal_date", "vt_symbol"]), max_rows=100),
        "",
        "## 图表视觉复盘",
        "",
        "- 需要确认前置信号是否真的把 2021-2022 深水抬上来，还是只是降低整体复利斜率。",
        "- 若候选 NAV 长期低于 `risk060_clean`，即便最大回撤略好，也不能视为收益保留解法。",
        "- 若候选水下仍在 2022 初或 2025 二次水下穿过 `-40%`，说明前置信号没有识别核心风险簇。",
        "",
        "## 过拟合与继续价值反思",
        "",
        "- 运行前过拟合反思：否。所有参数为既有粗档，未按坏窗口或品种过滤。",
        "- 运行后过拟合反思：以决策标签为准；若失败，不继续扫相关阈值或波动目标小数。",
        "- 运行前继续价值反思：是。Stage205 已显示滞后门控失败，前置信号是下一条低自由度路径。",
        "- 运行后继续价值反思：若无 clean DD40+65 候选，继续该形状价值低，应换更独立的收益源或更前置的数据。",
    ]
    REPORT_PATH.write_text("\n".join(report), encoding="utf-8")


def main() -> None:
    _patch_stage506_raw_roots()
    ...
    clean_dd40_return65 = clean_dd40[clean_dd40["return65_pass"].eq(1)].copy()
    if not clean_dd40_return65.empty:
        best = clean_dd40_return65.sort_values("total_return_pct", ascending=False).iloc[0]
        decision_label = "forward_risk_signal_clean_dd40_return65_candidate_needs_audit"
    elif not clean_dd40.empty:
        best = clean_dd40.sort_values("total_return_pct", ascending=False).iloc[0]
        decision_label = "forward_risk_signal_clean_dd40_but_return_retention_short"
    else:
        best = frontier.sort_values("total_return_pct", ascending=False).iloc[0]
        decision_label = "forward_risk_signal_no_clean_dd40_candidate"
    ...
    decision = {
        ...
        "next_step": "若出现 clean DD40+65 候选，做成本、多周期和视觉复核；若无，停止该前置风险信号形状。",
    }
