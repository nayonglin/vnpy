...
STAGE = "Stage008"
MODEL_TAG = "stage008_no_follow_reduce_true_engine_v1"
OUTPUT_PREFIX = "qmt_roll_stage008_c9_minrisk_no_follow_reduce_true_engine"
...
OUTPUT_DIR = LINE_DIR / "outputs" / "stage008_no_follow_reduce_true_engine"

A_ARM = "A_official_stage847_c9_15w"
C_ARM = "C_stage008_no_follow_30m_reduce_to_half"
...
NO_FOLLOW_EVENTS_OUT = OUTPUT_DIR / f"{OUTPUT_PREFIX}_no_follow_reduce_events_{MODEL_TAG}.csv"
...
class QmtRollPortfolioStrategyStage008NoFollowReduce(s847.QmtRollPortfolioStrategyStage847C9StopRetry):
    enable_stage008_no_follow_reduce: bool = False
    stage008_reduce_fraction: float = REDUCE_FRACTION
    stage008_window_minutes: int = WINDOW_MINUTES

    parameters = s847.QmtRollPortfolioStrategyStage847C9StopRetry.parameters + [
        "enable_stage008_no_follow_reduce",
        "stage008_reduce_fraction",
        "stage008_window_minutes",
    ]
    variables = s847.QmtRollPortfolioStrategyStage847C9StopRetry.variables + [
        "stage008_no_follow_reduce_count",
    ]

    def __init__(self, strategy_engine, strategy_name: str, vt_symbols: list[str], setting: dict) -> None:
        super().__init__(strategy_engine, strategy_name, vt_symbols, setting)
        self.stage008_no_follow_reduce_events: list[dict[str, Any]] = []
        self.stage008_no_follow_reduce_count: int = 0
        # Reuse the Stage002 frame collector without inheriting its trading logic.
        self.stage002_restore_events = self.stage008_no_follow_reduce_events
    ...
    def stage827_intraday_exit_after_open_trade(self, trade: s827.TradeData) -> dict[str, Any] | None:
        if self.enable_stage847_half_r_stop_retry:
            ...
            if event:
                return event
        if self.enable_stage008_no_follow_reduce:
            event = self._stage008_no_follow_reduce_after_open_trade(trade)
            if event:
                return event
        return super(s847.QmtRollPortfolioStrategyStage847C9StopRetry, self).stage827_intraday_exit_after_open_trade(trade)

    def _stage008_no_follow_reduce_after_open_trade(self, trade: s827.TradeData) -> dict[str, Any] | None:
        ...
        window_minutes = max(1, int(self.stage008_window_minutes))
        ...
        target_volume = max(1, int(math.floor(current_volume * float(self.stage008_reduce_fraction))))
        ...
        self._record_trade_event(
            ...
            offset="Close",
            reason="stage008_no_follow_30m_reduce_to_half",
        ...
        self.stage008_no_follow_reduce_count += 1
        event = {
            ...
            "final_state": "reduced_to_half",
            "exit_reason": "stage008_no_follow_30m_reduce_to_half",
            "note": "first 30 minute close did not follow trade direction; reduce to floor(50%) min one lot, no hard deletion",
            "synthetic_trades": [
                {
                    "action": "close",
                    "source": "stage008_no_follow_30m_reduce_to_half",
                    "price": reduce_price,
                    "volume": reduce_volume,
                    "time": reduce_time,
                }
        ...
        self.stage008_no_follow_reduce_events.append(event)
        return event
...
def _candidate_profile(metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    capital = replace(
        ...
        label="Stage008 no-follow 30m reduce official C9/15w",
        account_capital=CAPITAL,
        c3_capital=CAPITAL,
        note=(
            f"{spec.capital.note} | Stage008 frozen no-follow risk reduction. "
            "Official C9 opens normally. If the first 30 entry-day minute bars close with non-positive "
            "directional progress, reduce active volume to floor(50%) with a one-lot minimum. Missing "
            "entry-day minute bars are left unchanged. No parameter, product, direction, year or month scan."
        ),
    )
    overrides = {
        **spec.overrides,
        "enable_stage008_no_follow_reduce": True,
        "stage008_reduce_fraction": REDUCE_FRACTION,
        "stage008_window_minutes": WINDOW_MINUTES,
    }
    result = dict(profile)
    result["profile"] = C_ARM
    result["strategy_cls"] = QmtRollPortfolioStrategyStage008NoFollowReduce
    ...
    row.update(
        {
            ...
            "no_follow_reduce_event_count": int(len(no_follow_events)),
            "no_follow_reduce_volume": float(pd.to_numeric(no_follow_events.get("reduce_volume", 0), errors="coerce").fillna(0).sum())
...
def _load_baseline() -> tuple[pd.Series, pd.DataFrame]:
    ...
    row["no_follow_reduce_event_count"] = 0
    row["no_follow_reduce_volume"] = 0.0
...
def _comparison(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    row = {
        ...
        "C_no_follow_reduce_event_count": int(c.get("no_follow_reduce_event_count", 0)),
        "C_no_follow_reduce_volume": float(c.get("no_follow_reduce_volume", 0.0)),
    }
...
def _plot_path(curve: pd.DataFrame) -> None:
    ...
    labels = {
        ...
        C_ARM: "C no-follow 30m reduce-to-half",
    }
    for arm, group in data.groupby("arm"):
        group = group.sort_values("date")
    ...
    axes[0].set_title("Stage008 full-path equity")
...
def _plot_atlas(events: pd.DataFrame, minute_bars: pd.DataFrame) -> tuple[list[Path], pd.DataFrame]:
    ...
    for page in range(1, pages + 1):
        ...
        fig.suptitle("Stage008 no-follow 30m reduce true-engine entry-day minute-K atlas", fontsize=12)
...
def _decision(comparison: pd.DataFrame, cost_stress: pd.DataFrame) -> dict[str, Any]:
    ...
    if retention_pass and dd_pass and broker_pass and sharpe_pass:
        label = "stage008_full_period_pass_next_multistart"
    elif not retention_pass:
        label = "stage008_failed_return_retention_no_param_rescue"
    elif not dd_pass:
        label = "stage008_failed_drawdown_no_param_rescue"
    else:
        label = "stage008_mixed_no_promotion"
    return {
        "stage": STAGE,
        "line_id": LINE_ID,
        "model_tag": MODEL_TAG,
        ...
        "candidate_hypothesis": (
            ...
            "direction, reduce active exposure to a one-lot-minimum half position. This tests no-follow as a negative "
        ...
        "external_research_judgment": (
            "Trend-following references support gradual risk build/reduction as a broad risk-management principle, "
            "while intraday momentum and data-quality references warn that minute rules must be based on live-visible "
            "bars and must not fabricate missing data."
        ),
        "overfit_reflection_before": (
            "No: the rule is a single frozen negative-quality response from Stage007, uses the already documented "
            "30-minute no-follow label and the existing one-lot-minimum half-risk convention, and does not branch by "
            "product, year, direction, month, or final PnL."
        ),
        "continue_value_before": (
            "Yes: Stage007 only proved a closed-lot contribution clue; a true path engine is needed to see whether "
            "partial de-risking preserves the official C9 right tail."
        ),
...
def _write_report(
    ...
    view_cols = [
        ...
        "no_follow_reduce_event_count",
    ]
    lines = [
        "# Stage008 no-follow 30m 降风险真实引擎",
        "",
        f"- 生成时间：`{datetime.now().strftime('%Y-%m-%d %H:%M')}`",
        f"- line_id：`{LINE_ID}`",
        f"- 当前官方正式版本：`{OFFICIAL_LIVE_VERSION}` / `{OFFICIAL_LIVE_ALIAS}`",
        "- A：当前官方 C9/15w 全路径。",
        "- C：C9/15w + `no_follow_30m_reduce_to_half`。",
        ...
        "- 若 C9 自身 `0.5R` stop/retry 已触发，则优先执行 C9，不再叠加 Stage008。",
        "- 若入场日已有分钟K且前 `30` 根分钟K收盘相对入场价的方向性 R `<=0`，把 active volume 降到 `floor(50%)`，最低保留 `1` 手。",
        ...
        "",
        "## No-Follow Reduce Events By Year",
        "",
...
def main() -> None:
    ...
    print("[stage008] loading metadata and minute bars", flush=True)
    ...
    print("[stage008] running candidate true engine", flush=True)
    ...
    if decision["decision"] == "stage008_full_period_pass_next_multistart":
    ...
    elif decision["decision"] == "stage008_failed_return_retention_no_param_rescue":
        decision["overfit_reflection_after"] = (
            "No new overfit was introduced, but trying to rescue the shape by changing 30 minutes or 50% after seeing "
    ...
    elif decision["decision"] == "stage008_failed_drawdown_no_param_rescue":
        decision["overfit_reflection_after"] = (
    ...
    print(f"[stage008] decision={decision['decision']}", flush=True)
    print(f"[stage008] comparison={COMPARISON_OUT}", flush=True)
    print(f"[stage008] path_chart={PATH_CHART_OUT}", flush=True)
