...
STAGE = "Stage231"
MODEL_TAG = "stage231_predecision_lookback_tick_aggregate_delivery_batch_v1"
OUTPUT_PREFIX = "qmt_roll_stage231_c9_minrisk_predecision_lookback_tick_aggregate_delivery_batch"
...
def _load_stage178_module():
    spec = importlib.util.spec_from_file_location("stage178_base_for_stage231", STAGE178_PATH)
...
def _configure(module) -> None:
    output_dir = LINE_DIR / "outputs" / "stage231_predecision_lookback_tick_aggregate_delivery_batch"
    module.STAGE = STAGE
    module.MODEL_TAG = MODEL_TAG
    module.OUTPUT_PREFIX = OUTPUT_PREFIX
    module.SELECTION_POLICY = "stage177_remaining_highest_priority_exchange_round_robin_stage231_no_pnl_no_rule"
    module.REPORT_TITLE = "Stage231 Predecision Lookback Tick Aggregate Delivery Batch"
    module.SCOPE_NOTE = "Stage177 predecision lookback raw/normalized/proof delivery expansion batch."
    module.SELECTION_NOTE = "priority classes are Stage177 coverage obligations only, not trade filters; Stage231 continues from existing triplets."
    module.SUCCESS_DECISION = "stage231_predecision_lookback_tick_aggregate_delivery_written_refresh_stage179_180_181_no_rule"
    module.FAIL_DECISION = "stage231_predecision_lookback_tick_aggregate_delivery_none_written_need_source_route_repair_no_rule"
    module.SUCCESS_NEXT_ACTION = "refresh_stage179_180_181_for_all_delivered_predecision_requests"
    module.FAIL_NEXT_ACTION = "repair_stage231_source_or_reduce_batch_before_validator_refresh"
    ...
    module.MAX_REQUESTS = int(os.getenv("STAGE231_MAX_REQUESTS", "4"))
    module.WRITE_INCOMING = os.getenv("STAGE231_WRITE_INCOMING", "1").strip() != "0"
    module.OVERWRITE_EXISTING = os.getenv("STAGE231_OVERWRITE_EXISTING", "0").strip() == "1"
    module.MAX_SECONDS_TICK = int(os.getenv("STAGE231_MAX_SECONDS_TICK", "240"))
    module.TICK_DATA_LENGTH = int(os.getenv("STAGE231_TICK_DATA_LENGTH", "10000"))
    module.MIN_NORMALIZED_ROWS = int(os.getenv("STAGE231_MIN_NORMALIZED_ROWS", "61"))
    module.MIN_POSITIVE_VOLUME_BARS = int(os.getenv("STAGE231_MIN_POSITIVE_VOLUME_BARS", "60"))
