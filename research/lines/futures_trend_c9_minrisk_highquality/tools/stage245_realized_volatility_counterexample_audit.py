...
STAGE = "Stage245"
MODEL_TAG = "stage245_realized_volatility_counterexample_audit_v1"
OUTPUT_PREFIX = "qmt_roll_stage245_c9_minrisk_realized_volatility_counterexample_audit"

SCRIPT_PATH = Path(__file__).resolve()
REPO_DIR = SCRIPT_PATH.parents[4]
LINE_DIR = REPO_DIR / "research" / "lines" / LINE_ID
OUTPUT_DIR = LINE_DIR / "outputs" / "stage245_realized_volatility_counterexample_audit"
...
EVENT_OUT = OUTPUT_DIR / f"{OUTPUT_PREFIX}_event_volatility_audit_{MODEL_TAG}.csv"
...
SPLIT_SUMMARY_OUT = OUTPUT_DIR / f"{OUTPUT_PREFIX}_q2_q3_q5_split_summary_{MODEL_TAG}.csv"
JOINT_MATRIX_OUT = OUTPUT_DIR / f"{OUTPUT_PREFIX}_volatility_efficiency_joint_matrix_{MODEL_TAG}.csv"
...
QUINTILE_CHART_OUT = OUTPUT_DIR / f"{OUTPUT_PREFIX}_low_volatility_quintile_label_rates_{MODEL_TAG}.png"
GROUP_CHART_OUT = OUTPUT_DIR / f"{OUTPUT_PREFIX}_fixed_volatility_group_label_rates_{MODEL_TAG}.png"
SPLIT_HEATMAP_OUT = OUTPUT_DIR / f"{OUTPUT_PREFIX}_q3_q5_minus_q2_split_delta_{MODEL_TAG}.png"
JOINT_HEATMAP_OUT = OUTPUT_DIR / f"{OUTPUT_PREFIX}_volatility_efficiency_joint_heatmap_{MODEL_TAG}.png"
SCATTER_OUT = OUTPUT_DIR / f"{OUTPUT_PREFIX}_realized_volatility_context_scatter_{MODEL_TAG}.png"
...
def _prepare_event(joined: pd.DataFrame, artifact: pd.DataFrame) -> pd.DataFrame:
    required = [
        ...
        "candidate_realized_volatility_30m",
        "audit_value_low_realized_volatility_30m",
        "quality_value_low_realized_volatility_30m",
        "quality_quintile_low_realized_volatility_30m",
    ...
    event["low_vol_quality_quintile"] = event["quality_quintile_low_realized_volatility_30m"].astype(int)
    ...
    event["raw_realized_volatility_30m"] = event["candidate_realized_volatility_30m"]
    event["low_raw_vol_q4q5_flag"] = event["low_vol_quality_quintile"].ge(4).astype(int)
    event["high_raw_vol_q1q2_flag"] = event["low_vol_quality_quintile"].le(2).astype(int)
    ...
    event["stage245_strategy_rule_allowed"] = 0
    event["stage245_true_engine_allowed"] = 0
    return event
...
def _summarize_group(group_id: str, frame: pd.DataFrame) -> dict[str, Any]:
    return {
        ...
        "raw_realized_volatility_30m_median": _median(frame["raw_realized_volatility_30m"]),
        ...
        "stage245_strategy_rule_allowed": 0,
    }
...
def _build_quintile_summary(event: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for q in range(1, 6):
        group = event[event["low_vol_quality_quintile"].eq(q)]
        row = _summarize_group(f"low_vol_quality_q{q}", group)
        row["low_vol_quality_quintile"] = q
        rows.append(row)
...
def _build_group_summary(event: pd.DataFrame) -> pd.DataFrame:
    specs = [
        ("q1_highest_raw_vol", event[event["low_vol_quality_quintile"].eq(1)]),
        ("q2_mid_high_raw_vol", event[event["low_vol_quality_quintile"].eq(2)]),
        ("q3_mid_raw_vol", event[event["low_vol_quality_quintile"].eq(3)]),
        ("q4_low_raw_vol", event[event["low_vol_quality_quintile"].eq(4)]),
        ("q5_lowest_raw_vol", event[event["low_vol_quality_quintile"].eq(5)]),
        ("low_raw_vol_q4q5", event[event["low_raw_vol_q4q5_flag"].eq(1)]),
        ("high_raw_vol_q1q2", event[event["high_raw_vol_q1q2_flag"].eq(1)]),
        ("q2_bad", event[event["low_vol_quality_quintile"].eq(2) & event["risk_bad_label"].eq(1)]),
        ("q2_tail", event[event["low_vol_quality_quintile"].eq(2) & event["right_tail_label"].eq(1)]),
        ("q3_bad", event[event["low_vol_quality_quintile"].eq(3) & event["risk_bad_label"].eq(1)]),
        ("q5_bad", event[event["low_vol_quality_quintile"].eq(5) & event["risk_bad_label"].eq(1)]),
        ("high_raw_vol_tail_miss", event[event["low_vol_quality_quintile"].le(2) & event["right_tail_label"].eq(1)]),
    ]
...
def _build_split_summary(event: pd.DataFrame) -> pd.DataFrame:
    ...
    for split_type, split_column in [("year", "decision_year"), ("exchange", "exchange"), ("direction", "direction")]:
        for split_value, split_group in event.groupby(split_column, dropna=False):
            q2 = split_group[split_group["low_vol_quality_quintile"].eq(2)]
            q3 = split_group[split_group["low_vol_quality_quintile"].eq(3)]
            q5 = split_group[split_group["low_vol_quality_quintile"].eq(5)]
            records.append(
                {
                    ...
                    "q2_artifact_context_rate": _rate(q2["artifact_context_flag"]),
                    "q3_artifact_context_rate": _rate(q3["artifact_context_flag"]),
                    "q5_artifact_context_rate": _rate(q5["artifact_context_flag"]),
                    "q3_minus_q2_risk_bad_rate": _rate(q3["risk_bad_label"]) - _rate(q2["risk_bad_label"]),
                    "q5_minus_q2_risk_bad_rate": _rate(q5["risk_bad_label"]) - _rate(q2["risk_bad_label"]),
                    "q5_minus_q2_right_tail_rate": _rate(q5["right_tail_label"]) - _rate(q2["right_tail_label"]),
                    "q5_minus_q2_artifact_context_rate": _rate(q5["artifact_context_flag"]) - _rate(q2["artifact_context_flag"]),
                    "valid_q2_q3_compare": int(len(q2) >= MIN_SPLIT_ROWS and len(q3) >= MIN_SPLIT_ROWS),
                    "valid_q2_q5_compare": int(len(q2) >= MIN_SPLIT_ROWS and len(q5) >= MIN_SPLIT_ROWS),
                    "stage245_strategy_rule_allowed": 0,
                }
            )
...
def _build_joint_matrix(event: pd.DataFrame) -> pd.DataFrame:
    ...
    for qv in range(1, 6):
        for qe in range(1, 6):
            group = event[event["low_vol_quality_quintile"].eq(qv) & event["efficiency_quintile"].eq(qe)]
            records.append(
                {
                    "low_vol_quality_quintile": qv,
...
def _load_bars(row: pd.Series) -> pd.DataFrame:
    ...
    bars["stage245_realized_volatility_30m_path"] = log_ret.rolling(30, min_periods=30).std(ddof=1)
    return bars.tail(ATLAS_LOOKBACK_BARS).reset_index(drop=True)


def _select_atlas(event: pd.DataFrame) -> pd.DataFrame:
    specs = [
        ("q2_tail", event.query("low_vol_quality_quintile == 2 and right_tail_label == 1")),
        ("q2_bad", event.query("low_vol_quality_quintile == 2 and risk_bad_label == 1")),
        ("q3_bad", event.query("low_vol_quality_quintile == 3 and risk_bad_label == 1")),
        ("q5_bad", event.query("low_vol_quality_quintile == 5 and risk_bad_label == 1")),
        ("high_raw_vol_tail_miss", event.query("low_vol_quality_quintile <= 2 and right_tail_label == 1")),
    ]
    ...
    for category, frame in specs:
        if frame.empty:
            continue
        if category == "q5_bad":
            picked = frame.sort_values("raw_realized_volatility_30m", ascending=True).head(6).copy()
        elif category == "high_raw_vol_tail_miss":
            picked = frame.sort_values("raw_realized_volatility_30m", ascending=False).head(6).copy()
        else:
            picked = frame.sort_values("raw_realized_volatility_30m", ascending=False).head(6).copy()
...
def _plot_official_path(curve: pd.DataFrame, summary: dict[str, Any]) -> None:
    ...
    axes[0].set_title("Official path unchanged; Stage245 audits realized_volatility_30m only")
    ...
    text = (
        f"Q2 risk={summary['q2_risk_bad_rate']:.3f} tail={summary['q2_right_tail_rate']:.3f} | "
        f"Q3 risk={summary['q3_risk_bad_rate']:.3f} | "
...
def _plot_quintile_rates(quintile_summary: pd.DataFrame) -> None:
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([f"Q{int(q)}" for q in quintile_summary["low_vol_quality_quintile"]])
    ax.set_ylabel("rate")
    ax.set_title("low_realized_volatility_30m quality quintile label rates; Q5 is lowest raw volatility")
...
def _plot_group_rates(group_summary: pd.DataFrame) -> None:
    ...
    ax.set_ylabel("rate")
    ax.set_title("Fixed realized-volatility groups; Q2 best-looking bucket is isolated")
    ax.grid(axis="y", alpha=0.25)
    ax.legend(loc="best")
...
def _plot_split_delta(split_summary: pd.DataFrame) -> None:
    ...
    pivot = subset.set_index("split_label")[
        [
            "q3_minus_q2_risk_bad_rate",
            "q5_minus_q2_risk_bad_rate",
            "q5_minus_q2_right_tail_rate",
            "q5_minus_q2_artifact_context_rate",
        ]
    ]
    ...
    fig, ax = plt.subplots(figsize=(9.2, max(4.8, 0.38 * len(pivot))))
    data = pivot.to_numpy(dtype=float)
    image = ax.imshow(np.ma.masked_invalid(data), aspect="auto", cmap="RdBu_r", vmin=-0.6, vmax=0.6)
    ax.set_xticks(np.arange(len(pivot.columns)))
    ax.set_xticklabels(["risk Q3-Q2", "risk Q5-Q2", "tail Q5-Q2", "artifact Q5-Q2"], rotation=20, ha="right")
    ax.set_yticks(np.arange(len(pivot.index)))
    ax.set_yticklabels(pivot.index, fontsize=8)
    ax.set_title("Q3/Q5 minus Q2 by split; Q2 is the tempting sample bucket")
...
def _plot_joint_heatmap(joint: pd.DataFrame) -> None:
    ...
    for ax, value_col, title, cmap in [
        (axes[0], "risk_bad_rate", "risk_bad by low-vol Q / efficiency Q", "Reds"),
        (axes[1], "right_tail_rate", "right_tail by low-vol Q / efficiency Q", "Greens"),
    ]:
        pivot = joint.pivot(index="efficiency_quintile", columns="low_vol_quality_quintile", values=value_col)
        count = joint.pivot(index="efficiency_quintile", columns="low_vol_quality_quintile", values="row_count")
        data = pivot.to_numpy(dtype=float)
        ...
        ax.set_xticks(np.arange(len(pivot.columns)))
        ax.set_xticklabels([f"LQ{col}" for col in pivot.columns])
        ax.set_yticks(np.arange(len(pivot.index)))
        ax.set_yticklabels([f"EQ{idx}" for idx in pivot.index])
        ax.set_xlabel("low_realized_volatility_30m quality quintile; Q5=lowest raw vol")
...
def _plot_scatter(event: pd.DataFrame) -> None:
    ...
    for q, marker in markers.items():
        mask = event["low_vol_quality_quintile"].eq(q)
        axes[0].scatter(
            event.loc[mask, "raw_realized_volatility_30m"],
            event.loc[mask, "quality_value_directional_efficiency_30m"],
        ...
        axes[1].scatter(
            event.loc[mask, "raw_realized_volatility_30m"],
            event.loc[mask, "audit_value_volume_zscore_60m"],
            c=colors[mask.to_numpy()],
            s=sizes[mask.to_numpy()],
            marker=marker,
            alpha=0.78,
    ...
    other = ~event["low_vol_quality_quintile"].isin(markers)
    axes[0].scatter(
        event.loc[other, "raw_realized_volatility_30m"],
        event.loc[other, "quality_value_directional_efficiency_30m"],
        c=colors[other.to_numpy()],
        s=sizes[other.to_numpy()],
        marker=".",
        alpha=0.45,
        edgecolor="none",
        label="other",
    ...
    axes[1].scatter(
        event.loc[other, "raw_realized_volatility_30m"],
        event.loc[other, "audit_value_volume_zscore_60m"],
        c=colors[other.to_numpy()],
        s=sizes[other.to_numpy()],
        marker=".",
        alpha=0.45,
    ...
    axes[0].set_ylabel("directional_efficiency_30m")
    axes[0].set_title("Realized volatility vs directional efficiency")
    axes[1].set_ylabel("volume_zscore_60m")
    axes[1].set_title("Realized volatility vs volume surprise")
    for ax in axes:
        ax.set_xlabel("raw realized_volatility_30m")
        ax.grid(alpha=0.25)
        ax.legend(loc="best", fontsize=8)
    fig.suptitle("Stage245 scatter; red=risk_bad, green=right_tail; marker highlights LQ2/LQ3/LQ5", fontsize=12)
    ...
    for category, group in selected.groupby("atlas_category", sort=False):
        ...
        for start in range(0, len(rows), 6):
            ...
            for panel_idx, (_, row) in enumerate(page_rows.iterrows()):
                ...
                ax.grid(alpha=0.2)
                ax.set_title(
                    f"{row['vt_symbol']} {row['direction']} LQ{int(row['low_vol_quality_quintile'])} "
                    f"risk={int(row['risk_bad_label'])} tail={int(row['right_tail_label'])}",
                    fontsize=8,
                )
                ax.tick_params(axis="both", labelsize=7)
                ax2 = ax.twinx()
                rv = pd.to_numeric(bars["stage245_realized_volatility_30m_path"], errors="coerce")
                ax2.plot(x, rv, color="#ff7f0e", linewidth=0.9, alpha=0.85)
                ax2.tick_params(axis="y", labelsize=6, colors="#ff7f0e")
                ax2.set_ylabel("rv30", fontsize=6, color="#ff7f0e")
                subtitle = (
                    f"rv={float(row['raw_realized_volatility_30m']):.6f}; "
                    f"effQ={int(row['efficiency_quintile'])}; "
                    f"art={int(row['artifact_context_flag'])}"
                )
                ...
                rows_out.append(
                    {
                        ...
                        "low_vol_quality_quintile": int(row["low_vol_quality_quintile"]),
                        "raw_realized_volatility_30m": float(row["raw_realized_volatility_30m"]),
            ...
            fig.suptitle(
                f"Stage245 {category}: blue=directional price path, orange=rolling realized volatility path",
                fontsize=12,
            )
...
def _build_gate_status(summary: dict[str, Any], atlas_pages: list[Path]) -> pd.DataFrame:
    gates = [
        ...
        ("event_row_count_219", summary["event_volatility_row_count"] == 219, "219 volatility rows"),
        ("q2_is_best_but_single_bucket", summary["q2_risk_bad_rate"] < summary["q3_risk_bad_rate"] and summary["q2_risk_bad_rate"] < summary["q5_risk_bad_rate"], "Q2 is the tempting sample bucket"),
        ("low_vol_q4q5_not_better_than_q2", summary["low_raw_vol_q4q5_risk_bad_rate"] > summary["q2_risk_bad_rate"], "lowest volatility aggregate does not beat Q2"),
        ("q5_has_bad_cases", summary["q5_risk_bad_count"] > 0, "lowest-vol bucket still has bad cases"),
        ("high_raw_vol_tail_miss_exists", summary["high_raw_vol_tail_miss_count"] > 0, "high/mid-high raw volatility still contains right-tail cases"),
    ...
    return pd.DataFrame(
        [{"gate_id": gate_id, "pass": int(bool(passed)), "description": description} for gate_id, passed, description in gates]
    )
...
def _write_report(
    ...
    lines = [
        "# Stage245 Realized Volatility Counterexample Audit",
        "",
        "## Decision",
        "",
        f"- decision: `{summary['decision']}`",
        f"- event_volatility_row_count: `{summary['event_volatility_row_count']}`",
        f"- q2_risk_bad_rate: `{summary['q2_risk_bad_rate']:.6f}`",
        f"- q2_right_tail_rate: `{summary['q2_right_tail_rate']:.6f}`",
        f"- q3_risk_bad_rate: `{summary['q3_risk_bad_rate']:.6f}`",
        f"- q5_risk_bad_rate: `{summary['q5_risk_bad_rate']:.6f}`",
        f"- low_raw_vol_q4q5_risk_bad_rate: `{summary['low_raw_vol_q4q5_risk_bad_rate']:.6f}`",
        f"- high_raw_vol_tail_miss_count: `{summary['high_raw_vol_tail_miss_count']}`",
        ...
        "- Fixed quality quintile for `low_realized_volatility_30m`; Q5 means the lowest raw realized volatility.",
        "- This stage checks whether low realized volatility is a universal low-risk/high-tail context.",
        "- No threshold scan, no Q2 promotion, no true engine, no A/B.",
        "",
        "## Quintile Summary",
        "",
...
def main() -> None:
    ...
    q2 = event[event["low_vol_quality_quintile"].eq(2)]
    q3 = event[event["low_vol_quality_quintile"].eq(3)]
    q5 = event[event["low_vol_quality_quintile"].eq(5)]
    low_raw = event[event["low_raw_vol_q4q5_flag"].eq(1)]
    high_tail_miss = event[event["low_vol_quality_quintile"].le(2) & event["right_tail_label"].eq(1)]

    feature_row = feature_summary[feature_summary["audit_feature_id"].eq("low_realized_volatility_30m")]
    ...
    summary: dict[str, Any] = {
        ...
        "decision": decision,
        "event_volatility_row_count": int(len(event)),
        ...
        "q5_right_tail_rate": _rate(q5["right_tail_label"]),
        "q5_artifact_context_rate": _rate(q5["artifact_context_flag"]),
        "low_raw_vol_q4q5_count": int(len(low_raw)),
        "low_raw_vol_q4q5_risk_bad_count": int(low_raw["risk_bad_label"].sum()),
        "low_raw_vol_q4q5_risk_bad_rate": _rate(low_raw["risk_bad_label"]),
        "low_raw_vol_q4q5_right_tail_count": int(low_raw["right_tail_label"].sum()),
        "low_raw_vol_q4q5_right_tail_rate": _rate(low_raw["right_tail_label"]),
        "q3_minus_q2_risk_bad_rate": _rate(q3["risk_bad_label"]) - _rate(q2["risk_bad_label"]),
        "q5_minus_q2_risk_bad_rate": _rate(q5["risk_bad_label"]) - _rate(q2["risk_bad_label"]),
        "q5_minus_q2_right_tail_rate": _rate(q5["right_tail_label"]) - _rate(q2["right_tail_label"]),
        "low_raw_vol_q4q5_minus_q2_risk_bad_rate": _rate(low_raw["risk_bad_label"]) - _rate(q2["risk_bad_label"]),
        "high_raw_vol_tail_miss_count": int(len(high_tail_miss)),
