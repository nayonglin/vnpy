# Stage431 正式Stage372影子盘20260608无信号

- line_id：`futures_trend_drawdown30_preserve_return`
- 当前模式：`day`
- 记录时间：`2026-06-08 21:47 CST`
...
- 参考资料：
  ...
  - 官方交易日历核对：中国金融期货交易所 2026 年休市安排显示端午休市为 `2026-06-19` 至 `2026-06-21`，`2026-06-08` 与 `2026-06-09` 不在节假日休市段。
  - GitHub/外部策略代码：本次不是新策略研究或优化，不引入外部策略代码。
- 我的判断：
  - `2026-06-08` 日线已可由 TQ 数据源补齐，适合生成给 `2026-06-08` 夜盘与 `2026-06-09` 日盘参考的影子盘信号。
  - 本次必须固定使用当前正式版本 `official_live_stage372_20w_recovery_sleeve`，不能回退 Stage653 原版或旧 Stage78-1 50万口径。
...
- 删除脚本：无。
- 新增参数：
  - 数据更新：`--mapping-start 2026-06-01 --bar-start 2026-06-08 --end 2026-06-08`
  - 影子盘：`--target-date 2026-06-08`
- 修改参数：无。
- 删除参数：无。

## 回测/归因参数

- 数据区间：`2026-01-01` 至 `2026-06-08`
- 最新完成数据日：`2026-06-08`
- 账户规模：`200,000`
...
- 当前理论影子持仓：`jm2609.DCE` 多头 `2` 手，收盘价 `1429.5`，精确保证金 `34,308`
- 目标信号数：`0`
- 6月8日夜盘交易信号：无新增开仓、无平仓、无反手。
- 6月9日日盘交易信号：无新增开仓、无平仓、无反手。
- 订单 API 调用次数：`0`
...
- 本阶段结论：`2026-06-08` 官方 Stage372/20万影子盘没有生成可用于 `2026-06-08` 夜盘或 `2026-06-09` 日盘的新增交易信号。
- 是否进入下一步：只做日常跟踪，不进入策略研究或A/B。
- 下一步：
  - 若只是人工盘前检查：今晚和明天日盘不需要按影子盘信号发起新订单。
