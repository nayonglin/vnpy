...
OUTPUT_PREFIX = "rebuilt_c9_stage046_warehouse_build_add_risk_proxy"
...
OUTPUT_DIR = LINE_DIR / "outputs" / "stage046_warehouse_build_add_risk_proxy"
...
def select_warehouse_build_lots(lots: pd.DataFrame) -> pd.DataFrame:
    values = pd.to_numeric(lots.get("external_warehouse_change_20d_sum"), errors="coerce")
    return lots[values.gt(0)].copy().reset_index(drop=True)
...
def _build_lot_deltas() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    external = s045.build_external_daily_features(basis, warehouse, min_history=s045.MIN_EXTERNAL_HISTORY)
    attached = s045.attach_t1_external_features(closed, external)
    attached["stage046_external_feature_matched"] = attached["external_warehouse_change_20d_sum"].notna()
    attached["stage046_selected_for_warehouse_build_proxy"] = pd.to_numeric(
        attached["external_warehouse_change_20d_sum"], errors="coerce"
    ).gt(0)
    attached["stage046_add_risk_fraction"] = ADD_RISK_FRACTION
    attached["stage046_proxy_delta_pnl"] = np.where(
        attached["stage046_selected_for_warehouse_build_proxy"],
        attached["realized_pnl"] * ADD_RISK_FRACTION,
        0.0,
    )
    selected = select_warehouse_build_lots(attached)
    keep = [
        ...
        "stage046_external_feature_matched",
        "stage046_selected_for_warehouse_build_proxy",
        "stage046_add_risk_fraction",
        "stage046_proxy_delta_pnl",
    ]
    audit = {
        "stage013_closed_lot_count": int(len(closed)),
        "external_feature_matched_lot_count": int(attached["stage046_external_feature_matched"].sum()),
        "external_feature_match_rate_pct": float(attached["stage046_external_feature_matched"].mean() * 100.0)
        if len(attached)
        else np.nan,
        "selected_lots": int(len(selected)),
        "selected_realized_pnl": float(selected["realized_pnl"].sum()) if len(selected) else 0.0,
        "total_proxy_delta_pnl": float(selected["stage046_proxy_delta_pnl"].sum()) if len(selected) else 0.0,
...
def _build_proxy_curves(base_curves: pd.DataFrame, lot_deltas: pd.DataFrame) -> tuple[pd.DataFrame, int]:
    ...
    daily_delta = (
        lot_deltas.groupby(["requested_start_month", "exit_date"], dropna=False)["stage046_proxy_delta_pnl"]
        .sum()
        .reset_index()
        if not lot_deltas.empty
        else pd.DataFrame(columns=["requested_start_month", "exit_date", "stage046_proxy_delta_pnl"])
    )
    merged = curves.merge(
        daily_delta.rename(columns={"exit_date": "date", "stage046_proxy_delta_pnl": "stage046_daily_delta"}),
        on=["requested_start_month", "date"],
        how="left",
    )
    merged["stage046_daily_delta"] = pd.to_numeric(merged["stage046_daily_delta"], errors="coerce").fillna(0.0)
    frames: list[pd.DataFrame] = []
    for _, group in merged.groupby("requested_start_month", sort=True):
        g = group.sort_values("date").copy()
        g["stage046_cum_delta"] = g["stage046_daily_delta"].cumsum()
        g["stage046_account_equity"] = g["account_equity"] + g["stage046_cum_delta"]
        g["stage046_nav"] = g["stage046_account_equity"] / CAPITAL
        g["stage046_drawdown_pct"] = _drawdown_pct(g["stage046_account_equity"])
        frames.append(g)
...
def _summary(proxy_curves: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, group in proxy_curves.groupby("requested_start_month"):
        ...
        rows.append(_summarize_curve(group, "stage046_account_equity", "stage046_warehouse_build_add_risk_proxy"))
...
def _wide_summary(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    wide["return_delta_pp_stage046_vs_stage013"] = (
        wide["total_return_pct_stage046_warehouse_build_add_risk_proxy"] - wide["total_return_pct_stage013_engine"]
    )
    wide["maxdd_delta_pp_stage046_vs_stage013"] = (
        wide["max_dd_pct_stage046_warehouse_build_add_risk_proxy"] - wide["max_dd_pct_stage013_engine"]
    )
...
def _goal_audit(proxy_curves: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for variant, column in [
        ...
        ("stage046_warehouse_build_add_risk_proxy", "stage046_account_equity"),
    ]:
...
def _retention(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    merged["stage046_vs_base_stage006_return_ratio"] = (
        merged["total_return_pct_stage046_warehouse_build_add_risk_proxy"]
        / pd.to_numeric(merged["total_return_pct_base_stage006"], errors="coerce").replace(0.0, np.nan)
    )
    merged["stage046_vs_stage013_return_ratio"] = (
        merged["total_return_pct_stage046_warehouse_build_add_risk_proxy"]
        / pd.to_numeric(merged["total_return_pct_stage013_engine"], errors="coerce").replace(0.0, np.nan)
    )
    merged["passes_80pct_retention_vs_base_stage006"] = merged["stage046_vs_base_stage006_return_ratio"].ge(0.80).astype("int64")
    merged["passes_80pct_retention_vs_stage013"] = merged["stage046_vs_stage013_return_ratio"].ge(0.80).astype("int64")
    return merged
...
def _decision(
    ...
    result: dict[str, Any] = {
        ...
        "audit_type": "stage013_closed_lot_read_only_warehouse_build_add_risk_proxy",
        **audit,
        "unmatched_delta_dates": int(unmatched_delta_dates),
        "sample_count": int(wide["requested_start_month"].nunique()),
        "stage046_min_return_pct": float(wide["total_return_pct_stage046_warehouse_build_add_risk_proxy"].min()),
        "stage046_median_return_pct": float(wide["total_return_pct_stage046_warehouse_build_add_risk_proxy"].median()),
        "stage046_worst_max_dd_pct": float(wide["max_dd_pct_stage046_warehouse_build_add_risk_proxy"].min()),
        "stage046_median_max_dd_pct": float(wide["max_dd_pct_stage046_warehouse_build_add_risk_proxy"].median()),
        "return_improved_count_vs_stage013": int(wide["return_delta_pp_stage046_vs_stage013"].gt(EPS).sum()),
        "return_unchanged_count_vs_stage013": int(wide["return_delta_pp_stage046_vs_stage013"].abs().le(EPS).sum()),
        "return_worse_count_vs_stage013": int(wide["return_delta_pp_stage046_vs_stage013"].lt(-EPS).sum()),
        "maxdd_improved_count_vs_stage013": int(wide["maxdd_delta_pp_stage046_vs_stage013"].gt(EPS).sum()),
        "maxdd_unchanged_count_vs_stage013": int(wide["maxdd_delta_pp_stage046_vs_stage013"].abs().le(EPS).sum()),
        "maxdd_worse_count_vs_stage013": int(wide["maxdd_delta_pp_stage046_vs_stage013"].lt(-EPS).sum()),
        ...
        "external_research_judgment": (
            "库存、仓单与基差在商品期货中有经济含义，但必须 point-in-time 并验证组合路径。"
            "Stage046 只冻结 Stage045 最强仓单条件 external_warehouse_build_20d 和 25% 非挤占风险，不扫阈值。"
        ),
        "overfit_reflection_before": (
            "否。只冻结 Stage045 第一候选 `external_warehouse_build_20d`，不扫分位、产品、年份、方向或倍率。"
        ),
        "continue_value_before": (
            "有。用户目标要求识别超高质量信号并加大风险投入，必须检验仓单外生候选能否改善组合路径。"
        ),
        "overfit_reflection_after": (
            "否。本阶段无参数搜索；若失败后改 `20d`、分位、产品、年份或方向就是过拟合。"
        ),
    }
    result.update(_strict_metrics(aggregate, "stage013_engine"))
    result.update(_strict_metrics(aggregate, "stage046_warehouse_build_add_risk_proxy"))
    strict_negative = result["stage046_warehouse_build_add_risk_proxy_all_gt1y_negative_count"]
    ...
    if strict_negative == 0 and retention_full:
        decision = "stage046_warehouse_build_proxy_meets_goal_requires_true_engine"
        continue_after = "有。proxy 达到目标形状，下一步必须写真实引擎验真，不能直接上线。"
    elif strict_negative < stage013_negative and retention_full:
        decision = "stage046_warehouse_build_proxy_partially_improves_not_goal"
        continue_after = "有但未达标。仓单条件可保留为候选，下一步先做日级冷启动探针或真引擎验真。"
    else:
        decision = "stage046_warehouse_build_proxy_not_enough_no_param_rescue"
...
def _write_report(decision: dict[str, Any], summary: pd.DataFrame, aggregate: pd.DataFrame, retention: pd.DataFrame) -> None:
    strict = aggregate[
        aggregate["variant"].eq("stage046_warehouse_build_add_risk_proxy")
    ...
    lines = [
        ...
        f"- Stage046 严格任意 `>1` 年负窗口：`{decision['stage046_warehouse_build_add_risk_proxy_all_gt1y_negative_count']}` / `{decision['stage046_warehouse_build_add_risk_proxy_all_gt1y_window_count']}`；最差 `{decision['stage046_warehouse_build_add_risk_proxy_all_gt1y_min_return_pct']:.4f}%`。",
        f"- Stage013 严格任意 `>1` 年负窗口：`{decision['stage013_engine_all_gt1y_negative_count']}`。",
        f"- 到 `2026-06-30` 负窗口：`{decision['stage046_warehouse_build_add_risk_proxy_to_final_negative_count']}`；最差 `{decision['stage046_warehouse_build_add_risk_proxy_to_final_min_return_pct']:.4f}%`。",
        ...
        _md_table(
            wide[
                [
                    ...
                    "total_return_pct_stage046_warehouse_build_add_risk_proxy",
                    "return_delta_pp_stage046_vs_stage013",
                    "max_dd_pct_stage013_engine",
                    "max_dd_pct_stage046_warehouse_build_add_risk_proxy",
                    "maxdd_delta_pp_stage046_vs_stage013",
                ]
            ],
            max_rows=30,
        ),
        ...
        _md_table(
            retention[
                [
                    "requested_start_month",
                    "stage046_vs_base_stage006_return_ratio",
                    "stage046_vs_stage013_return_ratio",
...
def _write_stage_record(decision: dict[str, Any], summary: pd.DataFrame, aggregate: pd.DataFrame, retention: pd.DataFrame) -> Path:
    ...
    path = STAGES_DIR / f"{timestamp:%Y%m%d_%H%M}_stage046_warehouse_build_add_risk_proxy.md"
    report = REPORT_PATH.read_text(encoding="utf-8")
    header = f"""# Stage046 - warehouse_build_20d 非挤占加风险 proxy
...
- 新增脚本：`research/lines/{LINE_ID}/tools/stage046_warehouse_build_add_risk_proxy.py`
