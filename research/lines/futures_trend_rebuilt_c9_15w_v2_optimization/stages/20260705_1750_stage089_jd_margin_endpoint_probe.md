...
|:------------------------------------------|:--------------------------------|:------------------|------------:|---------------:|:---------------------|:---------------------|:-----------------|:------------------------------|:------------------------------------------------|:-------------------------------|:---------------------------------------------------------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| akshare_futures_settle_dce                | exchange_settlement_api_wrapper | empty_unsupported |           0 |              0 | False                | False                | False            | False                         | False                                           | rejected                       | current_akshare_wrapper_unsupported_dce                              | DCE returns empty because wrapper has no DCE branch in installed akshare.                                                                                                                 |
...
- reviewer：`019f31b0-082f-7041-ba1d-aab8b88de4ba`
- 置信度：`0.88`
- 严重问题：无。
- reviewer 结论：Stage089 的判断成立，只能说明当前样本、当前仓库和当前 AKShare wrapper 下没有 accepted `jd_contract_daily_margin_history`，不能外推为 DCE 或 vendor 外部数据不存在。
- 安全性：脚本不触碰官方实盘、CTP、邮件、launchd、真实引擎或下单路径；“只读”应理解为只读数据源探针并在研究线内落盘。
- 口径确认：`gtja_sample_date_count=14`、`gtja_ok_sample_count=11`、`gtja_jd_sample_count=11`、`gtja_contract_adjustment_count=11`、`accepted_candidate_count=0`、`rebuild_candidate_count=1` 均由输出支撑。
- 中等风险：GTJA 是 broker/HTML/AKShare 路线，仍缺全交易日覆盖、原始响应 hash、发布时间/有效时间链；Stage090 若直接进入 true ledger 或回测就是越界。
- 低风险：GTJA adjustment regex 偏窄，Stage090 需要兼容多合约、分号、多空格、中文全角百分号等格式；`gtja_jd_sample_count` 是有 JD 的样本日期数，不是 JD 行数。
- reviewer 建议：继续 Stage090，但只做 GTJA 批量覆盖/解析验收，不允许 true ledger 或回测；DCE/vendor 官方逐日参数仍应保持更高优先级。
