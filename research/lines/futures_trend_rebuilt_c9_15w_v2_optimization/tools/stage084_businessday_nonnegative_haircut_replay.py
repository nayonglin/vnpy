...
STAGE = "Stage084"
MODEL_TAG = "stage084_businessday_nonnegative_haircut_replay_v1"
OUTPUT_PREFIX = "rebuilt_c9_v2_stage084_businessday_nonnegative_haircut_replay"

LINE_DIR = ROOT / "research" / "lines" / LINE_ID
OUT = LINE_DIR / "outputs" / "stage084_businessday_nonnegative_haircut_replay"
STAGES_DIR = LINE_DIR / "stages"
STAGE083_TOOL_PATH = LINE_DIR / "tools" / "stage083_money_fund_friction_sensitivity.py"
...
VARIANT_SPECS = (
    {
        "version": "stage082_base_conservative",
        "label": "Stage082 base conservative",
        "business_delay_days": 0,
        "annual_haircut_bps": 0.0,
        "floor_daily_return_at_zero": True,
    },
    {
        "version": "business_tplus1_delay",
        "label": "Business-day T+1 delay",
        "business_delay_days": 1,
        "annual_haircut_bps": 0.0,
        "floor_daily_return_at_zero": True,
    },
    {
        "version": "business_tplus2_delay",
        "label": "Business-day T+2 delay",
        "business_delay_days": 2,
        "annual_haircut_bps": 0.0,
        "floor_daily_return_at_zero": True,
    },
    {
        "version": "business_tplus1_haircut100_neg_allowed",
        "label": "Business T+1 + 100bp haircut negative allowed",
        "business_delay_days": 1,
        "annual_haircut_bps": 100.0,
        "floor_daily_return_at_zero": False,
    },
    {
        "version": "business_tplus1_haircut150_neg_allowed",
        "label": "Business T+1 + 150bp haircut negative allowed",
        "business_delay_days": 1,
        "annual_haircut_bps": 150.0,
        "floor_daily_return_at_zero": False,
    },
    {
        "version": "business_tplus1_haircut200_neg_allowed",
        "label": "Business T+1 + 200bp haircut negative allowed",
        "business_delay_days": 1,
        "annual_haircut_bps": 200.0,
        "floor_daily_return_at_zero": False,
    },
    {
        "version": "business_tplus1_haircut100_floor0",
        "label": "Business T+1 + 100bp haircut floor0",
        "business_delay_days": 1,
        "annual_haircut_bps": 100.0,
        "floor_daily_return_at_zero": True,
    },
    {
        "version": "business_tplus1_haircut150_floor0",
        "label": "Business T+1 + 150bp haircut floor0",
        "business_delay_days": 1,
        "annual_haircut_bps": 150.0,
        "floor_daily_return_at_zero": True,
    },
    {
        "version": "business_tplus1_haircut200_floor0",
        "label": "Business T+1 + 200bp haircut floor0",
        "business_delay_days": 1,
        "annual_haircut_bps": 200.0,
        "floor_daily_return_at_zero": True,
    },
    {
        "version": "business_tplus2_haircut100_floor0",
        "label": "Business T+2 + 100bp haircut floor0",
        "business_delay_days": 2,
        "annual_haircut_bps": 100.0,
        "floor_daily_return_at_zero": True,
    },
...
def _load_stage083_module() -> Any:
    spec = importlib.util.spec_from_file_location("stage083_money_fund_friction_sensitivity", STAGE083_TOOL_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to load {STAGE083_TOOL_PATH}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


S83 = _load_stage083_module()
S82 = S83.S82
...
def _confirmation_date(start_date: pd.Timestamp, business_delay_days: int) -> pd.Timestamp:
    if business_delay_days <= 0:
        return start_date.normalize()
    return (start_date + BDay(business_delay_days)).normalize()
...
def _fund_factor_lookup(
    ...
    *,
    ...
    floor_daily_return_at_zero: bool,
) -> pd.Series:
    ...
    daily_return = pd.to_numeric(merged["income_per_10k"], errors="coerce").fillna(0.0) / 10_000.0
    adjusted = daily_return - daily_haircut
    if floor_daily_return_at_zero:
        adjusted = adjusted.clip(lower=0.0)
    else:
        adjusted = adjusted.clip(lower=-0.999999)
    return pd.Series((1.0 + adjusted).cumprod().to_numpy(dtype=float), index=merged["date"])
...
def _reserve_equity_for_start(
    ...
    *,
    ...
    income_start = _confirmation_date(start_date, business_delay_days)
    reserve = pd.Series(0.0, index=dates.index)
    for code in basket_codes:
        if code not in active_codes:
            reserve = reserve + PER_FUND_RESERVE_CAPITAL
            continue
        lookup = _fund_factor_lookup(
            history,
            code,
            date_start=date_start,
            annual_haircut_bps=annual_haircut_bps,
            floor_daily_return_at_zero=floor_daily_return_at_zero,
        )
        start_factor = float(lookup.loc[income_start]) if income_start in lookup.index else float(lookup.loc[:income_start].iloc[-1])
        strategy_dates = pd.to_datetime(dates).dt.normalize()
        date_values = lookup.reindex(strategy_dates, method="ffill")
        factors = date_values.to_numpy(dtype=float) / start_factor
        factors = np.where(strategy_dates.le(income_start), 1.0, factors)
...
def build() -> dict[str, Any]:
    ...
    for start_month, group in c9.groupby("requested_start_month", sort=True):
        ...
        for spec in VARIANT_SPECS:
            reserve = _reserve_equity_for_start(
                ...
                business_delay_days=int(spec["business_delay_days"]),
                annual_haircut_bps=float(spec["annual_haircut_bps"]),
                floor_daily_return_at_zero=bool(spec["floor_daily_return_at_zero"]),
            )
            rows.append(
                pd.DataFrame(
                    {
        ...
        variant_rows.append(
            {
                ...
                "business_delay_days": int(spec_rows["business_delay_days"].dropna().iloc[0]) if not spec_rows.empty else 0,
                "annual_haircut_bps": float(spec_rows["annual_haircut_bps"].dropna().iloc[0]) if not spec_rows.empty else 0.0,
                "floor_daily_return_at_zero": bool(spec_rows["floor_daily_return_at_zero"].dropna().iloc[0]) if not spec_rows.empty else True,
    ...
    robust = variant_summary[variant_summary["version"].eq("business_tplus1_haircut100_floor0")]
    decision_name = (
        "stage084_businessday_floor0_confirms_cash_basket_weak_not_promotion"
        if not robust.empty and bool(robust["passes_account_level_stage077_proxy_goal"].iloc[0])
        else "stage084_businessday_replay_breaks_cash_basket_edge_downgrade"
    )
    decision = {
        "stage": STAGE,
        "model_tag": MODEL_TAG,
...
def _plot(variant_summary: pd.DataFrame) -> None:
    ...
    axes[0].set_title("Stage084 business-day and non-negative haircut replay")
...
def _write_report(variant_summary: pd.DataFrame, retention: pd.DataFrame, decision: dict[str, Any]) -> None:
    ...
    text = f"""# Stage084 business-day non-negative haircut replay

## 结论

- 决策：`{decision['decision']}`。
- 口径：不换基金、不扫篮子大小；固定 Stage082 保守篮子。`T+1/T+2` 用近似基金工作日，即周一到周五顺延；当前未引入中国节假日日历。haircut 同时比较允许负 carry 和日收益 floor 到 `0` 的两类口径。
- 外部/GitHub 调研判断：基金回测框架通常显式建模 T+n 确认与到账，但未找到可直接复用到本仓库货基万份收益数据的专用实现；本阶段保留轻量自研口径。
...
def _write_stage_record(variant_summary: pd.DataFrame, retention: pd.DataFrame, decision: dict[str, Any]) -> None:
    ...
    path = STAGES_DIR / f"{stamp}_stage084_businessday_nonnegative_haircut_replay.md"
...
- 新增脚本：`research/lines/{LINE_ID}/tools/stage084_businessday_nonnegative_haircut_replay.py`
