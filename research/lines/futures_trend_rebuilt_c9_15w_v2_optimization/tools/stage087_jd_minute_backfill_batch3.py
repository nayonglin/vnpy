...
STAGE = "Stage087"
MODEL_TAG = "stage087_jd_minute_backfill_batch3_v1"
OUTPUT_PREFIX = "rebuilt_c9_v2_stage087_jd_minute_backfill_batch3"

LINE_DIR = ROOT / "research" / "lines" / LINE_ID
OUT = LINE_DIR / "outputs" / "stage087_jd_minute_backfill_batch3"
...
ENABLE_DOWNLOAD = os.getenv("STAGE087_ENABLE_DOWNLOAD", "0").strip() == "1"
MAX_SYMBOLS = int(os.getenv("STAGE087_MAX_SYMBOLS", "6"))
MAX_SECONDS_PER_SYMBOL = int(os.getenv("STAGE087_MAX_SECONDS_PER_SYMBOL", "140"))
...
def _make_decision(plan: pd.DataFrame, status: pd.DataFrame, before_coverage: pd.DataFrame, after_coverage: pd.DataFrame) -> dict[str, Any]:
    ...
    if not ENABLE_DOWNLOAD:
        ...
        decision = "stage087_jd_minute_backfill_batch3_partial_success_margin_still_blocked"
    else:
        decision = "stage087_jd_minute_backfill_batch3_no_success_keep_stage052_state"
    return {
        "stage": STAGE,
        "line_id": LINE_ID,
        "model_tag": MODEL_TAG,
...
def _write_stage_record(decision: dict[str, Any], plan: pd.DataFrame, status: pd.DataFrame, after_coverage: pd.DataFrame) -> Path:
    stage_path = STAGES_DIR / f"{datetime.now():%Y%m%d_%H%M}_stage087_jd_minute_backfill_batch3.md"
    lines = [
        "# Stage087 jd minute backfill batch3",
        "",
        f"- line_id：`{LINE_ID}`",
        ...
        "## 本次变更",
        "",
        f"- 新增脚本：`research/lines/{LINE_ID}/tools/stage087_jd_minute_backfill_batch3.py`",
