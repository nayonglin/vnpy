...
STAGE = "Stage115"
MODEL_TAG = "stage115_jd_atomic_retry_backfill_v1"
OUTPUT_PREFIX = "rebuilt_c9_v2_stage115_jd_atomic_retry_backfill"

LINE_DIR = ROOT / "research" / "lines" / LINE_ID
OUT = LINE_DIR / "outputs" / "stage115_jd_atomic_retry_backfill"
...
ENABLE_DOWNLOAD = os.getenv("STAGE115_ENABLE_DOWNLOAD", "0").strip() == "1"
MAX_SYMBOLS = int(os.getenv("STAGE115_MAX_SYMBOLS", "2"))
MAX_SECONDS_PER_SYMBOL = int(os.getenv("STAGE115_MAX_SECONDS_PER_SYMBOL", "600"))
...
def make_decision(plan: pd.DataFrame, status: pd.DataFrame, temp_audit: pd.DataFrame, publish: pd.DataFrame, summary: pd.DataFrame) -> dict[str, Any]:
    ...
    if not ENABLE_DOWNLOAD:
        ...
        decision = "stage115_jd_atomic_retry_partial_success_margin_still_blocked"
    else:
        decision = "stage115_jd_atomic_retry_no_publish_keep_blocked"
    ...
    return {
        ...
        "external_research_judgment": (
            "TqSdk 官方 DataDownloader 文档确认历史数据下载会直接写 CSV；Stage115 因此不让下载器直接写入回测可发现目录，"
...
def _write_stage_record(decision: dict[str, Any], plan: pd.DataFrame, status: pd.DataFrame, temp_audit: pd.DataFrame, publish: pd.DataFrame, summary: pd.DataFrame) -> Path:
    stage_path = STAGES_DIR / f"{datetime.now():%Y%m%d_%H%M}_stage115_jd_atomic_retry_backfill.md"
    lines = [
        "# Stage115 jd atomic retry backfill",
        "",
        f"- line_id：`{LINE_ID}`",
        ...
        "## 本次变更",
        "",
        f"- 新增脚本：`research/lines/{LINE_ID}/tools/stage115_jd_atomic_retry_backfill.py`",
