...
STAGE = "Stage118"
MODEL_TAG = "stage118_jd_atomic_backfill_batch4_v1"
OUTPUT_PREFIX = "rebuilt_c9_v2_stage118_jd_atomic_backfill_batch4"

LINE_DIR = ROOT / "research" / "lines" / LINE_ID
OUT = LINE_DIR / "outputs" / "stage118_jd_atomic_backfill_batch4"
...
ENABLE_DOWNLOAD = os.getenv("STAGE118_ENABLE_DOWNLOAD", "0").strip() == "1"
MAX_SYMBOLS = int(os.getenv("STAGE118_MAX_SYMBOLS", "2"))
MAX_SECONDS_PER_SYMBOL = int(os.getenv("STAGE118_MAX_SECONDS_PER_SYMBOL", "600"))
...
def make_decision(plan: pd.DataFrame, status: pd.DataFrame, temp_audit: pd.DataFrame, publish: pd.DataFrame, summary: pd.DataFrame) -> dict[str, Any]:
    ...
    if not ENABLE_DOWNLOAD:
        ...
        decision = "stage118_jd_atomic_batch4_partial_success_margin_still_blocked"
    else:
        decision = "stage118_jd_atomic_batch4_no_publish_keep_blocked"
    ...
    return {
        ...
        "external_research_judgment": (
            "TqSdk 官方 DataDownloader 文档确认历史数据下载会直接写 CSV；Stage118 因此不让下载器直接写入回测可发现目录，"
...
def _write_stage_record(decision: dict[str, Any], plan: pd.DataFrame, status: pd.DataFrame, temp_audit: pd.DataFrame, publish: pd.DataFrame, summary: pd.DataFrame) -> Path:
    stage_path = STAGES_DIR / f"{datetime.now():%Y%m%d_%H%M}_stage118_jd_atomic_backfill_batch4.md"
    lines = [
        "# Stage118 jd atomic retry backfill",
        "",
        f"- line_id：`{LINE_ID}`",
        ...
        "## 本次变更",
        "",
        f"- 新增脚本：`research/lines/{LINE_ID}/tools/stage118_jd_atomic_backfill_batch4.py`",
