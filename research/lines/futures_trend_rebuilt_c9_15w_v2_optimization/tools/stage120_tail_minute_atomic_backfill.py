...
STAGE = "Stage120"
MODEL_TAG = "stage120_tail_minute_atomic_backfill_v1"
OUTPUT_PREFIX = "rebuilt_c9_v2_stage120_tail_minute_atomic_backfill"

LINE_DIR = ROOT / "research" / "lines" / LINE_ID
OUT = LINE_DIR / "outputs" / "stage120_tail_minute_atomic_backfill"
...
ENABLE_DOWNLOAD = os.getenv("STAGE120_ENABLE_DOWNLOAD", "0").strip() == "1"
MAX_SYMBOLS = int(os.getenv("STAGE120_MAX_SYMBOLS", "6"))
MAX_SECONDS_PER_SYMBOL = int(os.getenv("STAGE120_MAX_SECONDS_PER_SYMBOL", "900"))
...
def build_atomic_plan(mod052: Any, mod112: Any, before_strict: pd.DataFrame) -> pd.DataFrame:
    ...
    data = data[
        ...
        & data["priority"].astype(str).eq("P1_tail_contract_gap")
        & data["blocking_reason"].astype(str).eq("missing_file")
    ...
    data = data.sort_values(["request_start_ts", "contract_vt"], ascending=[True, True])
    if MAX_SYMBOLS > 0:
    ...
    for row in temp_audit.itertuples(index=False):
        ...
        if strict_ready and download_ok and temp_path.exists():
            ...
            if not publish_device_match:
                ...
                temp_target = _quarantine_path(temp_path, "existing_final_publish_blocked")
                shutil.move(str(temp_path), str(temp_target))
                temp_quarantine_path = str(temp_target)
                action = "quarantined"
            else:
...
def build_summary(before_strict: pd.DataFrame, after_strict: pd.DataFrame, publish: pd.DataFrame, stale: pd.DataFrame) -> pd.DataFrame:
    before_file_ready = before_strict["minute_file_ready"].astype(bool)
    before_strict_ready = before_strict["strict_ready"].astype(bool)
    after_file_ready = after_strict["minute_file_ready"].astype(bool)
    after_strict_ready = after_strict["strict_ready"].astype(bool)
    return pd.DataFrame(
        [
            {
                "before_strict_ready": int(before_strict_ready.sum()),
                "before_minute_missing": int((~before_file_ready).sum()),
                "before_strict_failed": int((before_file_ready & ~before_strict_ready).sum()),
                "before_remaining_jd_not_ready": int(
                    (before_strict["product_vt_symbol"].astype(str).eq("jd.DCE") & ~before_strict_ready).sum()
                ),
                ...
                "after_strict_ready": int(after_strict_ready.sum()),
                "after_minute_missing": int((~after_file_ready).sum()),
                "after_strict_failed": int((after_file_ready & ~after_strict_ready).sum()),
                "after_remaining_jd_not_ready": int(
                    (after_strict["product_vt_symbol"].astype(str).eq("jd.DCE") & ~after_strict_ready).sum()
                ),
            }
        ]
    )
...
def _margin_ready(mod112: Any) -> tuple[bool, str]:
    try:
        return mod112._stage091_margin_ready()
...
def make_decision(
    ...
    after_strict_failed = int(row.get("after_strict_failed", 0) or 0)
    after_remaining_jd_not_ready = int(row.get("after_remaining_jd_not_ready", 0) or 0)
    margin_ready, margin_decision = _margin_ready(mod112)
    remaining_tail = after_strict[
        ~after_strict["product_vt_symbol"].astype(str).eq("jd.DCE")
        & ~after_strict["strict_ready"].astype(bool)
    ]["contract_vt"].astype(str).tolist()
    remaining_blockers: list[str] = []
    if after_minute_missing > 0 or after_strict_failed > 0:
        remaining_blockers.append(f"tail_minute_files={len(remaining_tail)}")
    if after_remaining_jd_not_ready > 0:
        remaining_blockers.append(f"jd_minute_files={after_remaining_jd_not_ready}")
    if not margin_ready:
        remaining_blockers.append("jd_contract_daily_margin_history")
    ready_for_true_ledger = after_minute_missing == 0 and after_strict_failed == 0 and after_remaining_jd_not_ready == 0 and margin_ready
    if not ENABLE_DOWNLOAD:
        decision = "stage120_tail_minute_atomic_plan_only"
    elif ready_for_true_ledger:
        decision = "stage120_tail_minute_atomic_success_true_ledger_data_ready"
    elif published == len(plan) and published > 0:
        decision = "stage120_tail_minute_atomic_success_margin_still_blocked"
    elif published > 0:
        decision = "stage120_tail_minute_atomic_partial_success_margin_still_blocked"
    else:
        decision = "stage120_tail_minute_atomic_no_publish_keep_blocked"
    return {
        "stage": STAGE,
        "line_id": LINE_ID,
        "model_tag": MODEL_TAG,
        ...
        "after_remaining_jd_not_ready": after_remaining_jd_not_ready,
        "after_minute_missing": after_minute_missing,
        "after_strict_failed": after_strict_failed,
        "jd_margin_history_ready": bool(margin_ready),
        "stage091_decision": margin_decision,
        "ready_for_true_ledger_replay": bool(ready_for_true_ledger),
        "remaining_blocker": remaining_blockers[0] if remaining_blockers else "",
        "remaining_blockers": remaining_blockers,
        "remaining_tail_minute_files": remaining_tail,
        ...
        "external_research_judgment": (
            "TqSdk 官方文档确认 TqBacktest 可推进历史行情，get_kline_serial 可取得分钟 K 线；"
            "但 Stage120 仍不信任下载成功本身，而是先落临时文件、用 Stage112 strict gate 验收后再发布。"
        ),
        ...
        "continue_value_before": "有。Stage119 已把 jd 分钟缺口清零，Stage112 仍显示 6 个非 jd tail minute files 阻塞 true ledger。",
        "continue_value_after": "有。若发布成功可转向 jd 精确逐日保证金；若未成功，应先修数据而不是跑 true ledger。",
        "outputs": {
            "plan": str(PLAN_PATH),
...
def _write_report(decision: dict[str, Any], plan: pd.DataFrame, status: pd.DataFrame, temp_audit: pd.DataFrame, publish: pd.DataFrame, summary: pd.DataFrame) -> None:
    lines = [
        ...
        "- 阶段性质：补 6 个非 jd tail minute files；先临时下载、strict gate 验收、再发布；不回测收益、不改策略、不连接 CTP、不调用订单 API。",
        "",
        "## 外部调研与判断",
        "",
        "- TqSdk 历史回测模式可推进行情并更新 K 线；本阶段沿用已验证下载路线，但把下载输出和回测可发现目录隔离。",
        ...
        f"- remaining_blockers：`{', '.join(decision['remaining_blockers']) if decision['remaining_blockers'] else ''}`",
        f"- remaining_tail_minute_files：`{', '.join(decision['remaining_tail_minute_files']) if decision['remaining_tail_minute_files'] else ''}`",
        "",
        "## Plan",
        "",
...
def _write_stage_record(decision: dict[str, Any], plan: pd.DataFrame, status: pd.DataFrame, temp_audit: pd.DataFrame, publish: pd.DataFrame, summary: pd.DataFrame) -> Path:
    stage_path = STAGES_DIR / f"{datetime.now():%Y%m%d_%H%M}_stage120_tail_minute_atomic_backfill.md"
    lines = [
        "# Stage120 tail minute atomic backfill",
        "",
        f"- line_id：`{LINE_ID}`",
        ...
        "## 本次变更",
        "",
        f"- 新增脚本：`research/lines/{LINE_ID}/tools/stage120_tail_minute_atomic_backfill.py`",
        ...
        f"- after_strict_failed：`{decision['after_strict_failed']}`",
...
def run() -> dict[str, Any]:
    ...
    decision = make_decision(mod112, plan, status, temp_audit, publish, summary, after_strict)
