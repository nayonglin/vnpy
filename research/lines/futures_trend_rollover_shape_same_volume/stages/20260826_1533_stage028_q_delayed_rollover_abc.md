...
- 边界闭环：D0-D4即使新主力当日Bar缺失也不得提前平旧；旧合约管理与D5平仓只接受当日Bar，不得使用引擎缓存旧日Bar。D5旧Bar缺失时记录到期阻塞并于后续交易日重试；目标变化显式记录 `target_changed_reset`。
...
- 10次等待期退出已逐笔核对同产品旧合约真实Close；执行、风控退出、目标变化重置三种终态由合同门分别验证。本次历史目标变化重置为0次。
- 等待期间没有加仓或反向新开；退出后任务清空，不会在D5凭旧信号重新开仓。

## 验证与闸门

- 状态机/配置聚焦测试：`13 passed`；`py_compile`、`git diff --check`通过。
- 真引擎小样本验证：`si2501→si2502` 于2024-12-16登记，12月23日第五个后续交易日执行。
- 独立review初审发现：新主力缺Bar会在D0-D4提前平仓、D5可能使用旧日缓存Bar、取消任务证据仅为数量相减、目标变化会被误归类。以上均已修复并新增合同测试。
- 最终独立review：`P0/P1/P2=0/0/1`，唯一P2为本段记录待更新，现已闭环；相关测试 `46 passed + 14 subtests`。独立 `/tmp` 全周期三臂均为2098日且截至2026-08-25，summary/curve/delay/PNG SHA256分别为 `e5a284b9…/1312e34c…/df8e8acd…/032abc89…`，与共享产物逐字节一致；decision仅生成时间不同，去除该字段后一致。
- 运行环境事故：一次命令错误启用 `QMT_BACKTEST_ALLOW_NON_PROJECT_TRADER_DIR=1`，读到主仓旧库并把区间截断至2026-07-22；该批产物未提交，随后用 `PYTHONPATH=$PWD` 与项目运行守卫完整重跑恢复。回测器新增每臂 `analysis_end=2026-08-25` 硬门，后续截断将直接失败。
