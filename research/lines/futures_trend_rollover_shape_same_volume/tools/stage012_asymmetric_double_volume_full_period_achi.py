...
OUTPUT_DIR = LINE_DIR / "artifacts" / "stage012"

SUMMARY_PATH = OUTPUT_DIR / "stage012_achi_summary.csv"
COMPARISON_PATH = OUTPUT_DIR / "stage012_achi_comparison.csv"
CURVE_PATH = OUTPUT_DIR / "stage012_achi_curve.csv"
ENTRY_RISK_PATH = OUTPUT_DIR / "stage012_full_i_entry_risk.csv"
TRADES_PATH = OUTPUT_DIR / "stage012_full_i_trades.csv"
TRADE_EVENTS_PATH = OUTPUT_DIR / "stage012_full_i_trade_events.csv"
RISK_CONTRACT_PATH = OUTPUT_DIR / "stage012_full_i_risk_split_contract_summary.csv"
DECISION_PATH = OUTPUT_DIR / "stage012_decision.json"
CHART_PATH = OUTPUT_DIR / "stage012_full_period_equity_achi.png"
...
ARMS: tuple[dict[str, Any], ...] = (
    {
        ...
        "nonconfirmation_multiplier": 1.0,
    ...
    },
    {
        ...
        "confirmation_multiplier": 1.2,
        "nonconfirmation_multiplier": 1.0,
        "label": "H: C + 30D direction and volume >2x risk x1.2 otherwise x1.0",
        "plot_label": "H Hit 1.2 / Miss 1.0",
        "color": "#ea580c",
    },
    {
        "arm": "I",
        "profile": "stage012_I_double_volume_asymmetric_risk_15_05",
        "rollover_candidate": True,
        "risk_boost": True,
        "volume_confirmation": True,
        "volume_ratio_threshold": 2.0,
        "confirmation_multiplier": 1.5,
        "nonconfirmation_multiplier": 0.5,
        "label": "I: C + 30D direction and volume >2x risk x1.5 otherwise x0.5",
        "plot_label": "I Hit 1.5 / Miss 0.5",
        "color": "#059669",
    },
)
...
REUSED_ARMS = {"A", "C", "H"}
NEW_RUN_ARMS = {"I"}
REUSED_SOURCE_STAGE = "Stage010"
REUSED_SOURCE_COMMIT = "911b09166"
...
def _load_reused_arms() -> tuple[pd.DataFrame, pd.DataFrame]:
    summary = pd.read_csv(s10.SUMMARY_PATH)
    curve = pd.read_csv(s10.CURVE_PATH)
    ...
    if len(summary) != len(REUSED_ARMS) or set(summary["experiment_arm"].astype(str)) != REUSED_ARMS:
        raise RuntimeError("stage012_reused_summary_identity_mismatch")
    if set(curve["experiment_arm"].astype(str)) != REUSED_ARMS:
        raise RuntimeError("stage012_reused_curve_identity_mismatch")
    counts = curve.groupby("experiment_arm").size().to_dict()
    if len(set(counts.values())) != 1:
        raise RuntimeError("stage012_reused_curve_length_mismatch")
...
def _run_i_arm(
    ...
    arm = next(item for item in ARMS if item["arm"] == "I")
    ...
    summary["experiment_arm"] = "I"
    curve = curve.copy()
    curve["experiment_arm"] = "I"
...
def _risk_split_contract_summary(entry_risk: pd.DataFrame) -> pd.DataFrame:
    ...
    def summarize(group_type: str, group_value: str, group: pd.DataFrame) -> dict[str, Any]:
        ...
        confirmed = aligned & volume_confirmed
        applied = group["directional_30d_risk_boost_applied"].fillna(0).astype(int).eq(1)
        expected_multiplier = np.where(confirmed, 1.5, 0.5)
        ...
        nonconfirmation_exact = np.isclose(
            group["directional_30d_risk_nonconfirmation_multiplier"].to_numpy(dtype="float64"),
            0.5,
            rtol=0.0,
            atol=1e-12,
        )
        valid_risk = (
            applied.eq(confirmed)
        ...
        return {
            ...
            "confirmation_count": int(confirmed.sum()),
            "nonconfirmation_count": int((~confirmed).sum()),
            "threshold_contract_pass": int(
                len(group) > 0
                and bool(np.asarray(threshold_exact).all())
                and bool(np.asarray(nonconfirmation_exact).all())
            ),
...
def _decision(
    ...
    contract_gates = {
        ...
        "confirmation_branch_present": bool(int(total["confirmation_count"]) > 0),
        "nonconfirmation_branch_present": bool(int(total["nonconfirmation_count"]) > 0),
    }
    promotion_comparisons = {"A_vs_I", "C_vs_I"}
    ...
    return {
        ...
        "I_rule": {
            ...
            "risk_multiplier_when_both_confirm": 1.5,
            "risk_multiplier_otherwise": 0.5,
        ...
        "decision": (
            "run_asymmetric_double_volume_multicycle"
            if escalate
            else "stop_asymmetric_double_volume_risk_after_full_period"
        ),
...
def _validate_summary(summary: pd.DataFrame, curve: pd.DataFrame) -> None:
    ...
    if len(summary) != len(ARMS) or set(summary["experiment_arm"].astype(str)) != expected:
        raise RuntimeError("stage012_arm_identity_mismatch")
    critical = [
        *METRICS,
    ...
    if numeric.isna().any().any() or not np.isfinite(numeric.to_numpy(dtype="float64")).all():
        raise RuntimeError("stage012_critical_metric_invalid")
    ...
    if curve.empty or not np.isfinite(curve_equity.to_numpy(dtype="float64")).all():
        raise RuntimeError("stage012_curve_invalid")
    if set(curve["experiment_arm"].astype(str)) != expected:
        raise RuntimeError("stage012_curve_arm_identity_mismatch")
    counts = curve.groupby("experiment_arm").size().to_dict()
    if len(set(counts.values())) != 1:
        raise RuntimeError("stage012_curve_length_mismatch")


def _plot_full(curve: pd.DataFrame) -> bytes:
    fig, ax = plt.subplots(figsize=(14, 6))
    for arm in ARMS:
        frame = curve[curve["experiment_arm"].astype(str).eq(str(arm["arm"]))].sort_values(
            "date"
        )
        ax.plot(
            ...
            color=str(arm["color"]),
            lw=1.45,
            label=str(arm["plot_label"]),
        )
    ax.set_title("Stage012 Full-Period Equity: A/C/H vs I Hit 1.5 / Miss 0.5")
...
def _publish_atomically(
    ...
    temporary_dir = Path(tempfile.mkdtemp(prefix=".stage012.tmp-", dir=OUTPUT_DIR.parent))
    backup_dir = OUTPUT_DIR.with_name(f".stage012.backup-{uuid4().hex}")
    try:
...
def main() -> None:
    ...
    print("[stage012] arm=I", flush=True)
    i_summary, i_curve, i_frames = _run_i_arm(metadata)
    summary = pd.concat([reused_summary, i_summary], ignore_index=True, sort=False)
    curve = pd.concat([reused_curve, i_curve], ignore_index=True, sort=False)
    arm_order = {str(arm["arm"]): index for index, arm in enumerate(ARMS)}
    summary["_arm_order"] = summary["experiment_arm"].astype(str).map(arm_order)
    summary = summary.sort_values("_arm_order").drop(columns="_arm_order")
    curve["_arm_order"] = curve["experiment_arm"].astype(str).map(arm_order)
    curve = curve.sort_values(["_arm_order", "date"]).drop(columns="_arm_order")
    _validate_summary(summary, curve)
    comparison = _comparison(summary)
    entry_risk = i_frames.get("entry_risk", pd.DataFrame()).copy()
    trades = i_frames.get("trades", pd.DataFrame()).copy()
    trade_events = i_frames.get("trade_events", pd.DataFrame()).copy()
