...
import stage014_long_only_confirmation_only_full_period_achjk as s14


LINE_DIR = Path(__file__).resolve().parents[1]
OUTPUT_DIR = LINE_DIR / "artifacts" / "stage015"

SUMMARY_PATH = OUTPUT_DIR / "stage015_achkl_summary.csv"
COMPARISON_PATH = OUTPUT_DIR / "stage015_achkl_comparison.csv"
CURVE_PATH = OUTPUT_DIR / "stage015_achkl_curve.csv"
ENTRY_RISK_PATH = OUTPUT_DIR / "stage015_full_l_entry_risk.csv"
TRADES_PATH = OUTPUT_DIR / "stage015_full_l_trades.csv"
TRADE_EVENTS_PATH = OUTPUT_DIR / "stage015_full_l_trade_events.csv"
RISK_CONTRACT_PATH = OUTPUT_DIR / "stage015_full_l_risk_split_contract_summary.csv"
DECISION_PATH = OUTPUT_DIR / "stage015_decision.json"
CHART_PATH = OUTPUT_DIR / "stage015_full_period_equity_achkl.png"

ARMS: tuple[dict[str, Any], ...] = (
    {"arm": "A", "profile": "stage015_A_official_live_c9_15w", "label": "A: Official C9/150k", "plot_label": "A Official", "color": "#2563eb", "volume_ratio_threshold": 1.0, "confirmation_multiplier": 1.0, "nonconfirmation_multiplier": 1.0, "long_only": False},
    {"arm": "C", "profile": "stage015_C_rollover_continuation", "label": "C: Official + rollover continuation", "plot_label": "C Rollover", "color": "#dc2626", "volume_ratio_threshold": 1.0, "confirmation_multiplier": 1.0, "nonconfirmation_multiplier": 1.0, "long_only": False},
    {"arm": "H", "profile": "stage015_H_double_volume_hit_12", "label": "H: C + both-confirmed x1.2 otherwise x1.0", "plot_label": "H Hit 1.2 / Miss 1.0", "color": "#ea580c", "volume_ratio_threshold": 2.0, "confirmation_multiplier": 1.2, "nonconfirmation_multiplier": 1.0, "long_only": False},
    {"arm": "K", "profile": "stage015_K_long_hit_15_otherwise_10", "label": "K: C + long >2x volume x1.5 otherwise x1.0", "plot_label": "K Long >2x Hit 1.5", "color": "#0891b2", "volume_ratio_threshold": 2.0, "confirmation_multiplier": 1.5, "nonconfirmation_multiplier": 1.0, "long_only": True},
    {"arm": "L", "profile": "stage015_L_long_triple_volume_hit_15_otherwise_10", "label": "L: C + long >3x volume x1.5 otherwise x1.0", "plot_label": "L Long >3x Hit 1.5", "color": "#16a34a", "volume_ratio_threshold": 3.0, "confirmation_multiplier": 1.5, "nonconfirmation_multiplier": 1.0, "long_only": True},
)
...
def _load_reused_arms() -> tuple[pd.DataFrame, pd.DataFrame]:
    summary = pd.read_csv(s14.SUMMARY_PATH)
    curve = pd.read_csv(s14.CURVE_PATH)
    ...
    if len(summary) != len(REUSED_ARMS) or set(summary["experiment_arm"].astype(str)) != REUSED_ARMS:
        raise RuntimeError("stage015_reused_summary_identity_mismatch")
    if set(curve["experiment_arm"].astype(str)) != REUSED_ARMS:
        raise RuntimeError("stage015_reused_curve_identity_mismatch")
    counts = curve.groupby("experiment_arm").size().to_dict()
    if len(set(counts.values())) != 1:
        raise RuntimeError("stage015_reused_curve_length_mismatch")
    return summary, curve


def _run_l_arm(metadata: dict[str, Any]) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, pd.DataFrame]]:
    arm = next(item for item in ARMS if item["arm"] == "L")
    ...
    summary["experiment_arm"] = "L"
    curve = curve.copy()
    curve["experiment_arm"] = "L"
...
def _risk_split_contract_summary(entry_risk: pd.DataFrame) -> pd.DataFrame:
    ...
    def summarize(group_type: str, group_value: str, group: pd.DataFrame) -> dict[str, Any]:
        ...
        short_bypass = (
            ...
            & np.isclose(group["directional_30d_risk_boost_multiplier"], 1.0, rtol=0.0, atol=1e-12)
        )
...
def _decision(comparison: pd.DataFrame, risk_contract: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        ...
        "L_rule": {
            ...
            "volume_condition": "recent 10-day sum strictly greater than 3.0 times prior 10-day sum",
        ...
        "decision": "run_long_only_triple_volume_multicycle" if escalate else "stop_long_only_triple_volume_after_full_period",
...
def _validate_summary(summary: pd.DataFrame, curve: pd.DataFrame) -> None:
    ...
    if len(summary) != len(ARMS) or set(summary["experiment_arm"].astype(str)) != expected:
        raise RuntimeError("stage015_arm_identity_mismatch")
    ...
    if numeric.isna().any().any() or not np.isfinite(numeric.to_numpy(dtype="float64")).all():
        raise RuntimeError("stage015_critical_metric_invalid")
    ...
    if curve.empty or not np.isfinite(curve_equity.to_numpy(dtype="float64")).all():
        raise RuntimeError("stage015_curve_invalid")
    if set(curve["experiment_arm"].astype(str)) != expected:
        raise RuntimeError("stage015_curve_arm_identity_mismatch")
    counts = curve.groupby("experiment_arm").size().to_dict()
    if len(set(counts.values())) != 1:
        raise RuntimeError("stage015_curve_length_mismatch")
...
def _plot_full(curve: pd.DataFrame) -> bytes:
    ...
    ax.set_title("Stage015 Full-Period Equity: L Long >3x Volume Hit 1.5")
...
def _publish_atomically(frames: dict[str, pd.DataFrame], decision: dict[str, Any], chart: bytes) -> None:
    ...
    temporary_dir = Path(tempfile.mkdtemp(prefix=".stage015.tmp-", dir=OUTPUT_DIR.parent))
    backup_dir = OUTPUT_DIR.with_name(f".stage015.backup-{uuid4().hex}")
    try:
...
def main() -> None:
    ...
    l_summary, l_curve, frames = _run_l_arm(metadata)
    summary = pd.concat([reused_summary, l_summary], ignore_index=True, sort=False)
    curve = pd.concat([reused_curve, l_curve], ignore_index=True, sort=False)
