...
OUTPUT_DIR = LINE_DIR / "artifacts" / "stage016"
...
SUMMARY_PATH = OUTPUT_DIR / "stage016_window_summary.csv"
COMPARISON_PATH = OUTPUT_DIR / "stage016_window_comparison.csv"
AGGREGATE_PATH = OUTPUT_DIR / "stage016_cycle_aggregate.csv"
CURVE_PATH = OUTPUT_DIR / "stage016_equity_curves.csv"
L_ENTRY_RISK_PATH = OUTPUT_DIR / "stage016_full_l_entry_risk.csv"
L_TRADES_PATH = OUTPUT_DIR / "stage016_full_l_trades.csv"
L_TRADE_EVENTS_PATH = OUTPUT_DIR / "stage016_full_l_trade_events.csv"
RISK_CONTRACT_PATH = OUTPUT_DIR / "stage016_full_l_risk_split_contract_summary.csv"
DECISION_PATH = OUTPUT_DIR / "stage016_decision.json"
CHART_FILES = {
    "full_period": "stage016_full_period_equity_ackl.png",
    "1y": "stage016_equity_curves_1y_ackl.png",
    "2y": "stage016_equity_curves_2y_ackl.png",
    "3y": "stage016_equity_curves_3y_ackl.png",
    "aggregate": "stage016_cycle_aggregate_ackl.png",
}

ARMS: tuple[dict[str, Any], ...] = (
    {"arm": "A", "profile": "stage016_A_official_live_c9_15w", "label": "A: Official C9/150k", "plot_label": "A Official", "color": "#2563eb", "volume_ratio_threshold": 1.0, "confirmation_multiplier": 1.0, "nonconfirmation_multiplier": 1.0, "long_only": False},
    {"arm": "C", "profile": "stage016_C_rollover_continuation", "label": "C: Official + rollover continuation", "plot_label": "C Rollover", "color": "#dc2626", "volume_ratio_threshold": 1.0, "confirmation_multiplier": 1.0, "nonconfirmation_multiplier": 1.0, "long_only": False},
    {"arm": "K", "profile": "stage016_K_long_hit_15_otherwise_10", "label": "K: C + long >2x volume x1.5 otherwise x1.0", "plot_label": "K Long >2x Hit 1.5", "color": "#0891b2", "volume_ratio_threshold": 2.0, "confirmation_multiplier": 1.5, "nonconfirmation_multiplier": 1.0, "long_only": True},
    {"arm": "L", "profile": "stage016_L_long_triple_volume_hit_15_otherwise_10", "label": "L: C + long >3x volume x1.5 otherwise x1.0", "plot_label": "L Long >3x Hit 1.5", "color": "#16a34a", "volume_ratio_threshold": 3.0, "confirmation_multiplier": 1.5, "nonconfirmation_multiplier": 1.0, "long_only": True},
)
REUSED_ARMS = {"A", "C"}
NEW_RUN_ARMS = {"K", "L"}
...
def _load_reused_arms() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if len(summary) != len(expected) or summary_pairs != expected:
        raise RuntimeError("stage016_reused_summary_identity_mismatch")
    if curve_pairs != expected:
        raise RuntimeError("stage016_reused_curve_identity_mismatch")
...
def _verify_full_identity(summary: pd.DataFrame, curve: pd.DataFrame) -> None:
    source_summary = pd.read_csv(s15.SUMMARY_PATH).set_index("experiment_arm")
    source_curve = pd.read_csv(s15.CURVE_PATH)
    ...
    for arm in ("A", "C", "K", "L"):
        if not np.allclose(
            ...
            raise RuntimeError(f"stage016_full_summary_drift:{arm}")
        left = curve[
        ...
        if not _csv_equity_values_match(
            ...
            raise RuntimeError(f"stage016_full_curve_equity_drift:{arm}")
...
def _decision(
    ...
    promotion_comparisons = {"A_vs_L", "C_vs_L"}
    ...
    return {
        ...
        "diagnostic_override_reason": "user_requested_multicycle_after_stage015_full_period_gate_failure",
        "gate_contract": {
            ...
            "K_rule": "long aligned and recent 10d volume >2x prior 10d: x1.5; otherwise x1.0; shorts x1.0",
            "L_rule": "long aligned and recent 10d volume >3x prior 10d: x1.5; otherwise x1.0; shorts x1.0",
        ...
        "run_provenance": {
            ...
            "full_period_identity_verified_against_stage015": True,
        },
        ...
        "long_only_triple_volume_all_gates_pass": all_pass,
        "decision": (
            "multicycle_diagnostic_supports_new_predeclared_review"
            if all_pass
            else "confirm_long_only_triple_volume_not_promotable_after_multicycle"
        ),
...
def _plot_full(curves: pd.DataFrame) -> bytes:
    ...
    ax.set_title("Stage016 Full-Period Equity: A / C / K / L")
...
def _plot_window_grid(curves: pd.DataFrame, comparison: pd.DataFrame, years: int) -> bytes:
    ...
    fig.suptitle(f"Stage016 {years}-Year Independent Equity Curves: January + June Starts", y=0.998, fontsize=15)
...
def _plot_aggregate(aggregate: pd.DataFrame) -> bytes:
    ...
    fig.suptitle("Stage016 Multi-Cycle A/C/K/L: Combined, January, June", fontsize=15)
...
def _publish_atomically(frames: dict[str, pd.DataFrame], decision: dict[str, Any], charts: dict[str, bytes]) -> None:
    ...
    temporary_dir = Path(tempfile.mkdtemp(prefix=".stage016.tmp-", dir=OUTPUT_DIR.parent))
    backup_dir = OUTPUT_DIR.with_name(f".stage016.backup-{uuid4().hex}")
    try:
...
def main() -> None:
    ...
    full_l_frames: dict[str, pd.DataFrame] = {}
    ...
    for window in WINDOWS:
        for arm in ARMS:
            if str(arm["arm"]) not in NEW_RUN_ARMS:
                continue
            run_index += 1
            print(f"[stage016] {run_index}/{run_total} {window['window_id']} arm={arm['arm']}", flush=True)
            summary, curve, frames = s13._run_candidate_window(metadata, window, arm)
            new_summaries.append(summary)
            new_curves.append(curve)
            if str(window["window_id"]) == "full_2018_2026" and str(arm["arm"]) == "L":
                full_l_frames = {key: value.copy() for key, value in frames.items()}
    ...
    entry_risk = full_l_frames.get("entry_risk", pd.DataFrame()).copy()
    trades = full_l_frames.get("trades", pd.DataFrame()).copy()
    trade_events = full_l_frames.get("trade_events", pd.DataFrame()).copy()
    contract = s15._risk_split_contract_summary(entry_risk)
    ...
    _publish_atomically(
        {
            ...
            L_ENTRY_RISK_PATH.name: entry_risk,
            L_TRADES_PATH.name: trades,
            L_TRADE_EVENTS_PATH.name: trade_events,
            RISK_CONTRACT_PATH.name: contract,
        },
        decision,
...
def rebuild_from_published() -> None:
    ...
    comparison = _comparison(summary)
    aggregate = _aggregate(comparison)
    entry_risk = pd.read_csv(L_ENTRY_RISK_PATH)
    contract = s15._risk_split_contract_summary(entry_risk)
    decision = _decision(comparison, aggregate, contract)
    _publish_atomically(
        {
            ...
            CURVE_PATH.name: curve,
            L_ENTRY_RISK_PATH.name: entry_risk,
            L_TRADES_PATH.name: pd.read_csv(L_TRADES_PATH),
            L_TRADE_EVENTS_PATH.name: pd.read_csv(L_TRADE_EVENTS_PATH),
            RISK_CONTRACT_PATH.name: contract,
        },
        decision,
        _charts(curve, comparison, aggregate),
