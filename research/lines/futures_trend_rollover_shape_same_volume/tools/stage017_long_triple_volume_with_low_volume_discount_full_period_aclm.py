...
import stage015_long_only_triple_volume_full_period_achkl as s15


LINE_DIR = Path(__file__).resolve().parents[1]
OUTPUT_DIR = LINE_DIR / "artifacts" / "stage017"

SUMMARY_PATH = OUTPUT_DIR / "stage017_aclm_summary.csv"
COMPARISON_PATH = OUTPUT_DIR / "stage017_aclm_comparison.csv"
CURVE_PATH = OUTPUT_DIR / "stage017_aclm_curve.csv"
ENTRY_RISK_PATH = OUTPUT_DIR / "stage017_full_m_entry_risk.csv"
TRADES_PATH = OUTPUT_DIR / "stage017_full_m_trades.csv"
TRADE_EVENTS_PATH = OUTPUT_DIR / "stage017_full_m_trade_events.csv"
RISK_CONTRACT_PATH = OUTPUT_DIR / "stage017_full_m_risk_split_contract_summary.csv"
DECISION_PATH = OUTPUT_DIR / "stage017_decision.json"
CHART_PATH = OUTPUT_DIR / "stage017_full_period_equity_aclm.png"

ARMS: tuple[dict[str, Any], ...] = (
    {"arm": "A", "profile": "stage017_A_official_live_c9_15w", "label": "A: Official C9/150k", "plot_label": "A Official", "color": "#2563eb", "high_volume_ratio_threshold": 1.0, "high_volume_multiplier": 1.0, "low_volume_ratio_threshold": 0.0, "low_volume_multiplier": 1.0, "long_only": False},
    {"arm": "C", "profile": "stage017_C_rollover_continuation", "label": "C: Official + rollover continuation", "plot_label": "C Rollover", "color": "#dc2626", "high_volume_ratio_threshold": 1.0, "high_volume_multiplier": 1.0, "low_volume_ratio_threshold": 0.0, "low_volume_multiplier": 1.0, "long_only": False},
    {"arm": "L", "profile": "stage017_L_long_triple_volume_hit_15_otherwise_10", "label": "L: C + long >3x volume x1.5 otherwise x1.0", "plot_label": "L Long >3x Hit 1.5", "color": "#16a34a", "high_volume_ratio_threshold": 3.0, "high_volume_multiplier": 1.5, "low_volume_ratio_threshold": 0.0, "low_volume_multiplier": 1.0, "long_only": True},
    {"arm": "M", "profile": "stage017_M_long_triple_volume_hit_15_low_half_05", "label": "M: C + long >3x x1.5, <0.5x x0.5, otherwise x1.0", "plot_label": "M Long >3x 1.5 / <0.5x 0.5", "color": "#7c3aed", "high_volume_ratio_threshold": 3.0, "high_volume_multiplier": 1.5, "low_volume_ratio_threshold": 0.5, "low_volume_multiplier": 0.5, "long_only": True},
)

REUSED_ARMS = {"A", "C", "L"}
NEW_RUN_ARMS = {"M"}
REUSED_SOURCE_STAGE = "Stage015"
...
def _load_reused_arms() -> tuple[pd.DataFrame, pd.DataFrame]:
    summary = pd.read_csv(s15.SUMMARY_PATH)
    curve = pd.read_csv(s15.CURVE_PATH)
    ...
    if len(summary) != len(REUSED_ARMS) or set(summary["experiment_arm"].astype(str)) != REUSED_ARMS:
        raise RuntimeError("stage017_reused_summary_identity_mismatch")
    if set(curve["experiment_arm"].astype(str)) != REUSED_ARMS:
        raise RuntimeError("stage017_reused_curve_identity_mismatch")
    counts = curve.groupby("experiment_arm").size().to_dict()
    if len(set(counts.values())) != 1:
        raise RuntimeError("stage017_reused_curve_length_mismatch")
    return summary, curve


def _run_m_arm(metadata: dict[str, Any]) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, pd.DataFrame]]:
    arm = next(item for item in ARMS if item["arm"] == "M")
    ...
    summary["experiment_arm"] = "M"
    curve = curve.copy()
    curve["experiment_arm"] = "M"
    return summary, curve, frames


def _comparison(summary: pd.DataFrame) -> pd.DataFrame:
    original = s15.s10.COMPARISONS
    try:
        s15.s10.COMPARISONS = COMPARISONS
        return s15.s10._comparison(summary)
    finally:
        s15.s10.COMPARISONS = original
...
def _risk_split_contract_summary(entry_risk: pd.DataFrame) -> pd.DataFrame:
    ...
    empty = {
        ...
        "long_high_volume_count": 0, "long_low_volume_count": 0,
        "long_base_volume_count": 0, "short_bypass_count": 0,
    ...
    def summarize(group_type: str, group_value: str, group: pd.DataFrame) -> dict[str, Any]:
        ...
        long_high = long_mask & aligned & high_volume
        long_low = long_mask & low_volume
        long_base = long_mask & ~long_high & ~long_low
        short_bypass = (
            short_mask
        ...
        expected_multiplier = np.select([long_low, long_high], [0.5, 1.5], default=1.0)
        ...
        risk_ok = (
            boost_applied.eq(long_high)
            & discount_applied.eq(long_low)
...
def _decision(comparison: pd.DataFrame, risk_contract: pd.DataFrame) -> dict[str, Any]:
    ...
    for comparison_name, _, _ in COMPARISONS:
        ...
        gates = s15.s7._full_period_gates(row)
    ...
    promotion_comparisons = {"A_vs_M", "C_vs_M"}
    ...
    return {
        ...
        "run_provenance": {
            "candidate_freeze_commit": s15.s13._git_head(),
        ...
        "M_rule": {
            ...
            "long_high_volume_condition": "direction aligned and recent sum strictly greater than 3.0 times prior sum",
            "long_high_volume_risk_multiplier": 1.5,
            "long_low_volume_condition": "recent sum strictly less than 0.5 times prior sum, independent of 30d direction alignment",
            "long_low_volume_risk_multiplier": 0.5,
            "long_other_risk_multiplier": 1.0,
        ...
        "decision": "run_long_triple_volume_with_low_volume_discount_multicycle" if escalate else "stop_long_triple_volume_with_low_volume_discount_after_full_period",
...
def _validate_summary(summary: pd.DataFrame, curve: pd.DataFrame) -> None:
    ...
    if len(summary) != len(ARMS) or set(summary["experiment_arm"].astype(str)) != expected:
        raise RuntimeError("stage017_arm_identity_mismatch")
    ...
    if numeric.isna().any().any() or not np.isfinite(numeric.to_numpy(dtype="float64")).all():
        raise RuntimeError("stage017_critical_metric_invalid")
    ...
    if curve.empty or not np.isfinite(curve_equity.to_numpy(dtype="float64")).all():
        raise RuntimeError("stage017_curve_invalid")
    if set(curve["experiment_arm"].astype(str)) != expected:
        raise RuntimeError("stage017_curve_arm_identity_mismatch")
    counts = curve.groupby("experiment_arm").size().to_dict()
    if len(set(counts.values())) != 1:
        raise RuntimeError("stage017_curve_length_mismatch")
...
def _plot_full(curve: pd.DataFrame) -> bytes:
    ...
    ax.set_title("Stage017 Full-Period Equity: Long >3x 1.5 / <0.5x 0.5")
...
def _publish_atomically(frames: dict[str, pd.DataFrame], decision: dict[str, Any], chart: bytes) -> None:
    ...
    temporary_dir = Path(tempfile.mkdtemp(prefix=".stage017.tmp-", dir=OUTPUT_DIR.parent))
    backup_dir = OUTPUT_DIR.with_name(f".stage017.backup-{uuid4().hex}")
    try:
...
def main() -> None:
    ...
    m_summary, m_curve, frames = _run_m_arm(metadata)
    summary = pd.concat([reused_summary, m_summary], ignore_index=True, sort=False)
    curve = pd.concat([reused_curve, m_curve], ignore_index=True, sort=False)
