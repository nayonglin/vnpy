...
OUTPUT_DIR = LINE_DIR / "artifacts" / "stage019"
SUMMARY_PATH = OUTPUT_DIR / "stage019_acmo_summary.csv"
COMPARISON_PATH = OUTPUT_DIR / "stage019_acmo_comparison.csv"
CURVE_PATH = OUTPUT_DIR / "stage019_acmo_curve.csv"
ENTRY_RISK_PATH = OUTPUT_DIR / "stage019_full_o_entry_risk.csv"
TRADES_PATH = OUTPUT_DIR / "stage019_full_o_trades.csv"
TRADE_EVENTS_PATH = OUTPUT_DIR / "stage019_full_o_trade_events.csv"
ATR_FILTER_PATH = OUTPUT_DIR / "stage019_full_o_long_signal_atr_shock.csv"
ATR_CONTRACT_PATH = OUTPUT_DIR / "stage019_full_o_atr_filter_contract_summary.csv"
DECISION_PATH = OUTPUT_DIR / "stage019_decision.json"
CHART_PATH = OUTPUT_DIR / "stage019_full_period_equity_acmo.png"

ARMS: tuple[dict[str, Any], ...] = (
    {"arm": "A", "profile": "stage019_A_official_live_c9_15w", "label": "A: Official C9/150k", "plot_label": "A Official", "color": "#2563eb"},
    {"arm": "C", "profile": "stage019_C_rollover_continuation", "label": "C: Official + rollover continuation", "plot_label": "C Rollover", "color": "#dc2626"},
    {"arm": "M", "profile": "stage019_M_long_triple_volume_hit_15_low_half_05", "label": "M: Long >3x x1.5, <0.5x x0.5; short x1.0", "plot_label": "M Long Only", "color": "#7c3aed"},
    {
        "arm": "O",
        "profile": "stage019_O_m_plus_long_atr5_2x_shock_filter",
        "label": "O: M + block long entry when signal-day drop > 2x prior ATR5",
        "plot_label": "O M + Long ATR Shock",
        "color": "#f59e0b",
        "atr_period": 5,
        "atr_multiplier": 2.0,
...
def _load_reused_arms() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if len(summary) != len(REUSED_ARMS) or set(summary["experiment_arm"].astype(str)) != REUSED_ARMS:
        raise RuntimeError("stage019_reused_summary_identity_mismatch")
    if set(curve["experiment_arm"].astype(str)) != REUSED_ARMS:
        raise RuntimeError("stage019_reused_curve_identity_mismatch")
    if len(set(curve.groupby("experiment_arm").size().to_dict().values())) != 1:
        raise RuntimeError("stage019_reused_curve_length_mismatch")
    return summary, curve


def _run_o_arm(metadata: dict[str, Any]) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, pd.DataFrame]]:
    arm = next(item for item in ARMS if item["arm"] == "O")
    ...
    summary["experiment_arm"] = "O"
    curve = curve.copy()
    curve["experiment_arm"] = "O"
...
def _atr_filter_contract_summary(diagnostics: pd.DataFrame) -> pd.DataFrame:
    ...
    def summarize(group_type: str, group_value: str, group: pd.DataFrame) -> dict[str, Any]:
        ...
        config_ok = (
            ...
            & np.isclose(group["long_signal_atr_shock_multiplier"], 2.0, rtol=0.0, atol=1e-12)
        )
        relevant_threshold_ok = ~valid | np.isclose(
            group["long_signal_atr_shock_threshold"], group["long_signal_atr_shock_atr"] * 2.0,
        ...
        return {
            ...
            "configuration_contract_pass": int(len(group) > 0 and bool(np.asarray(config_ok & relevant_threshold_ok).all())),
...
def _decision(comparison: pd.DataFrame, contract: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        ...
        "O_rule": {
            ...
            "condition": "block only when signal-day drop is strictly greater than 2.0 times ATR5",
            "invalid_or_insufficient_history": "keep M behavior",
            "excluded": "short entries, all adds, C9 stop retry",
        },
        ...
        "decision": "run_m_long_atr_shock_filter_multicycle" if escalate else "stop_m_long_atr_shock_filter_after_full_period",
...
def _validate_summary(summary: pd.DataFrame, curve: pd.DataFrame) -> None:
    ...
    if len(summary) != len(ARMS) or set(summary["experiment_arm"].astype(str)) != expected:
        raise RuntimeError("stage019_arm_identity_mismatch")
    ...
    if numeric.isna().any().any() or not np.isfinite(numeric.to_numpy(dtype="float64")).all():
        raise RuntimeError("stage019_critical_metric_invalid")
    ...
    if curve.empty or not np.isfinite(equity.to_numpy(dtype="float64")).all():
        raise RuntimeError("stage019_curve_invalid")
    if set(curve["experiment_arm"].astype(str)) != expected or len(set(curve.groupby("experiment_arm").size())) != 1:
        raise RuntimeError("stage019_curve_identity_or_length_mismatch")
...
def _plot_full(curve: pd.DataFrame) -> bytes:
    ...
    ax.set_title("Stage019 Full-Period Equity: M + Long Signal-Day ATR Shock Filter")
...
def _publish_atomically(frames: dict[str, pd.DataFrame], decision: dict[str, Any], chart: bytes) -> None:
    ...
    temporary_dir = Path(tempfile.mkdtemp(prefix=".stage019.tmp-", dir=OUTPUT_DIR.parent))
    backup_dir = OUTPUT_DIR.with_name(f".stage019.backup-{uuid4().hex}")
    try:
...
def main() -> None:
    ...
    o_summary, o_curve, frames = _run_o_arm(metadata)
    summary = pd.concat([reused_summary, o_summary], ignore_index=True, sort=False)
    curve = pd.concat([reused_curve, o_curve], ignore_index=True, sort=False)
