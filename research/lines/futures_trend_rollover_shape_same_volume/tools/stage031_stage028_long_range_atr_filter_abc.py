...
import qmt_roll_candidate_stage031_long_range_atr_filter_config as stage031_cfg  # noqa: E402
...
OUTPUT_DIR = LINE_DIR / "artifacts" / "stage031"
...
ARMS: tuple[dict[str, str], ...] = (
    ...
    {
        "arm": "B",
        "profile": "stage031_B_stage028_delay_5td",
    ...
    {
        "arm": "C",
        "profile": "stage031_C_stage028_long_range_3atr_filter",
        "label": "C: Stage028 + 多头10日区间>3×前置ATR5禁开",
        "plot_label": "C Stage031: B + long range > 3 ATR5 filter",
        "color": "#16a34a",
    },
)

METRICS = s28.METRICS
SUMMARY_NAME = "stage031_abc_summary.csv"
COMPARISON_NAME = "stage031_abc_comparison.csv"
CURVE_NAME = "stage031_abc_curve.csv"
FILTER_NAME = "stage031_long_range_atr_diagnostics.csv"
FILTER_CONTRACT_NAME = "stage031_long_range_atr_contract_summary.csv"
ROLLOVER_NAME = "stage031_rollover_diagnostics.csv"
DELAY_NAME = "stage031_delay_diagnostics.csv"
TRADES_NAME = "stage031_trades.csv"
TRADE_EVENTS_NAME = "stage031_trade_events.csv"
DECISION_NAME = "stage031_decision.json"
REPORT_NAME = "stage031_report.md"
CHART_NAME = "stage031_full_period_equity_abc.png"
...
def build_arm_overrides(arm: str) -> dict[str, Any]:
    ...
    if arm == "C":
        return stage031_cfg.build_candidate_overrides()
    raise ValueError(f"unknown stage031 arm:{arm}")
...
def _preflight() -> dict[str, Any]:
    subprocess.run(
        ["git", "merge-base", "--is-ancestor", stage031_cfg.BASE_COMMIT, "HEAD"],
        cwd=PROJECT_DIR,
        check=True,
    )
    ...
    if diff != _expected_override_diff():
        raise RuntimeError(f"stage031_override_scope_drift:{diff}")
    runtime = s29._assert_runtime_database_binding()
    database_path = Path(runtime["database_path"])
    return {
        **identity,
        "stage028_base_commit": stage031_cfg.BASE_COMMIT,
        "stage028_to_stage031_override_diff": diff,
        "runtime_binding": {
            **runtime,
...
def _assert_full_period_coverage(summary: pd.DataFrame, curve: pd.DataFrame) -> None:
    ...
    if len(summary) != 3 or set(summary["experiment_arm"].astype(str)) != expected_arms:
        raise RuntimeError("stage031_summary_arm_identity_failed")
    for row in summary.itertuples(index=False):
        ...
        if actual_start != EXPECTED_FIRST_TRADING_DAY or actual_end != END:
            raise RuntimeError(
                f"stage031_full_period_coverage_failed:{row.experiment_arm}:"
        ...
        if dates.duplicated().any():
            raise RuntimeError(f"stage031_duplicate_curve_date:{row.experiment_arm}")
        if len(dates) != 2098 or dates.min() != actual_start or dates.max() != actual_end:
            raise RuntimeError(f"stage031_curve_coverage_failed:{row.experiment_arm}")
...
def _verify_stage028_reproduction(
    ...
    for old_arm, new_arm in (("A", "A"), ("C", "B")):
        ...
        result["pairs"][f"stage028_{old_arm}_to_stage031_{new_arm}"] = {
...
def _filter_contract(diagnostics: pd.DataFrame) -> dict[str, Any]:
    ...
    valid = frame[
        frame["long_signal_range_atr_reason"].astype(str).isin(
            {"range_strictly_above_threshold", "range_not_above_threshold"}
        )
    ]
    ...
    result = {
        ...
        "positive_unblocked_threshold_pass": bool(
            positive_unblocked.empty
            or positive_unblocked["long_signal_range_value"]
            .le(positive_unblocked["long_signal_range_atr_threshold"])
            .all()
        ),
    ...
    result["all_pass"] = bool(
        ...
        and result["positive_unblocked_threshold_pass"]
...
def _plot(curve: pd.DataFrame) -> bytes:
    ...
    ax.set_title("Stage031: Formal Q vs Stage028 +5TD vs Long 10D Range > 3 ATR5 Filter")
...
def _publish(
    ...
    temporary = Path(tempfile.mkdtemp(prefix=".stage031.tmp-", dir=OUTPUT_DIR.parent))
    backup = OUTPUT_DIR.with_name(f".stage031.backup-{uuid4().hex}")
    try:
...
def main() -> None:
    ...
    for arm in ARMS:
        print(f"[stage031] running {arm['arm']} {START.date()}->{END.date()}", flush=True)
    ...
    if not reproduction["all_pass"]:
        raise RuntimeError(f"stage031_stage028_reproduction_failed:{reproduction}")
    ...
    if not history_b["all_pass"] or not history_c["all_pass"] or not delay_c["all_pass"]:
        raise RuntimeError("stage031_stage028_contract_failed")
    filter_contract = _filter_contract(frames["long_signal_range_atr"])
    if not filter_contract["all_pass"]:
        raise RuntimeError(f"stage031_filter_contract_failed:{filter_contract}")
    ...
    decision = {
        ...
        "candidate_version": stage031_cfg.CANDIDATE_VERSION,
        ...
        "decision": (
            "stage031_long_range_3atr_pass_full_period_run_multicycle"
            if escalate
            else "stage031_long_range_3atr_fail_full_period_stop"
        ),
