...
- 修改脚本：
  ...
  - `run_qmt_roll_stage947_official_live_production_support_launcher.py`：先持久化 receipt candidate 再更新内存状态；resolver 前先落 provisional canonical receipt，resolver 失败可形成唯一 failed root receipt；最后阶段成功与 terminal succeeded receipt 合并为一次事务写；terminal failed receipt 使用写后读取确认，只有未落盘时才受控重试一次。
...
- TDD RED：Stage935 专项测试未绑定 release surface；第二次、terminal succeeded 或 terminal failed receipt 写失败后 latest 残留 `running`；resolver 失败后 watchdog 形成第二通知身份；secret sentinel 可进入 `root_stage`。
- TDD GREEN：六个定向用例全部通过。
- 相关完整回归：`111 passed, 56 subtests passed`。
...
- orders：无。
- daily：无。
- quality：`111 passed, 56 subtests passed`；`git diff --check` 待提交前复核。
