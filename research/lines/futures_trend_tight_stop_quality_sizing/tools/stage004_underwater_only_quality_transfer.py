...
STAGE = "Stage004"
MODEL_TAG = "stage004_underwater_only_quality_transfer_v1"
PROFILE_NAME = "stage004_underwater_only_quality_transfer"
OUTPUT_PREFIX = "tight_stop_quality_stage004"
STARTS = s003.STARTS
END = s003.END
EXPECTED_CAPITAL = s003.EXPECTED_CAPITAL
VARIANT = "C_stage004"

OUT = LINE_DIR / "outputs" / "stage004_underwater_only_quality_transfer"
...
STAGE004_AUDIT_FIELDS = (
    "stage004_enabled",
    "stage004_portfolio_drawdown_pct",
    "stage004_underwater_gate_active",
    "stage004_gate_reason",
    "stage004_budget_weight",
    "stage004_account_equity",
    "stage004_account_high_water",
    "stage004_drawdown_source",
)
...
def _json_safe(value: Any) -> Any:
    return s003._json_safe(value)


def _md_table(frame: pd.DataFrame) -> str:
    return s003._md_table(frame)


def underwater_gate_active(*, enabled: bool, entry_context: str, portfolio_drawdown_pct: float) -> tuple[bool, str]:
    if not enabled:
        return False, "disabled"
    if entry_context != "flat_entry":
        return False, "non_flat_entry"
    if float(portfolio_drawdown_pct) <= 0.0:
        return False, "high_water_unchanged"
    return True, "underwater_quality_transfer"


def _stage004_payload(sizing_snapshot: dict[str, Any]) -> dict[str, Any]:
    return {key: sizing_snapshot.get(key) for key in STAGE004_AUDIT_FIELDS}


class QmtRollPortfolioStrategyStage004UnderwaterOnly(s003.QmtRollPortfolioStrategyStage003RiskTransfer):
    enable_stage004_underwater_only: bool = False
    ...
    def _calculate_entry_sizing(self, *args: Any, **kwargs: Any) -> dict[str, Any]:
        ...
        drawdown = max(0.0, float(getattr(self, "stage000_account_drawdown_pct", 0.0) or 0.0))
        gate_active, gate_reason = underwater_gate_active(
            enabled=bool(self.enable_stage004_underwater_only),
        ...
        result.update(
            {
                "stage004_enabled": int(bool(self.enable_stage004_underwater_only)),
                "stage004_portfolio_drawdown_pct": drawdown,
                "stage004_underwater_gate_active": int(gate_active),
                "stage004_gate_reason": gate_reason,
                "stage004_budget_weight": float(result.get("stage003_budget_weight", 1.0) or 1.0),
                "stage004_account_equity": float(getattr(self, "stage000_account_equity", 0.0) or 0.0),
                "stage004_account_high_water": float(getattr(self, "stage000_account_high_water", 0.0) or 0.0),
                "stage004_drawdown_source": "engine_trades_marked_to_signal_close",
            }
        )
        return result
    ...
    def _record_entry_candidate_snapshot(self, *args: Any, **kwargs: Any) -> None:
        ...
        if isinstance(sizing_snapshot, dict) and self.entry_candidate_snapshots:
            self.entry_candidate_snapshots[-1].update(_stage004_payload(sizing_snapshot))
    ...
    def _record_entry_risk_diagnostic(self, *args: Any, **kwargs: Any) -> None:
        ...
        if isinstance(sizing_snapshot, dict) and self.entry_risk_diagnostics:
            self.entry_risk_diagnostics[-1].update(_stage004_payload(sizing_snapshot))
...
def _candidate_profile(metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    result["strategy_cls"] = QmtRollPortfolioStrategyStage004UnderwaterOnly
    result["spec"] = replace(
        spec,
        overrides={**spec.overrides, "enable_stage004_underwater_only": True},
...
def config_audit(metadata: dict[str, Any]) -> pd.DataFrame:
    ...
    for key in sorted(set(a_overrides) | set(c_overrides)):
        ...
        if changed and key not in allowed:
            raise RuntimeError(f"unexpected Stage004 A/C override difference: {key}")
...
def validate_feature_evidence(candidates: pd.DataFrame) -> dict[str, Any]:
    required = set(s003.STAGE003_AUDIT_FIELDS) | set(STAGE004_AUDIT_FIELDS)
    missing = sorted(required - set(candidates.columns))
    if candidates.empty or missing:
        raise RuntimeError(f"Stage004 feature evidence missing: rows={len(candidates)}, columns={missing}")
    if not candidates["stage004_enabled"].eq(1).all():
        raise RuntimeError("Stage004 candidate evidence contains disabled rows")
    if not candidates["stage004_drawdown_source"].astype(str).eq(
        "engine_trades_marked_to_signal_close"
    ).all():
        raise RuntimeError("Stage004 drawdown source is not strict account equity")
    ...
    if candidate_date.isna().any() or feature_date.isna().any() or not candidate_date.eq(feature_date).all():
        raise RuntimeError("Stage004 feature date is not signal date/T-1")
    drawdown = pd.to_numeric(candidates["stage004_portfolio_drawdown_pct"], errors="coerce")
    active = pd.to_numeric(candidates["stage004_underwater_gate_active"], errors="coerce").fillna(-1).astype(int)
    expected_active = drawdown.gt(0.0).astype(int)
    if drawdown.isna().any() or not active.eq(expected_active).all():
        raise RuntimeError("Stage004 underwater gate is not causal/equivalent to drawdown > 0")
    weight = pd.to_numeric(candidates["stage004_budget_weight"], errors="coerce")
    inactive = active.eq(0)
    if not np.allclose(weight[inactive], 1.0, rtol=0.0, atol=1e-12):
        raise RuntimeError("Stage004 changed risk at the high-water mark")
    if not candidates.loc[inactive, "stage004_gate_reason"].astype(str).eq("high_water_unchanged").all():
        raise RuntimeError("Stage004 high-water reason mismatch")
    active_reason = candidates.loc[active.eq(1), "stage003_reason"].astype(str)
    allowed_active_reasons = {
        "quality_risk_increase",
        "other_risk_decrease",
        "recovery_sleeve_exempt",
        "feature_unavailable_fail_unchanged",
    }
    if not set(active_reason.unique()).issubset(allowed_active_reasons):
        raise RuntimeError("Stage004 active branch has unexpected Stage003 reason")
    ...
    if not comparable.any() or not np.allclose(
        after[comparable],
        before[comparable] * weight[comparable],
        rtol=0.0,
        atol=1e-8,
    ):
        raise RuntimeError("Stage004 risk amount before/after does not reconcile to state-gated weight")
    coverage = float(pd.to_numeric(candidates["stage003_feature_available"], errors="coerce").fillna(0).mean())
    if coverage < 0.99 or int(active.sum()) <= 0 or int(inactive.sum()) <= 0:
        raise RuntimeError("Stage004 did not exercise both state branches with sufficient feature coverage")
    return {
        "candidate_count": int(len(candidates)),
        "feature_coverage_ratio": coverage,
        "underwater_count": int(active.sum()),
        "high_water_count": int(inactive.sum()),
        "underwater_quality_count": int(active_reason.eq("quality_risk_increase").sum()),
        "underwater_other_count": int(active_reason.eq("other_risk_decrease").sum()),
...
def _feature_audit(candidates: pd.DataFrame) -> pd.DataFrame:
    return (
        candidates.groupby(
            ["requested_start_month", "stage004_gate_reason", "stage003_reason"],
            dropna=False,
        )
        .agg(
            ...
            average_budget_weight=("stage004_budget_weight", "mean"),
        )
        .reset_index()
        .sort_values(["requested_start_month", "stage004_gate_reason", "stage003_reason"])
    )
...
def _plot(curves: pd.DataFrame) -> None:
    ...
    for axis, title, label in zip(
        axes,
        ("Stage004 A/C absolute equity", "Normalized NAV by start", "Drawdown"),
        ("equity", "NAV", "drawdown %"),
    ):
...
def main() -> None:
    if (
        s003.OFFICIAL_LIVE_VERSION != s003.EXPECTED_VERSION
        or abs(float(s003.OFFICIAL_LIVE_CAPITAL) - EXPECTED_CAPITAL) > 1e-9
    ):
    ...
    for index, start in enumerate(STARTS, start=1):
        print(f"[stage004] {index}/{len(STARTS)} A start={start.date()}", flush=True)
        ...
        print(f"[stage004] {index}/{len(STARTS)} C start={start.date()}", flush=True)
    ...
    frames = {
        name: pd.concat(items, ignore_index=True, sort=False) if items else pd.DataFrame()
    ...
    ai_audit = s003._ai_audit(
        candidate_evidence,
        summary[summary["variant"].astype(str).eq(VARIANT)].copy(),
    )
    ...
    if int(ai_audit["status"].eq("FAIL").sum()) != 0:
        raise RuntimeError("Stage004 AI month audit failed")
    if any(s003.is_ai_derived_field(column) for column in STAGE004_AUDIT_FIELDS):
        raise RuntimeError("Stage004 introduced an AI-derived feature")
    ...
    decision = {
        ...
        "overfit_before": "高；水下状态门由 Stage003 路径归因形成，但没有新增数值阈值。",
        "overfit_after": "待独立 review；没有扫描阈值、权重、品种或年份。",
    ...
    REPORT_PATH.write_text(
        "\n".join(
            [
                "# Stage004 水下才启用质量风险转移 A/B",
                "",
                f"- 生成时间：`{decision['generated_at']}`",
                f"- 决策：`{decision['decision']}`",
                f"- 四锚点硬门：`{gate}`",
                "- 新增 AI 特征：`0`；高水位完全保持 A 风险，水下才启用 Stage003 风险转移。",
                "",
                "## A/C 摘要",
                "",
                _md_table(summary),
    ...
    input_paths = [
        ...
        Path(s003.OFFICIAL_LIVE_AI_ELIGIBILITY_PATH),
        s003.REPAIRED_MINUTE_PATCH_PATH,
        s003.REPAIRED_MINUTE_AUDIT_PATH,
        Path(s003.s000.DECISION_PATH),
        PORTFOLIO_DIR / "qmt_roll_official_live_config.py",
        Path(s003.s847.__file__),
    ]
    missing = [str(path) for path in input_paths if not path.exists()]
    if missing:
        raise RuntimeError(f"Stage004 missing input files: {missing}")
    pd.DataFrame(
    ...
    pd.DataFrame(
        [{"path": str(path), "bytes": path.stat().st_size, "sha256": _sha256(path)} for path in outputs]
