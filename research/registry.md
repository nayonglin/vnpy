...
| --- | --- | --- | --- | --- | --- | --- |
| `futures_trend` | 期货趋势策略 | 商品期货趋势/Stage78历史基准与执行安全资产 | 当前官方实盘默认已按用户 operator override 与账户补资，从 Stage372/20w 经 C9/30w 切换为 C9/15w `official_live_stage847_c9_15w_stage819_05r_stop_retry_once`；C9/30w 为 previous capital profile，Stage372/20w 为 legacy previous live default；CTP/SimNow daily gate、broker-test/SimNow 1手测试链路、普通 SimNow 开平仓/撤单/断网和执行安全验收继续作为当前官方实盘流程复用的执行资产 | Stage094：C9/15w official live 资金口径切换，账户只读 `balance/available=150000.449813`、持仓 `confirmed_flat`、订单 API `0`；官方 shadow 冷启动日 `2026-06-16`，当前 `2026-06-16 12:56` 因最新完成交易日仍为 `2026-06-15`，状态 `target_date_before_live_shadow_start_waiting_fail_closed` | `research/lines/futures_trend/` | 每日官方 shadow 必须先读取 `qmt_roll_official_live_config.py` 的 current live default 与 `OFFICIAL_LIVE_SHADOW_ANALYSIS_START_DATE`；真实执行仍需 read-only、dry-run、broker-state reconciliation 和显式下单确认；空仓账户不得执行历史 shadow 平仓回放 |
...
| `futures_trend_stage819_intraday_rules` | Stage819/C9 日内规则线 | 商品期货趋势/C9 15万实盘默认 | C9 已按用户 operator override 与账户补资切为当前 live default，当前部署资金口径 `15w`，shadow 冷启动日 `2026-06-16`；该线记录 C9 从 Stage819 上游候选、C2 intraday stop、broker10 cap 到 `0.5R` stop/retry once 的完整法证、补数、注册、shadow 和 Phase D 链路 | Stage094：C9/15w live default 切换，账户只读 `150000.449813` 且 `confirmed_flat`，Stage922 在 6/16 12:56 正确等待新冷启动日；Stage093 已完成 15w 半年起点审计，成熟窗口 `16/16` 正收益但仍有 DD/保证金尾部 | `research/lines/futures_trend_stage819_intraday_rules/` | 不再扫 R 倍数、重试次数、月份、品种、方向或窗口；下一步只在 2026-06-16 日线完成后跑冷启动 shadow、pending/diagnostics 复核、read-only、dry-run、账户持仓对账和 fail-closed 执行治理 |
