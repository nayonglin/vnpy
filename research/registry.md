...
| `futures_trend_drawdown30_preserve_return` | 期货趋势回撤30以内保收益线 | 商品期货趋势/第78-1风险压缩 | 独立研究线；当前最强单策略仍是`C3_supply_headwind`，全样本收益效率最好但裸策略最大回撤卡在`-31.08%`；多数补丁型方向已被反证；Stage055/067 明确正常成本部署层现金边界可达标但高滑点不达标；Stage059 已补齐并反证供需强逆风2020-2026直套路线；Stage060 已反证单品种黑名单/预声明行业组删减路线；Stage063 已反证风险簇热度快照降仓语义路线；Stage064 已反证当前月度季节性卫星形状；Stage066 已反证当前 xsmom 期货卫星承载形状；Stage068 已反证当前期限结构动态斜率卫星形状；Stage070 已反证当前50万内5%股票paper腿真实承载；Stage072 已反证当前25,000元ETF小腿真实整手承载的稳健性；Stage074 确认2015-2019基差可补、CZCE仓单部分可补但SHFE/DCE仓单缺口明显；Stage075/078 确认`50万C3+30万独立股票账户`为组合层forward paper黄灯候选；Stage079 复验`50万C3下单+11.5万外部现金`为正常成本部署黄灯候选；Stage080 复验`50万C3+30万股票账户+11.5万现金`更平滑但资本占用过重；Stage081 反证当前 CFFEX 金融期货简单动量承载，改善几乎等同现金稀释；Stage082 确认 Stage079 在平滑度/停滞期审计中也是当前主候选 | Stage082：Stage079 `50万C3+11.5万现金` 公共样本 `4746.6341%/-29.7007%/Sharpe1.3074/Ulcer15.1012`，相对 C3 收益保留 `81.3008%`、相对78-1收益保留 `95.6601%`、最大回撤改善 `10.4651pp`、Ulcer改善 `27.3585%`，无年度亏损和低于5%增长年份；Stage080 更平滑 `3212.4243%/-27.4358%` 但收益保留仅 `55.0227%` | `research/lines/futures_trend_drawdown30_preserve_return/` | 若接受正常成本和 `61.5万` 账户口径，优先把 Stage079 作为影子盘/部署账户边界审计；若强调“明显更平滑且愿意牺牲收益/占用91.5万资金”，Stage080 可作为 paper 体验备选；若要求高滑点也保收益、单策略不加现金、或不接受外部现金，则必须寻找更强独立收益源/费用敏感度更低的新承载工具；当前组合层候选只允许日更forward paper和只读归因，黄灯不得调股票权重或参数；2015-2019供需补齐只作为长期历史报告完整性任务；禁止继续扫静态现金留白、同源趋势周期/权重小数、现有卫星权重小数、`3万现金+xsmom overlay`、`35/15`、`min1_cheapest`、`max_risk_per_trade`相邻档位、供需阈值/有效期/组件权重小数、单品种黑名单、行业组删除、热度快照、季节性月份/权重、当前期限结构动态斜率窗口/合约选择/权重、股票paper `4%/6%/7%` 权重救援、2.5万个股整手股票腿救援、ETF `4%/6%/7%/8%` 权重/代码/参数救援、金融期货简单动量 `20/60/120` 相邻窗口和 `5%/10%/20%` 小权重救援、为了救2024窗口调整股票权重或参数 |
