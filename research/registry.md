...
更新时间：2026-06-16 14:23 CST

## 当前研究线

| line_id | 中文名 | 资产/策略 | 当前状态 | 最新关键阶段 | 主要记录目录 | 下一步 |
| --- | --- | --- | --- | --- | --- | --- |
| `futures_trend` | 期货趋势策略 | 商品期货趋势/Stage78历史基准与执行安全资产 | 当前官方实盘默认已按用户 operator override 与账户补资，从 Stage372/20w 经 C9/30w 切换为 C9/15w `official_live_stage847_c9_15w_stage819_05r_stop_retry_once`；C9/30w 为 previous capital profile，Stage372/20w 为 legacy previous live default；CTP/SimNow daily gate、broker-test/SimNow 1手测试链路、普通 SimNow 开平仓/撤单/断网和执行安全验收继续作为当前官方实盘流程复用的执行资产；C9/15w 定时报告链路、会话级执行链路和 1手真实报撤 smoke 已验收 | Stage099：C9/15w Stage930/931 session daemon 已切为 scheduled `live-real`；日盘 `08:55`、夜盘 `20:55` launchd 均为 `--mode live-real --submit-mode live-real` 且注入 real-submit env。手动 live-real/no-intent 单周期订单 API `0`、Stage931 因 `no_ready_stage905_intents` 阻断、账户 `confirmed_flat`、Stage906 aligned；当前两个 label `not running`，等待定时触发 | `research/lines/futures_trend/` | 今晚 `20:55` 检查 night session daemon heartbeat/events，`21:05/21:10` 检查 timed report/latest summary；若有真实订单，立即做 TCA/成交/撤单/账户持仓对账；空仓账户不得执行历史 shadow 平仓回放 |
...
| `futures_trend_stage819_intraday_rules` | Stage819/C9 日内规则线 | 商品期货趋势/C9 15万实盘默认 | C9 已按用户 operator override 与账户补资切为当前 live default，当前部署资金口径 `15w`，shadow 冷启动日 `2026-06-16`；该线记录 C9 从 Stage819 上游候选、C2 intraday stop、broker10 cap 到 `0.5R` stop/retry once 的完整法证、补数、注册、shadow、Phase D、定时报告与会话级自动化执行链路 | Stage099：Stage930/931 session daemon 已切为 scheduled `live-real`；手动 live-real/no-intent 单周期通过 fail-closed 验证，订单 API `0`；两个 launchd label 已重载，当前 `not running`，夜盘 `20:55` 自动触发 | `research/lines/futures_trend_stage819_intraday_rules/` | 不再扫 R 倍数、重试次数、月份、品种、方向或窗口；今晚观察 `20:55` night session heartbeat/events 和 `21:05/21:10` 报告；若有真实订单，立即做 TCA/成交/撤单/账户持仓对账 |
