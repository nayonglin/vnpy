...
更新时间：2026-06-16 13:15 CST

## 当前研究线

| line_id | 中文名 | 资产/策略 | 当前状态 | 最新关键阶段 | 主要记录目录 | 下一步 |
| --- | --- | --- | --- | --- | --- | --- |
| `futures_trend` | 期货趋势策略 | 商品期货趋势/Stage78历史基准与执行安全资产 | 当前官方实盘默认已按用户 operator override 与账户补资，从 Stage372/20w 经 C9/30w 切换为 C9/15w `official_live_stage847_c9_15w_stage819_05r_stop_retry_once`；C9/30w 为 previous capital profile，Stage372/20w 为 legacy previous live default；CTP/SimNow daily gate、broker-test/SimNow 1手测试链路、普通 SimNow 开平仓/撤单/断网和执行安全验收继续作为当前官方实盘流程复用的执行资产；C9/15w 定时报告链路已通过用户级 `launchd` 部署 | Stage095：C9/15w timed-cycle 自动报告部署完成，`16:35` post-close、`21:05` evening-report 两个 `launchd` label 已加载；手动自检 `wrapper_exit_code=0`、订单 API `0`、账户只读 `balance/available=150000.449813`、非零持仓 `0`；真实报单开关仍关闭 | `research/lines/futures_trend/` | 今晚 `21:05` 后检查 latest 报告；每日官方 shadow 必须先读取 current live default 与 `OFFICIAL_LIVE_SHADOW_ANALYSIS_START_DATE`；真实执行仍需 read-only、dry-run、broker-state reconciliation、Stage927 arming gate 和显式下单确认；空仓账户不得执行历史 shadow 平仓回放 |
...
| `futures_trend_stage819_intraday_rules` | Stage819/C9 日内规则线 | 商品期货趋势/C9 15万实盘默认 | C9 已按用户 operator override 与账户补资切为当前 live default，当前部署资金口径 `15w`，shadow 冷启动日 `2026-06-16`；该线记录 C9 从 Stage819 上游候选、C2 intraday stop、broker10 cap 到 `0.5R` stop/retry once 的完整法证、补数、注册、shadow、Phase D 与定时报告链路 | Stage095：新增 Stage929 timed-cycle 包装器和两个 `launchd` 任务，`16:35`/`21:05` 自动生成固定 latest 报告；手动自检 `wrapper_exit_code=0`、`order_api_called_count=0`、账户 `150000.449813` 且非零持仓 `0`；Codex 线程心跳 `c9-15w-evening-report-summary` 已创建 | `research/lines/futures_trend_stage819_intraday_rules/` | 不再扫 R 倍数、重试次数、月份、品种、方向或窗口；今晚 `21:05` 后检查 latest 报告、pending/diagnostics、read-only、dry-run 和 fail-closed 状态；真实无人值守报单仍需 live-submit adapter、Stage927 arming gate、账户/持仓强对账和 kill switch 验收 |
