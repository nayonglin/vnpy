"""Project startup guard for vn.py runtime paths.

When commands set ``PYTHONPATH`` to this repository, Python imports this module
before user code.  We temporarily switch to the repository root while importing
``vnpy.trader.utility`` so vn.py binds ``TRADER_DIR`` to the project
``.vntrader`` directory, then restore the original working directory.
"""
